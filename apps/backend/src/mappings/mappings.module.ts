...
@Module({ imports: [AssetsModule, ProjectsModule], controllers: [ProjectAssetMappingsController, LegacyReferenceMigrationController], providers: [
  ...
  { provide: ProjectAssetMappingsService, useFactory: (repository: LocalProjectAssetMappingsRepository, assets: LocalAssetsRepository, projects: LocalProjectRepository) => new ProjectAssetMappingsService(repository, assets, new ShortProjectMappingOwners(repository, projects)), inject: [LocalProjectAssetMappingsRepository, LocalAssetsRepository, LocalProjectRepository] },
