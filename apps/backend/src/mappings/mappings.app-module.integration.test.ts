...
describe.sequential("Project Asset Mapping HTTP routes", () => {
  it("creates a mapping, snapshots it, and rejects approval before a review starts", async () => {
    ...
    const project = createStoredProject("mapping_http", "Mapping HTTP", "2026-08-22T00:00:00.000Z"); project.script_revision = 1; project.scenes = [1, 2, 3, 4, 5, 6].map((number) => ({ number, description: String(number) })); await projects.create(project);
