...
describe("AudioLibraryService", () => {
  ...
  it("uploads a track, probes its real duration, and lists it newest first", async () => {
    ...
    const result = await service.upload({ buffer: MP3, originalname: "잔잔한 배경음.mp3", mimetype: "audio/mpeg" }, { title: "잔잔한 배경음", artist: "무명" });

    expect(result.track).toMatchObject({ title: "잔잔한 배경음", artist: "무명", durationSeconds: 128.4, bytes: MP3.length, source: "upload" });
  ...
  it("defaults the title to the original filename when none is given", async () => {
    const { service } = await setup();
    const result = await service.upload({ buffer: MP3, originalname: "track.mp3" }, undefined);
  ...
  it("lists newest upload first", async () => {
    const { service } = await setup();
    await service.upload({ buffer: MP3, originalname: "first.mp3" }, { title: "first" });
    await new Promise((resolve) => setTimeout(resolve, 5));
    await service.upload({ buffer: MP3, originalname: "second.mp3" }, { title: "second" });
  ...
  it("rejects a file that is not decodable as audio, and does not leave the bytes on disk", async () => {
    ...
    await expect(service.upload({ buffer: MP3, originalname: "not-audio.mp3" }, undefined))
  ...
  it("rejects an unsupported file extension", async () => {
    const { service } = await setup();
    await expect(service.upload({ buffer: MP3, originalname: "track.txt" }, undefined))
      .rejects.toMatchObject({ response: { code: "AUDIO_FILE_INVALID" } });
  });
  ...
  it("rejects an oversized file without probing it", async () => {
    ...
    await expect(service.upload({ buffer: big, originalname: "big.mp3" }, undefined))
      .rejects.toMatchObject({ response: { code: "AUDIO_FILE_INVALID" } });
  });

  it("rejects an empty file", async () => {
    const { service } = await setup();
    await expect(service.upload({ buffer: Buffer.alloc(0), originalname: "empty.mp3" }, undefined))
      .rejects.toMatchObject({ response: { code: "AUDIO_FILE_INVALID" } });
  });

  it("rejects a path-traversal filename", async () => {
    const { service } = await setup();
    await expect(service.upload({ buffer: MP3, originalname: "../../etc/passwd.mp3" }, undefined))
      .rejects.toMatchObject({ response: { code: "AUDIO_FILE_INVALID" } });
  });

  it("rejects an unknown body field", async () => {
    const { service } = await setup();
    await expect(service.upload({ buffer: MP3, originalname: "track.mp3" }, { unexpected: "x" }))
  ...
  it("streams a track's own file by trackId, and 404s for an unknown one", async () => {
    const { service } = await setup();
    const { track } = await service.upload({ buffer: MP3, originalname: "track.mp3" }, undefined);
