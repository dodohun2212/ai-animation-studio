...
import { storedGenerationSource } from "../videos/generation-source.js";
...
export class EpisodeScriptsService {
  ...
  private async outline(projectId: string, number: number): Promise<LongEpisodeOutline> {
    ...
    const raw = await readLongProjectJson(this.files(projectId, number).outlines);
  ...
  /**
   * Reads one Episode's stored record, validating the fields this service owns and preserving the rest.
   *
   * `...d` first, then the checked values over the top. It used to name every field it kept and silently drop
   * everything else — and `save()` writes this object straight back, so a parser that forgot a key deleted it
   * from disk on the next script save. Two were being lost on 캡틴D's real Episodes:
   *
   *   previous_instagram_posts   posts published and then forgotten. Its own doc comment calls it "the only
   *                              memory this app has of an action it cannot undo or re-check", and Episode 4 had
   *                              three of them on disk while the API answered with none.
   *   mapping_revision           written by episode-mapping-owner.ts's markMappingApproved, thrown away here.
   *
   * The rule that keeps this from happening a third time: validate what this service understands, carry what it
   * does not. Another module's record is not this parser's to decide about, and dropping it is a decision — one
   ...
   * not collapse.
   *
   * It survives on purpose: the outline is the editable truth for the six text fields (someone can rewrite a
   * title after the Episode exists), and the script goes through this service's own validating parse rather
   * than the lenient one. What it does not get to do is quietly know fewer fields — it was missing
   * `previousInstagramPosts`, so 캡틴D's Episode 4 reported no forgotten posts while three sat on disk. Exactly
   ...
   */
  private toApi(outline: LongEpisodeOutline, stored: StoredEpisode): LongEpisodeDetail { const script = Object.keys(stored.script).length ? this.parseScript(stored.script, stored.scene_count) : undefined; const storedWarnings = Array.isArray((stored as unknown as Record<string, unknown>).warnings) ? ((stored as unknown as Record<string, unknown>).warnings as unknown[]).filter((item): item is string => typeof item === "string") : []; const warnings = [...new Set([...(outline.warnings ?? []), ...storedWarnings])]; return { ...outline, ...(warnings.length > 0 ? { warnings } : {}), status: stored.state, approved: stored.approved, scriptRevision: stored.script_revision, ...(script ? { script } : {}), scriptHistoryCount: stored.script_history.length, sceneSubtitleLayout: isSceneSubtitleLayout({ scale: (stored as Record<string, unknown>).scene_subtitle_scale, center: (stored as Record<string, unknown>).scene_subtitle_center }) ? { scale: (stored as Record<string, unknown>).scene_subtitle_scale as number, center: (stored as Record<string, unknown>).scene_subtitle_center as number } : DEFAULT_SCENE_SUBTITLE_LAYOUT, updatedAt: typeof stored.updated_at === "string" ? stored.updated_at : new Date(0).toISOString(), ...(toEpisodeInstagramPost(stored.instagram_post) ? { instagramPost: toEpisodeInstagramPost(stored.instagram_post)! } : {}), ...(toEpisodePreviousInstagramPosts(stored.previous_instagram_posts).length > 0 ? { previousInstagramPosts: toEpisodePreviousInstagramPosts(stored.previous_instagram_posts) } : {}), ...(toEpisodeUsedAudio(stored.used_audio) ? { usedAudio: toEpisodeUsedAudio(stored.used_audio)! } : {}), ...(episodeErrors(stored).length > 0 ? { errors: episodeErrors(stored) } : {}), ...(typeof (stored as Record<string, unknown>).final_video_path === "string" && (stored as Record<string, unknown>).final_video_path ? { finalVideoPath: (stored as Record<string, unknown>).final_video_path as string, openablePath: episodeProjectRelativePath(stored.number, (stored as Record<string, unknown>).final_video_path as string), finalVideoGenerationSource: storedGenerationSource((stored as Record<string, unknown>).final_video_generation_source) } : {}) }; }
  private async save(projectId: string, outline: LongEpisodeOutline, stored: StoredEpisode): Promise<LongEpisodeDetail> { const files = this.files(projectId, outline.episodeNumber); const outlines = await readLongProjectJson(files.outlines); if (!Array.isArray(outlines)) throw longInvalidData(); const copy = [...outlines]; const current = asObject(copy[outline.episodeNumber - 1]); current.status = stored.state; copy[outline.episodeNumber - 1] = current; try { await fs.mkdir(files.episode, { recursive: true }); await Promise.all([atomicWriteUtf8File(files.episodeProject, JSON.stringify(stored, null, 2)), atomicWriteUtf8File(files.outline, JSON.stringify(stored.outline, null, 2)), atomicWriteUtf8File(files.script, JSON.stringify(stored.script, null, 2)), atomicWriteUtf8File(files.outlines, JSON.stringify(copy, null, 2))]); } catch { throw longStorageError(); } await this.seedStoryBibleMappings(projectId); return this.toApi({ ...outline, status: stored.state }, stored); }
  private async bibleContext(projectId: string): Promise<string> { const bible = asObject(await readLongProjectJson(this.files(projectId, 1).bible)); const names = ["characters", "locations", "props"].flatMap((collection) => Array.isArray(bible[collection]) ? bible[collection].map((item) => asObject(item).name).filter((name): name is string => typeof name === "string" && Boolean(name.trim())) : []); return names.join(", "); }
  /**
   ...
   * `readLongProjectJson()` raises LONG_PROJECT_JSON_MALFORMED for bad JSON and only 404 was caught, so a single corrupt
  ...
  private async buildContext(projectId: string, outline: LongEpisodeOutline, stored: StoredEpisode, continuity: ContinuityContext, userInstruction = ""): Promise<Record<string, unknown>> {
    const bible = asObject(await readLongProjectJson(this.files(projectId, 1).bible));
  ...
  private buildScriptPrompt(context: Record<string, unknown>, sceneCount: number, clipDurationSeconds: number): string {
    return [
      ...
      NO_TEXT_AS_EVENT_RULE,
      // Episode 6 scene 6 failed at Runway with INTERNAL.BAD_OUTPUT.CODE01 on 2026-09-06, and its prompt asked
      // one five-second shot for two camera moves and three separate deformations of a face. Nothing here said
