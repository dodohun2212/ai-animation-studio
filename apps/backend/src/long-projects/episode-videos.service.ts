...
export class EpisodeVideosService implements OnModuleDestroy {
  ...
  private progressFor(episode: Episode, job: string, records: VideoRecord[]): LongEpisodeVideoProgress { const done = records.filter((item) => item.status === "succeeded").map((item) => item.scene_number); const failedRecords = records.filter((item) => item.status === "failed"); const failed = failedRecords.map((item) => item.scene_number); const sceneErrors = Object.fromEntries(failedRecords.filter((item) => item.error).map((item) => [item.scene_number, item.error!])); const running = records.find((item) => item.status === "running")?.scene_number; return { jobId: job, status: episode.state === "interrupted" ? "interrupted" : failed.length > 0 ? "failed" : done.length === 6 ? "succeeded" : running ? "running" : "created", ...(running ? { currentSceneNumber: running } : {}), completedSceneNumbers: done, failedSceneNumbers: failed, episode: this.detail(episode), ...(Object.keys(sceneErrors).length > 0 ? { sceneErrors } : {}) }; }
  ...
  async preview(projectId: string, number: number): Promise<GetLongEpisodeVideoPreviewResponse> { const id = projectId.trim(); const episode = await this.loadEpisode(id, number); await this.assertReady(id, number, episode); const scenes = this.scenes(episode); const items = scenes.map((item, index) => ({ sceneNumber: SCENES[index]!, prompt: this.prompt(item, scenes[index - 1]), estimatedCostUsd: VIDEO_SCENE_ESTIMATED_COST_USD })); const hash = crypto.createHash("sha256").update(id).update(String(number)); for (const item of items) { hash.update(await fs.readFile(this.image(id, number, item.sceneNumber))); hash.update(item.prompt); } const estimatedCostUsd = items.reduce((sum, item) => sum + item.estimatedCostUsd, 0);
    ...
    return { confirmationId: hash.digest("hex"), model: "gen4_turbo", ratio: "720:1280", durationSecondsPerScene: 5, executionMode: "sequential", scenes: items, estimatedCostUsd, maximumProviderCalls: SCENES.length, ...(budget ? { budget } : {}) }; }
