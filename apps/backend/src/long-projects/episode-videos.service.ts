...
import { recordedVideoModel, resolveVideoModel } from "../videos/runway-video-adapter.js";
...
export class EpisodeVideosService implements OnModuleDestroy {
  ...
  /** The same RUNWAY_RATIO_FOR_ASPECT table as video-preview.service.ts's ratioFor() for the short-project side, read from the field episode-video-merge.service.ts's ratio() reads. */
  private async ratio(id: string, number: number): Promise<RunwayVideoRatio> {
    ...
    return RUNWAY_RATIO_FOR_ASPECT[raw.aspect_ratio];
  }
  ...
  private async runwayInputForScene(id: string, number: number, records: VideoRecord[], sceneNumber: SceneNumber) {
    ...
    // 🔴 The record's model. This passed none, so every Episode clip went to the adapter's default whatever had
    // been quoted and confirmed — invisible while there was one model, a different model and price the day
    // there were two.
    return { imageBytes, imageMimeType: "image/png", prompt: record.prompt, model: recordedVideoModel(record.model), durationSeconds: this.durationSecondsPerScene(episode), ratio };
  }
...
${additionalInstruction}`; }
  ...
  /**
   * The clips this Episode has, whatever stage it is at now.
   *
   * This used to require `videos_review` or `videos_approved`, so an Episode that had been merged answered its
   * own review list with 409 — Cowork measured exactly that on Episode 5 after it reached `completed`, and had
   * to wrap three separate calls on the screen because a finished Episode reports that video work "is not
   * allowed" when nothing was being attempted but a read.
   *
   * The image side has drawn this line since it was written, and says so in its own pair: **reading is not
   * reviewing**. The acts still refuse at a stage that has moved on — `approve` below is untouched — and this
