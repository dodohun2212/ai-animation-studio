...
type Code = "INVALID_REQUEST" | "UNSAFE_PROJECT_ID" | "LONG_PROJECT_NOT_FOUND" | "LONG_PROJECT_ALREADY_EXISTS" | "LONG_PROJECT_JSON_MALFORMED" | "LONG_PROJECT_DATA_INVALID" | "LONG_PROJECT_STORAGE_ERROR" | "LONG_PROJECT_ARCHIVE_NOT_ALLOWED" | "LONG_PROJECT_ASPECT_RATIO_LOCKED" | "LONG_PROJECT_EPISODE_COUNT_LOCKED" | "LONG_PROJECT_ARCHIVE_COLLISION" | "LONG_PROJECT_RESTORE_COLLISION" | "LONG_OUTLINE_STALE" | "LONG_OUTLINE_NOT_ALLOWED" | "LONG_OUTLINE_BUDGET_EXCEEDED" | "LONG_OUTLINE_PROVIDER_ERROR" | "LONG_EPISODE_NOT_FOUND" | "LONG_EPISODE_TIMELINE_NOT_ALLOWED" | "LONG_EPISODE_LIMIT_REACHED" | "LONG_EPISODE_SCRIPT_NOT_ALLOWED" | "LONG_EPISODE_SCRIPT_EXISTS" | "LONG_EPISODE_SETTINGS_NOT_ALLOWED" | "LONG_EPISODE_SCRIPT_BUDGET_EXCEEDED" | "LONG_EPISODE_SCRIPT_PROVIDER_ERROR" | "LONG_EPISODE_MAPPING_NOT_ALLOWED" | "LONG_EPISODE_MAPPING_NOT_FOUND" | "LONG_EPISODE_MAPPING_STALE" | "LONG_EPISODE_MAPPING_UNCONFIRMED" | "LONG_EPISODE_IMAGES_NOT_ALLOWED" | "LONG_EPISODE_IMAGES_INVALID" | "LONG_EPISODE_IMAGES_BUDGET_EXCEEDED" | "LONG_EPISODE_IMAGES_PROVIDER_ERROR" | "LONG_EPISODE_VIDEOS_NOT_ALLOWED" | "LONG_EPISODE_VIDEOS_INVALID" | "LONG_EPISODE_VIDEO_ASPECT_UNSUPPORTED" | "LONG_EPISODE_VIDEO_DURATION_OUT_OF_RANGE" | "LONG_EPISODE_RETRY_NEEDS_CHANGED_INPUT" | "LONG_EPISODE_VIDEO_JOB_NOT_FOUND" | "LONG_EPISODE_VIDEO_VERSION_NOT_FOUND" | "LONG_EPISODE_VIDEO_RESTORE_NOT_ALLOWED" | "LONG_EPISODE_VIDEO_RESTORE_IN_PROGRESS" | "LONG_EPISODE_MERGE_NOT_ALLOWED" | "LONG_EPISODE_MERGE_ALREADY_COMPLETED" | "LONG_EPISODE_MERGE_BUSY" | "AUDIO_START_OUT_OF_RANGE" | "LONG_EPISODE_MERGE_CLIPS_INVALID" | "LONG_EPISODE_FFMPEG_UNAVAILABLE" | "LONG_EPISODE_MERGE_FAILED" | "LONG_EPISODE_CONTINUITY_NOT_ALLOWED" | "LONG_EPISODE_CONTINUITY_INVALID" | "LONG_EPISODE_NARRATION_NOT_ALLOWED" | "LONG_EPISODE_NARRATION_NOT_ENABLED" | "LONG_EPISODE_NARRATION_MISSING_TEXT" | "LONG_EPISODE_NARRATION_GENERATION_FAILED" | "LONG_EPISODE_NARRATION_STORAGE_ERROR" | "LONG_EPISODE_NARRATION_BUDGET_EXCEEDED" | "LONG_EPISODE_NARRATION_PROVIDER_ERROR" | "LONG_EPISODE_NARRATION_CONTENT_UNAVAILABLE" | "STORY_BIBLE_ITEM_NOT_FOUND" | "STORY_BIBLE_ITEM_ALREADY_EXISTS" | "PROJECT_LOCKED" | "BUDGET_LEDGER_UNREADABLE";
 ...
 *
 * Its own code, like the short project's: a screen has something specific to say here, and what this refuses is
 * the app spending money on a repeat of a failure whose cause it already knows.
 */
export const longEpisodeRetryNeedsChangedInput = () => new LongProjectApiException("LONG_EPISODE_RETRY_NEEDS_CHANGED_INPUT", "This scene failed because of its input, so the same request would fail again. Say what to change before retrying.", HttpStatus.BAD_REQUEST);
...
/**
 * Something else holds this Episode's final video — a merge, a publish, or another restore.
 *
 * 🔴 Its own code rather than LONG_EPISODE_MERGE_BUSY, whose sentence would in fact read correctly here.
 * The name would not: a restore that reports a merge code sends the reader to the merge screen looking for a
 * render that is not the thing blocking them. And not RESTORE_NOT_ALLOWED either — that one means the
 * Episode's state forbids this, a fact that does not change while you wait, and this one clears by itself.
 *
...
/**
 * The memo a person is trying to save does not hold together — a field missing, a list too long, an entry that
 * is not text.
 *
 * Its own code rather than INVALID_REQUEST, because the screen had already written the sentence for it and could
 * never show it: `longProjectsApi.ts` has carried a LONG_EPISODE_CONTINUITY_INVALID entry — "연결 기억을
 * 저장하려면 검토한 값이 올바르게 채워져 있어야 합니다" — for a code nothing in this app has ever thrown, so the
 * refusal arrived as INVALID_REQUEST and rendered as the catch-all "입력 내용을 확인해 주세요".
 *
 * Both halves existed and neither could reach the other. Naming the code is the whole fix: the screen's own
 * sentence, which says *which* thing to look at, becomes reachable without the screen changing at all.
 *
 * 400 like the request error it replaces on this path — what is refused is still the request, and the status is
 * what callers other than this screen branch on.
 */
export const longEpisodeContinuityInvalid = () => new LongProjectApiException("LONG_EPISODE_CONTINUITY_INVALID", "Episode Continuity Memory request is invalid.", HttpStatus.BAD_REQUEST);
 ...
 */
/**
 * The project's frame is one the video model is told but does not make (a square to Gemini Omni Flash). Asked when
 * the Episode's video run starts, before any record is written: the adapter refuses it too, but per scene, after
 * a job of failures is already on disk. The model and its ratio travel in `details` for the screen to name.
 */
export const longEpisodeVideoAspectUnsupported = (details: { model: string; ratio: string }) => new LongProjectApiException("LONG_EPISODE_VIDEO_ASPECT_UNSUPPORTED", `The ${details.model} model does not make ${details.ratio} clips.`, HttpStatus.CONFLICT, details);

/** The Episode's scene length is outside the video model's own range — same moment and reason as the frame above, and the short side's VIDEO_CLIP_DURATION_OUT_OF_RANGE. */
export const longEpisodeVideoDurationOutOfRange = (details: { model: string; durationSeconds: number; minDurationSeconds: number; maxDurationSeconds: number }) => new LongProjectApiException("LONG_EPISODE_VIDEO_DURATION_OUT_OF_RANGE", `The ${details.model} model makes clips of ${details.minDurationSeconds}-${details.maxDurationSeconds} seconds, not ${details.durationSeconds}.`, HttpStatus.CONFLICT, details);
