...
export class EpisodeImagesController {
  ...
  /** Takes one scene's approval back. Its own route, so an approval and a withdrawal can never be the same request with a flipped flag nobody read. */
  @Post(`${API_ROUTES.longProjects}/:projectId/episodes/:episodeNumber/images/review/:sceneNumber/unapprove`) unapprove(@Param("projectId") id: string, @Param("episodeNumber") number: string, @Param("sceneNumber") scene: string, @Body() body: UnapproveLongEpisodeImageReviewRequest): Promise<UnapproveLongEpisodeImageReviewResponse> { return this.service.unapprove(id, Number(number), scene, body); }
