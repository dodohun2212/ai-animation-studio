...
export class LongProjectsService {
  ...
  private async outlines(id: string, count: number): Promise<LongEpisodeOutline[]> { const raw = await this.readJson(this.files(id).outlines); if (!Array.isArray(raw) || raw.length !== count) throw longInvalidData(); return raw.map((item, index) => { const outline = parseEpisodeOutlineEntry(item, index + 1); const d = object(item); const warnings = withoutStaleEpisodeRecoveryWarnings(Array.isArray(d.warnings) ? d.warnings.filter((entry): entry is string => typeof entry === "string") : [], outline.status); return { ...outline, ...(warnings.length > 0 ? { warnings } : {}) }; }); }
  /**
  ...
  private async project(id: string): Promise<LongProject> { const s = await this.load(id); const bible = object(await this.readJson(this.files(id).bible)); const basic = object(bible.basic); const world = object(bible.world); return { ...summary(s), settings: toSettings(s), storyBible: { basic, world }, episodes: await this.withContinuitySaved(id, await this.outlines(id, s.episode_count)) }; }
