...
const settingKeys = ["title", "logline", "overview", "genre", "tone", "theme", "episodeCount", "sceneCount", "clipDurationSeconds", "platform", "aspectRatio", "audience", "notes", "startingState", "midpoint", "endingDirection", "storyFlowSummary"] as const;
type Stored = { project_id: string; project_type: "long_story_project"; title: string; logline: string; overview: string; genre: string; tone: string; theme: string; episode_count: number; scene_count: number; clip_duration_seconds: number; platform: "YouTube Shorts" | "YouTube"; aspect_ratio: "9:16" | "16:9"; audience: string; notes: string; starting_state: string; midpoint: string; ending_direction: string; story_flow_summary: string; created_at: string; updated_at: string; outline_status: "planned" | "outline_ready"; outline_prompt_request?: { prompt_sha256: string; prompt: string; approved_at: string; modified: boolean }; };
...
function settings(value: unknown): LongProjectSettings {
  ...
  if (data.platform !== "YouTube Shorts" && data.platform !== "YouTube") throw longInvalidRequest();
  ...
  return {
    ...
    platform: data.platform, aspectRatio: data.aspectRatio,
...
function toSettings(s: Stored): LongProjectSettings { return { title: s.title, logline: s.logline, overview: s.overview, genre: s.genre, tone: s.tone, theme: s.theme, episodeCount: s.episode_count, episodeDurationSeconds: s.scene_count * s.clip_duration_seconds, sceneCount: s.scene_count, clipDurationSeconds: s.clip_duration_seconds, platform: s.platform, aspectRatio: s.aspect_ratio, audience: s.audience, notes: s.notes, startingState: s.starting_state, midpoint: s.midpoint, endingDirection: s.ending_direction, storyFlowSummary: s.story_flow_summary }; }
function setStored(id: string, s: LongProjectSettings, now: string, createdAt = now, status: Stored["outline_status"] = "planned"): Stored { return { project_id: id, project_type: "long_story_project", title: s.title, logline: s.logline, overview: s.overview, genre: s.genre, tone: s.tone, theme: s.theme, episode_count: s.episodeCount, scene_count: s.sceneCount, clip_duration_seconds: s.clipDurationSeconds, platform: s.platform, aspect_ratio: s.aspectRatio, audience: s.audience, notes: s.notes, starting_state: s.startingState, midpoint: s.midpoint, ending_direction: s.endingDirection, story_flow_summary: s.storyFlowSummary, created_at: createdAt, updated_at: now, outline_status: status }; }
