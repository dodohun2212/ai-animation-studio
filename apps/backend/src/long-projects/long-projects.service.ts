...
export class LongProjectsService {
  ...
  private async outlines(id: string, count: number): Promise<LongEpisodeOutline[]> { const raw = await this.readJson(this.files(id).outlines); if (!Array.isArray(raw) || raw.length !== count) throw longInvalidData(); return raw.map((item, index) => { const d = object(item); if (d.episode_number !== index + 1 || typeof d.title !== "string" || typeof d.summary !== "string" || typeof d.main_event !== "string" || typeof d.conflict !== "string" || typeof d.cliffhanger !== "string" || typeof d.next_episode_hook !== "string" || !(LONG_EPISODE_STATUSES as readonly string[]).includes(d.status as string)) throw longInvalidData(); const warnings = withoutStaleEpisodeRecoveryWarnings(Array.isArray(d.warnings) ? d.warnings.filter((entry): entry is string => typeof entry === "string") : [], d.status as string); return { episodeNumber: index + 1, title: d.title, summary: d.summary, mainEvent: d.main_event, conflict: d.conflict, cliffhanger: d.cliffhanger, nextEpisodeHook: d.next_episode_hook, status: d.status as LongEpisodeOutline["status"], ...(warnings.length > 0 ? { warnings } : {}) }; }); }
  /**
