...
function settings(value: unknown): LongProjectSettings {
  ...
  return {
    ...
    // Absent reads as blank, which is what these already mean when empty: no style line. A caller that predates
    // them therefore saves exactly what it saved before instead of being refused.
...
export class LongProjectsService {
  ...
  private parseStored(value: unknown): Stored { const d = object(value); const known = new Set(["project_id", "project_type", "title", "logline", "overview", "genre", "tone", "theme", "episode_count", "episode_duration_seconds", "scene_count", "clip_duration_seconds", "platform", "aspect_ratio", "audience", "notes", "starting_state", "midpoint", "ending_direction", "story_flow_summary", "narration_enabled", "subtitles_enabled", "visual_style", "color", "lighting", "avoid", "created_at", "updated_at", "outline_status", "outline_prompt_request", "warnings"]); if (Object.keys(d).some((key) => !known.has(key))) throw longInvalidData(); try { const { sceneCount, clipDurationSeconds } = coerceSceneCountAndClipDuration(d); const { narrationEnabled, subtitlesEnabled } = coerceNarrationSettings(d); const result = setStored(text(d.project_id, true), settings({ title: d.title, logline: d.logline, overview: d.overview, genre: d.genre, tone: d.tone, theme: d.theme, episodeCount: d.episode_count, sceneCount, clipDurationSeconds, aspectRatio: d.aspect_ratio, audience: d.audience, notes: d.notes, startingState: d.starting_state, midpoint: d.midpoint, endingDirection: d.ending_direction, storyFlowSummary: d.story_flow_summary, narrationEnabled, subtitlesEnabled, visualStyle: d.visual_style, color: d.color, lighting: d.lighting, avoid: d.avoid }), text(d.updated_at, true), text(d.created_at, true), d.outline_status === "outline_ready" ? "outline_ready" : d.outline_status === "planned" ? "planned" : (() => { throw longInvalidData(); })()); if (d.project_type !== "long_story_project") throw longInvalidData(); if (d.outline_prompt_request !== undefined) result.outline_prompt_request = d.outline_prompt_request as Stored["outline_prompt_request"]; if (Array.isArray(d.warnings)) { const kept = d.warnings.filter((item): item is string => typeof item === "string"); if (kept.length > 0) result.warnings = kept; } return result; } catch (error) { if (error instanceof Error && "getStatus" in error) throw error; throw longInvalidData(); } }
  private async load(id: string): Promise<Stored> { const stored = this.parseStored(await readLongProjectJson(this.files(id).project)); if (stored.project_id !== id) throw longInvalidData(); return stored; }
  private async outlines(id: string, count: number): Promise<LongEpisodeOutline[]> { const raw = await readLongProjectJson(this.files(id).outlines); if (!Array.isArray(raw) || raw.length !== count) throw longInvalidData(); return raw.map((item, index) => { const outline = parseEpisodeOutlineEntry(item, index + 1); const d = object(item); const warnings = withoutStaleEpisodeRecoveryWarnings(Array.isArray(d.warnings) ? d.warnings.filter((entry): entry is string => typeof entry === "string") : [], outline.status); return { ...outline, ...(warnings.length > 0 ? { warnings } : {}) }; }); }
  /**
   * Whether each Episode has a continuity memo on disk.
   *
  ...
  private async outlinesForEpisodeCount(stored: Stored, next: LongProjectSettings): Promise<unknown[] | undefined> {
    ...
    const raw = await readLongProjectJson(this.files(stored.project_id).outlines) as unknown[];
  ...
  private async renderOutlinePrompt(s: Stored): Promise<string> {
    const bible = object(await readLongProjectJson(this.files(s.project_id).bible));
    const { updated_at: _updatedAt, basic: rawBasic, ...rest } = bible;
    const bibleForPrompt = { ...rest, basic: storyBibleBasicForPrompt(rawBasic) };
