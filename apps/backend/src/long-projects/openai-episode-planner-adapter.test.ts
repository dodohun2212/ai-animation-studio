...
describe("callOpenAiEpisodePlannerApi", () => {
  it("posts the strict json_schema request sized to episodeCount and returns the parsed outline", async () => {
    ...
    const result = await callOpenAiEpisodePlannerApi("sk-test", "prompt text", 2, { fetchImpl: fetchMock, sleep: noSleep });
  ...
  it("uses a caller-supplied model instead of the default", async () => {
    ...
    await callOpenAiEpisodePlannerApi("sk", "p", 2, { model: "custom-model", fetchImpl: fetchMock, sleep: noSleep });
  ...
  it("rejects an episodeCount outside 1-365 before ever calling fetch", async () => {
    ...
    await expect(callOpenAiEpisodePlannerApi("sk", "p", 0, { fetchImpl: fetchMock, sleep: noSleep })).rejects.toMatchObject({ category: "invalid_request" });
    await expect(callOpenAiEpisodePlannerApi("sk", "p", 366, { fetchImpl: fetchMock, sleep: noSleep })).rejects.toMatchObject({ category: "invalid_request" });
  ...
  it("classifies a 401 as authentication and never retries", async () => {
    ...
    await expect(callOpenAiEpisodePlannerApi("sk-bad", "p", 2, { fetchImpl: fetchMock, sleep: noSleep })).rejects.toMatchObject({ category: "authentication" });
  ...
  it("rejects an empty output_text as empty_response", async () => {
    ...
    await expect(callOpenAiEpisodePlannerApi("sk", "p", 2, { fetchImpl: fetchMock, sleep: noSleep })).rejects.toMatchObject({ category: "empty_response" });
  });
  ...
  it("rejects unparsable JSON text as invalid_response", async () => {
    ...
    await expect(callOpenAiEpisodePlannerApi("sk", "p", 2, { fetchImpl: fetchMock, sleep: noSleep })).rejects.toMatchObject({ category: "invalid_response" });
  });
  ...
  it("is an instance of OpenAiEpisodePlannerAdapterError with a Korean message", async () => {
    ...
    try {
      await callOpenAiEpisodePlannerApi("sk", "p", 2, { fetchImpl: fetchMock, sleep: noSleep });
