...
describe("OpenAiBudget", () => {
  ...
  it("only counts usage from the month the person is in", async () => {
    ...
    try {
      const root = await makeRoot();
      const budget = new OpenAiBudget(root, 10);
      await budget.record("p1", "story", true, 5, new Date("2026-07-31T14:00:00.000Z")); // KST 07-31 23:00
      await budget.record("p1", "story", true, 3, new Date("2026-07-31T15:00:00.000Z")); // KST 08-01 00:00
