...
describe("RunwayBudget", () => {
  ...
  it("records estimated cost as actual cost for both a succeeded and a failed attempt, in its own file separate from OpenAI's", async () => {
    ...
    await budget.record("p1", "video", true, 0.25, now);
    await budget.record("p1", "video", false, 0.25, now);
    ...
    expect(raw[0]).toMatchObject({ project_id: "p1", api_type: "video", estimated_cost_usd: 0.25, actual_cost_usd: 0.25, succeeded: true });
  ...
  it("only counts usage from the current UTC month", async () => {
    const root = await makeRoot();
    const budget = new RunwayBudget(root, 10);
    await budget.record("p1", "video", true, 5, new Date("2026-07-31T23:59:59.000Z"));
    await budget.record("p1", "video", true, 3, new Date("2026-08-01T00:00:00.000Z"));
  ...
  it("reloads persisted usage across separate instances and tolerates a malformed usage file", async () => {
    const root = await makeRoot();
    await new RunwayBudget(root, 10).record("p1", "video", true, 1, new Date("2026-08-22T00:00:00.000Z"));
