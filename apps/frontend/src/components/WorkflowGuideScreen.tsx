    ...
    <section className="mt-8 max-w-4xl space-y-5">
      ...
      <StageCard
        ...
        sends={[
          ...
          /* 🔴 「1장」이 고정 문구였습니다. 2026-09-11 부터 이 앱은 **두 장**을 보냅니다 — 「장면 이어 그리기」를
             켠 프로젝트에서 모델이 끝 그림을 받으면, 클립 N 이 그림 N+1 로 끝나도록 다음 장면 그림이 같이
             나갑니다(CLI Round 789). 꽃말_버즘나무가 정확히 그 경우였고, 이 화면은 **무엇이 나가고 얼마가
             드는지 말하는 화면**인데 나가는 것 하나를 빠뜨리고 있었습니다.

             조건을 문장에 넣습니다 — 못 받는 모델에서는 여전히 한 장이고, 「1장」이라고만 적으면 받는 모델을
             고른 사람이 틀린 말을 읽습니다. 이 화면엔 「이어 그리기」 스위치가 없으므로(비용이 안 바뀌어
             계산 조건이 아닙니다) 그 조건도 문장으로 말합니다. */
          videoModel.acceptsLastFrame
            ? "확정한 장면 이미지 — 첫 프레임 1장. 「장면 이어 그리기」를 켠 프로젝트에서는 다음 장면 그림이 끝 프레임으로 1장 더 갑니다."
            : "확정한 장면 이미지 1장 (첫 프레임)",
        ]}
        receives={
