...
describe("LongProjectDetail", () => {
  ...
  /**
   * 🔴 전체 에피소드 is counted once, in the stage summary — and this pair is what says so.
   *
   * The number was moved here on purpose, out of the field list that used to repeat it. That decision then
   * lived only in a comment beside the panel, and a comment speaks to whoever reads it: while adding a header
   * to this screen the count went straight back into the header, because the header's row of two facts looked
   * a little thin with two. It was caught by eye in a browser, which is not a thing to rely on twice.
   */
  it("counts 전체 에피소드 once — the stage summary owns it, the header does not repeat it", async () => {
    const project = makeLongProject({
      id: "long_test",
      episodes: [
        makeLongEpisodeOutline({ episodeNumber: 1, status: "completed" }),
        makeLongEpisodeOutline({ episodeNumber: 2, status: "planned" }),
      ],
    });
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(jsonResponse(200, { project })));
    ...
    const panel = await screen.findByTestId("episode-stage-summary");
    expect(within(panel).getByTestId("episode-stage-전체 에피소드").textContent).toContain("2");
    expect(screen.getAllByText("전체 에피소드")).toHaveLength(1);
  });
