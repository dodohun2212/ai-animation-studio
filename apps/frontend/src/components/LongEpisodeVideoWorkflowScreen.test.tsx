...
describe("LongEpisodeVideoWorkflowScreen", () => {
  ...
  it("does not submit until final local confirmation and sends the exact explicit request", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Start local fake videos" })); await screen.findByTestId("episode-video-progress");
  ...
  it("renders persisted sequential progress, stop/restart, and review approval/regeneration confirmations", async () => {
    ...
    fireEvent.click(screen.getByTestId("episode-video-open-confirm")); fireEvent.click(screen.getByRole("button", { name: "Start local fake videos" })); await screen.findByTestId("episode-video-progress");
