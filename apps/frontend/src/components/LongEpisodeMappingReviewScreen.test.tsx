...
describe("LongEpisodeMappingReviewScreen", () => {
  ...
  it("reviews scoped candidates and submits no final approval until its separate confirmation", async () => {
    ...
    expect(await screen.findByText("Character: hero")).toBeTruthy();
    expect(screen.getByText(/Script revision: 3/)).toBeTruthy();
    ...
    fireEvent.click(screen.getByRole("button", { name: "Final approval" }));
    ...
    await waitFor(() => expect(screen.getByText(/Asset mapping is approved/)).toBeTruthy());
  ...
  it("requires explicit text-only confirmation only after an empty review has started", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Confirm text-only review" }));
    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(5));
  ...
  it("requires a separate confirmation before rerunning local automatic matching", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    render(<LongEpisodeMappingReviewScreen projectId="long" episodeNumber={1} onBack={() => {}} />);
    expect(await screen.findByRole("button", { name: "Re-run automatic matching" })).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Re-run automatic matching" }));
    expect(await screen.findByTestId("episode-asset-matching-rerun-confirm")).toBeTruthy();
    expect(fetchMock).toHaveBeenCalledTimes(2);
    fireEvent.click(screen.getByRole("button", { name: "Re-run matching" }));
