  ...
  it("opens the video workflow screen from a successful local video submission and shows live local fake progress", async () => {
    ...
    const fetchMock = vi.fn<FakeFetch>(async (input, init) => {
      ...
      if (url === "/projects/sample_project/videos/generations/job_42") {
        return jsonResponse(200, {
          // Stated, not left out: the client refuses a progress response without it, because a missing
          // paidProvider used to be read as "free" on a run that was charging.
          paidProvider: false,
          jobId: "job_42",
          status: "succeeded",
  ...
  it("reaches the final merge screen only after all six video reviews are approved, and completes it via POST .../videos/merge", async () => {
    ...
    const fetchMock = vi.fn<FakeFetch>(async (input, init) => {
      ...
      if (url === "/projects/sample_project/videos/generations/job_42" && method === "GET") {
        return jsonResponse(200, { paidProvider: false, jobId: "job_42", status: "succeeded", completedSceneNumbers: [1, 2, 3, 4, 5, 6], failedSceneNumbers: [], sceneNumbers: [1, 2, 3, 4, 5, 6] });
      }
    ...
    const fetchMock = vi.fn<FakeFetch>(async (input, init) => {
      ...
      if (url === "/long-projects" && method === "POST") {
        ...
        const project: LongProject = {
          ...
          // The style fields are optional on the way in and always present on the way back, the way the server answers.
          settings: { visualStyle: "", color: "", lighting: "", avoid: "", ...request.settings, episodeDurationSeconds: request.settings.sceneCount * request.settings.clipDurationSeconds },
