...
describe("화면 색은 한 군데에서만 정해진다", () => {
   ...
   * 자기 파일에 적어서 그렇게 됐습니다.
   */
  it("이름 있는 버튼은 한 군데에서만 정의된다 — 스물넷이 열한 가지로 갈라져 있었다", async () => {
    const root = path.dirname(url.fileURLToPath(import.meta.url));
    const files = await sourceFiles(root);
     ...
     */
    const SHARED = ["outlineButton", "smallOutlineButton", "dangerOutlineButton", "smallDangerOutlineButton"] as const;

    /*
     * 🔴 **이 표가 못 보는 것**: 이름 없이 `className="…"` 에 바로 적힌 버튼. 작은 장미 버튼 일곱 벌 중 둘이
     * 그랬고(Round 1000), 그래서 이름으로 찾는 이 짝에 **안 걸려서 손으로 세야 했습니다.** 🟠 인라인까지
     * 잡으려면 `border-rose-400` 같은 색 문자열을 세야 하는데, 그러면 **오류 문구(`text-rose-400`)까지**
     * 걸립니다. 걸러내려면 짝이 CSS 를 읽기 시작해야 하고, 그건 짝이 할 일이 아닙니다.
     *
     * 🟢 그래서 이 짝의 약속은 **「이름이 있는 것은 하나뿐이다」**까지입니다. 인라인을 막는 건 규칙이 아니라
     * 습관이고, 규칙이 못 하는 자리를 규칙인 척하지 않습니다.
     */
