...
- [ ] **🟠 보고만 함 — 패키징도 아무것도 안 만들고 산출물을 그대로 싣는다**: `electron-builder` 설정이 `../backend/dist-bundle` 과 `../frontend/dist` 를 **그대로 복사**하는데, `package` · `package:installer` 어느 쪽도 그것들을 빌드하지 않는다. 그러면 **설치 프로그램이 그 순간 트리에 있던 낡은 코드를 그대로 싣는다** — 방금 6일 묵은 번들에서 확인한 그 실패가 사용자 손에 가는 형태다.
  - **고치지 않았다.** 패키징·배포는 사용자가 순서를 정하기로 한 영역이라 먼저 손대지 않는다. 필요해지면 `package` 앞에 같은 체인을 붙이는 한 줄이면 된다.
...
- [ ] **🟠 제안만 — 병합 중간 산출물이 영구히 남는다**: `videos/final/normalized/` 에 정규화된 장면 클립과 `concat.txt` 가 남는데, **읽는 코드가 없다** — 병합할 때마다 새로 만들어 덮어쓴다. 순수 스크래치다.
  ```
