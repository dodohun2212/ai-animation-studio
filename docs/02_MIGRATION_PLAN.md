...
- [ ] 남은 범위(4단계, 계획 그대로 유지): `ImageGenerationScreen.tsx`/`VideoWorkflowScreen.tsx`/`MappingReviewScreen.tsx`(그리고 필요 시 `VideoPromptPreviewScreen.tsx`)를 아직 손대지 않은 고정 `SCENE_NUMBERS`/`assertExactlySixScenes`에서 전환. 이 단계까지 끝나야 사용자가 화면에서 실제로 6이 아닌 장면 수 프로젝트를 끝까지(대본→이미지→영상) 만들어 볼 수 있다. Long Episode 워크플로우는 이번 이전 작업 전체에서 계속 범위 밖이다.
