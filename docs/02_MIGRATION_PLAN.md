...
- [ ] **🔴 `SceneEditScreen` — 화면이 뜨자마자 고치면 그 입력이 조용히 사라진다 (Cowork 몫)**: 불안정 테스트(`sends only the fields that changed`, 전체 실행 3~4회에 1회 빨강)를 쫓다 나온 실제 결함.
...
- [x] **대본 생성 이중 과금 — 락으로 못 막는 나머지를 멱등키로 닫았다 (`04a5630`)**: 락은 **겹친** 두 번을 막는다. 첫 번째가 끝난 뒤 오는 두 번째는 못 막고, 대본은 **재생성이 정당한 반복**이라 상태 게이트도 안 막는다. 30초 동안 화면에 아무 표시가 없어서 다시 누른 사람이 두 번 낸다 — 개요가 당한 것과 같은 이야기.
  ...
  - 🟠 **`LongEpisodeVideoWorkflowScreen` 은 아직 클릭마다 `crypto.randomUUID()` 를 만든다** — 그쪽 멱등키는 지금 아무 일도 안 하고 있다. Cowork에 넘김.
