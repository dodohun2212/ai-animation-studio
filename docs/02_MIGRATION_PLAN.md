...
- [ ] 남은 범위(3단계): `ShortProjectSettingsScreen.tsx`의 "영상 길이" 입력을 "장면 수" 입력으로 교체, 공급자별 지원 클립 길이 선택 UI(`ProviderSettingsScreen.tsx`) 추가 및 Backend에 실제 연결. (4단계): `ImageGenerationScreen.tsx`/`VideoWorkflowScreen.tsx`/`MappingReviewScreen.tsx`(그리고 필요 시 `VideoPromptPreviewScreen.tsx`)를 아직 손대지 않은 고정 `SCENE_NUMBERS`/`assertExactlySixScenes`에서 이전. 3~4단계 전까지는 프론트엔드에 장면 수 입력 UI가 없어 사용자가 실제로 6이 아닌 장면 수 프로젝트를 만들 방법이 없다 — 이번 배치도 위험 없이 진행한 순수 Backend 기반 작업이다.
