...
- [ ] **🟠 장편 회차에는 자막 손잡이가 없다 — 결정이 필요하다 (2026-09-09 발견, 안 고쳤다)**: Cowork 이 화면 쪽에서 찾아낸 「단기만 고치고 장편을 안 봤다」가 **렌더 쪽에도** 있었다. `episode-video-merge.service.ts` 의 `mergeScenes()` 가 `MergeSceneInput` 을 만들 때 `sceneSubtitleLayout` 을 안 넘긴다 — 단기 쪽(`video-merge.service.ts`)은 넘긴다.
