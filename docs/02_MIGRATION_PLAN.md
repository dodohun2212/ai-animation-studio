...
- [ ] **🟠 28 — 확인창이 약속하는 "다시 꺼낼 수 있습니다" 에 갈 데가 없다 (캡틴D 결정 대기)**: 문구는 이미 맞게 적혀 있고 **일부러 안 고쳤다** — 무르게 하면 *"되돌릴 수 있으니 확인 문구로 충분하다"* 는 판단의 근거가 같이 무너지고, 화면을 만들면 **캡틴D가 범위에서 뺀 27을 몰래 하는 것**이 된다.
  ...
  - 이건 보통 "나중에" 로 밀리다가 **누군가 회차를 보관하고 나서야 없는 걸 아는** 종류다 — 그때는 이미 화면에서 사라진 뒤다. 다만 **뺀 항목이라 시작하지 않는다.**
...
- [ ] **🔴 회차 이미지가 자산 폴더로 등록되지 않는다 (견적 하루 · 착수 대기 — 캡틴D 지시 후)**: 캡틴D — *"프로젝트 파일처럼 장기 프로젝트도 에피소드마다 이미지 확인할 수 있도록 폴더 만들어 달라"*. 단편은 `local-image-generation.service.ts:170` 이 `indexGeneratedProjectImages()` 를 부르고, **회차는 그 호출이 없다** — `collectReferenceImages` 로 **읽으러만 가고 등록하러는 안 간다.** 폴더가 없으니 전체 특징·대표 이미지·다음 회차 참고가 전부 막힌다.
  - 🟢 **회차 번호 자리 걱정은 기우였다**: `source_project_id` 는 자유 문자열이고 파서는 `typeof === "string"` 만 본다(`asset-storage.ts:55`). 화면도 파싱하지 않고 **수동 자산 표식과 같은지만 비교한다**(`AssetLibraryScreen.tsx:31–32`) — 어디에서도 프로젝트 id 로 되돌려 쓰지 않는다. `"12/Episode01"`(= `EpisodeMappingOwner.id` 가 이미 쓰는 형태)이면 되고 **스키마 변경도 마이그레이션도 없다.**
  - **진짜 일은 경로와 형제 함수다**: `assets.repository.ts:617` 이 `<projectsRoot>/<id>/images/sceneN.png` 를 **스스로 조립한다**(회차는 `long_story/EpisodeNN/images/`). 그리고 `indexGeneratedProjectImages` · `approveGeneratedProjectImage` · `replaceGeneratedProjectSceneImage` **셋이 같은 `(source_project_id, source_scene_number)` 키로 자산을 찾는다** — 하나만 고치면 등록은 되고 승인·교체가 못 찾는다.
- [ ] **🔴 프로젝트를 보관하면 자산 폴더가 남고 그림만 죽는다 (견적 B 반나절 / A 하루 · 착수 대기 — 캡틴D 지시 후)**: `project-archive.ts` 에 **자산이라는 단어가 한 번도 안 나온다.** 순수 디렉터리 이동이고 `stored_path` 는 절대 경로라 이동 뒤 전부 죽는다. **없어지는 것보다 나쁘다** — 없으면 *"아, 보관했지"* 인데 있으면 왜 안 뜨는지 찾아 헤맨다.
  - **짝이 셋이다**: `archive` · `restore` · **`deleteArchived`**(빠지면 자산 기록이 영영 남는다).
  - **A. 경로를 따라 고친다(하루)** — 정확하지만 **쓰기가 디렉터리·자산 인덱스 두 군데로 늘고**, 중간에 죽으면 둘이 어긋난다. **B. 보관된 프로젝트의 자산을 목록에서 숨긴다(반나절)** — 경로를 안 건드리니 **되돌릴 게 없어 어긋날 것도 없다.** 사람이 겪는 문제가 *"폴더는 멀쩡한데 그림이 죽었다"* 라 **B 를 권한다.** A 는 *보관된 프로젝트의 그림을 보관함 밖에서 계속 쓰고 싶은가* 라는 캡틴D 질문에 "예" 일 때만 필요하다.
  - **② 를 ① 보다 먼저 권한다**: ①은 **없는 기능**이고 ②는 **있는 것처럼 보이는데 죽어 있는 것**이다. 그리고 ②는 **이미 그렇게 된 프로젝트가 지금 디스크에 있을 수 있다.**
