...
- [ ] **🔴 장면 자막이 명언 카드가 이미 고친 그 자리에 그대로 있었다 — 계약·백엔드 절반 (2026-09-08, Cowork Round 664 가 넘긴 것)**: 캡틴D 신고 「자막이 잘 안 보이고 너무 내려가 있다」. `PHOTO_CARD_SUBTITLE_CENTER` 주석이 **왜 아래가 안 되는지를 이미 적어 두고 있었다** — Reels 가 캡션·계정명·버튼을 그 자리에 그린다. 카드는 그래서 0.40 으로 올라갔고, **장면 자막은 그 수정을 못 받았다.** `MarginV = 0.042×height` 로 아래 정렬이라 덩어리 전체가 화면 아래 13% 안에 있었고, 외곽선도 카드가 3→4 로 올린 뒤에도 **3 인 채**였다(꽃 릴스 장면은 전부 사진이다).
