...
### 2026-09-11 — 첫 프레임이 이미 그 장면의 끝이었다 (캡틴D 「성장이 끊긴다」 · Cowork Round 714)

- 🔴 **앱이 한 파일을 두고 자기 자신과 모순이었다.** `promptFor` 가 Runway 에 보내는 첫 줄은 *"from the supplied exact first frame"* 이고 바로 다음 줄이 `Starts at: {start_motion}` 인데, 그 프레임을 그리는 `image-prompt.ts` 는 `Scene:` 에 **`visual_action`**(장면 전체의 핵심 행동)을 넣고 있었다. **「이게 시작이다」라고 말하면서 끝 그림을 건넸고**, 클립은 5초 동안 할 일이 없었다. `Scene:` 을 `start_motion` 으로 바꿨다.
- 🟢 **잰 것**: Cowork 이 `꽃말_해바라기` 의 장면 이미지 4장과 각 클립 마지막 프레임을 **사람이 보는 순서 그대로** 늘어놓고 봤다 — 이미지2 에 이미 떡잎 두 장이 올라와 있고 영상2 끝이 이미지2 와 거의 같다. **성장이 컷 안이 아니라 컷 사이에서** 일어나고 있었다. 프리셋(`CreateFlowerReelForm.tsx`)은 *「한 장면 안에서 식물이 눈에 띄게 자라야 하고, 첫 프레임과 마지막 프레임이 분명히 달라야 한다」* 고 **이미 정확히 요구하고 있었다** — 대본이 틀린 게 아니라 **빌더가 그 요구를 지킬 수 없게** 만들고 있었다.
- 🔴 **주석이 자기 근거를 한 칸 넓게 잡고 있었고 그게 이 줄이 버틴 이유다.** 옛 주석은 *「템플릿이 `visual_action` 과 구도 필드들을 이미지 생성에 배정한다(「이미지 한 장의 구도」)」* 고 적었는데, 템플릿에서 **「이미지 한 장의」라고 말하는 것은 `shot_size`·`camera_angle`·`composition`·`lens_feel`·`focus_subject` 다섯뿐**이다. `visual_action` 에는 그런 문장이 없다.
- 🟠 **더하지 않고 바꿨다.** 둘 다 넣으면 「동작은 X인데 X 직전을 그려라」가 되고, 그건 이 저장소가 이미 값을 치른 모양이다(사람을 금지하면서 대표 캐릭터를 요구한 꽃 프롬프트). 피사체는 `focus_subject`·`composition` 이 싣고, 디스크의 20개 장면 전부에서 `start_motion` 이 그 자체로 완결된 프레임 서술임을 확인했다.
- 🔴 **빌더를 바꾸면 기존 그림 20장이 애먼 소리를 듣는다 — 그쪽을 같이 막았다.** 디스크에 기록된 이미지 프롬프트 20개(5개 프로젝트) **전부** 첫 줄이 `Scene: {visual_action}` 이다. 그대로 두면 `imagePromptDrift` 가 20개를 `"scene"` 으로 판정하고 화면은 **「장면 내용이 바뀐 뒤로 다시 만들지 않았습니다」** 라고 말한다 — **20번 다 거짓**이다(장면은 안 바뀌었고 빌더가 바뀌었다). 날짜가 박힌 좁은 호환 규칙을 넣었다: 나머지 줄이 같은 채 `Scene:` 줄만 그 장면의 `visual_action` 이면 2026-09-11 이전 기록이고, **장면은 안 바뀐 것**이라 `"current"` 다. `styleStale` 주석과 `NO_LEGIBLE_TEXT_VIDEO_RULE` 이 적어 둔 같은 함정의 세 번째다.
- 🟠 **그 규칙이 못 보는 것도 짝으로 고정했다**: 옛 기록에는 `start_motion` 이 아예 안 들어 있으므로, **옛 기록 + `start_motion` 만 수정**한 경우는 안 잡힌다. 반대쪽(20장을 전부 「장면이 바뀌었다」로 보내 다시 그리게 하는 것)이 **돈이 나가는 방향으로 더 나쁘다.** 그 장면을 한 번 재생성하면 영구히 닫힌다.
- 🔴 **짝이 진짜 결함 하나를 잡았다 — 유료 호출 직전이었다.** 두 이미지 생성 검증기(`local-image-generation.service.ts`·`episode-images.service.ts`)가 `visual_action` 만 비었는지 봤다. 그리는 필드가 바뀌었으므로 **`start_motion` 이 빈 장면이 검증을 통과해 「장면 설명이 아예 없는」 프롬프트로 유료 이미지 모델에 나갈 수 있었다.** 검증기를 그리는 필드로 옮겼다. 디스크 확인: `start_motion` 이 없는 장면은 명언 카드 14개뿐이고 그것들은 `visual_action` 도 없어 애초에 이 경로를 안 탄다.
- 🟢 **템플릿 규칙 둘은 `motion-field-rules.ts` 에 넣었다** — 단기 템플릿에만 넣으면 장편 회차가 그걸 안 읽는다(이 파일이 만들어진 이유가 정확히 그 사고다). `FIRST_FRAME_IS_START_MOTION_RULE`(start_motion 은 그대로 첫 프레임으로 그려지니 한 장의 그림으로 완결되게 쓰고 끝난 뒤 상태를 쓰지 말 것 — 지금까지 두 프롬프트 어디도 이 사실을 안 말해서 「자세 한 조각」으로 써도 규칙 위반이 아니었다), `ONE_PACE_ACROSS_A_PROCESS_RULE`(이어지는 장면들이 하나의 연속된 과정이면 motion_speed 를 서로 같게). 🟠 후자는 **조건부로 썼다** — 전 장면 동일은 틀리다: 프로젝트 `1` 은 절정인 5번만 `보통`이고 그게 옳다. 해바라기는 같은 화분이 자라는 한 과정인데 `느림·느림·보통·느림` 이었다.
- 🟢 새 짝이 **그 규칙이 참말인지도 검사한다** — 「start_motion 이 그려진다」는 다른 파일에 대한 주장이라, `imagePromptFor` 가 실제로 그것을 그리는지 대조한다. 빌더가 다른 필드로 돌아가면 그 문장은 **대본 모델에게 하는 거짓말**이 되고, 프롬프트 안의 거짓말은 유료 이미지 한 장과 그 위에 세운 유료 클립으로 청구된다.
- 🔴 **원인 2(다음 이미지를 앞 클립의 마지막 프레임에서 편집)는 안 했다 — 근거가 없어서다.** Runway 공식 문서(`docs.dev.runwayml.com`)에서 `promptImage` 의 배열 형태도 `position: "last"` 도 **확인되지 않는다**(공식 예시는 URL 하나 또는 data URI 하나뿐). 배열을 말하는 것은 전부 재판매 문서이고 그중 하나는 오히려 Gen-4.5 가 `first` 만 제어한다고 적는다. **지원한다고 꾸며내지 않는다**(Round 709 경계). 대안 A(ffmpeg 로 앞 클립 마지막 프레임 추출)는 **참조 원본 교체가 아니라 파이프라인 순서를 뒤집는 변경**이라 결정 사항으로 올렸다: 이미지 전부 → 승인 → 영상 전부 가 **이미지1 → 영상1 → 이미지2 → …** 로 교차하고, 배치 승인·선지급 견적(`confirmationId`·`maximumProviderCalls`)이 성립하지 않으며, 클립 하나를 다시 만들면 **N..끝까지 연쇄로** 재생성·staleness 가 번진다.
- 검증: backend 1618 통과(+1 의도적 skip) · frontend 1423 · shared 57 · desktop 38, typecheck 셋과 전체 build 통과(전부 `set -o pipefail` 로 실제 종료 코드 확인). 실제 OpenAI·Runway·Instagram 요청은 0회다.
...
- [x] **🟠 실사용에서 나온 것 — `BAD_OUTPUT` 이 "알 수 없는 오류" 로 뭉개졌다 (계약 `6bb5ec6` · 화면 `31c5281` 로 닫힘)**: 2026-09-05 캡틴D의 5화 3번 장면이 **같은 오류로 두 번** 실패했다(`INTERNAL.BAD_OUTPUT.CODE01`, $0.50 나가고 결과물 0).
...
- [x] **🔴 오늘 밤 Cowork 쪽 작업 1,368줄이 커밋되지 않은 채 작업 트리에만 있었다 → `31c5281` 로 들어갔다 (2026-09-06)**
  ```
...
- [x] **🔴 게시 도중 프로세스가 죽으면 같은 영상이 두 번 올라갈 수 있었다 (2026-09-08, CLI Round 628 ③ · Cowork Round 640 ⑤)**: 기록이 두 번째 누름을 막고 교차 프로세스 락이 두 창을 막는데, **둘 다 프로세스가 무엇을 했는지 적을 때까지 살아 있다는 것을 전제**한다. `nest start --watch` 는 백엔드 파일을 저장할 때마다 재시작하고(D-005 가 락을 파일로 둔 바로 그 사실), `media_publish` 와 `projects.save` 사이에서 죽으면 **게시물은 공개돼 있고 디스크는 모른다** — 락은 60초 뒤 stale, 다음 누름이 통과.
  - 🟢 **흔적 파일을 락 옆에 둔다**(`publish-attempt.ts`). `media_publish` **직전**에 쓰고 **기록이 안전하게 저장된 뒤에만** 지운다. 🔴 **그 순서가 이 변경의 전부다** — 먼저 지우면 게시가 아예 없었던 것과 똑같이 읽힌다.
  - 🟠 **제안(「Meta 호출 직전」)보다 좁혔다**: 컨테이너는 게시물이 아니고 업로드된 영상도 게시물이 아니다. 시퀀스 전체를 덮으면 **평범한 거절마다** 「계정을 확인하고 오라」를 요구하게 되고, 그러면 **그 경고가 읽히지 않게 된다** — 지나가는 방법이 사람이 확인했다고 주장하는 것뿐이라 더 그렇다.
  - 🟢 **가르는 근거가 이미 코드에 있었다**: `diagnostics.status` 는 진짜 HTTP 응답에서만 채워진다(`classifyErrorResponse`). 400 거절은 흔적을 지우고, 끊긴 연결과 media id 없는 200 은 남긴다. **새 개념을 만들지 않았다.**
  - 🟢 **빠져나갈 문은 다이얼로그**(Cowork). 체크박스는 누르기 전에 켜고 잊을 수 있어 **경고가 영구히 미리 답해진 상태**가 된다. 필드는 모든 층에서 `?: true` 또는 없음 — 「거절한 사람」과 「기본값으로 안 켠 폼」이 전선 위에서 구분이 안 되기 때문.
  - 🔴 **주입 넷 중 하나가 안 잡혔고, 그게 핵심 한 줄이었다**: 「기록보다 먼저 흔적을 지움」이 0개 빨강. `projects.save` 가 던지게 해서(프로세스 사망 대역) 흔적이 살아남고 다음 누름이 거절되는지까지 보는 짝을 넣었다. 지금 넷 다 빨개진다. 백엔드 1550 → 1569 · 프론트 1387 → 1393.
  - 🟠 **계약 한 줄은 화면과 같은 커밋으로 냈다** — 화면이 안 읽는 계약 필드는 프론트의 계약 짝 둘이 거부한다. 638 ③ 이 산문으로 쓴 것을 그 짝이 기계로 잡는다.
- [x] **🔴 게시된 컨테이너를 「아무것도 게시되지 않았다」고 보고했다 (2026-09-08)**: 컨테이너 상태가 `PUBLISHED` 면 계정에 뭔가 올라가 있는데 `INSTAGRAM_PUBLISH_FAILED` 를 던졌고, 화면은 그 코드를 **「아무것도 게시되지 않았으니 다시 시도해도 됩니다」**로 그린다. `instagramPublishApi.ts` 주석이 이미 그 혼동을 금지하고 있었다.
  - 🔴 **백엔드의 영어 문장은 맞게 쓰여 있었고 아무에게도 안 갔다** — 화면은 코드로 갈라서 자기 문구를 쓴다. **틀린 코드 뒤의 맞는 문장은 절반만 맞은 게 아니라 그냥 틀린 것이다.**
- [x] **🟠 화면이 이미 써 둔 문장에 도달할 코드가 없었다 (2026-09-08)**: `longProjectsApi.ts` 가 `LONG_EPISODE_CONTINUITY_INVALID` 문구를 갖고 있는데 **그 코드를 던지는 곳이 앱 전체에 없었다.** 잘못된 연결 기억 저장은 `INVALID_REQUEST` 로 와서 catch-all 「입력 내용을 확인해 주세요」로 그려졌다 — **어디를 보라고 말하는 문장이 도달 불가**였다.
  - 🟢 **찾은 방법**: 화면이 그리는 코드 전부와 백엔드가 던질 수 있는 코드 전부를 뽑아 차집합. 후보 아홉 중 여덟은 sweep 이 생성 형태를 못 본 것이었고, **하나가 진짜였다.**
  - 🟠 **요청 경로만 개명한다.** 디스크의 continuity.json 이 안 읽히는 것은 **누가 낸 요청이 아니고**, 그걸 잘못된 요청이라 부르면 사람을 서식 쪽으로 보낸다.
- [x] **🔴 화면이 못 하는 말이 아홉 개 있었고, 하나는 정반대를 말하고 있었다 (2026-09-08, CLI Round 646 · Cowork Round 647)**: 모든 거절은 두 워크스페이스에 걸친 두 반쪽이다 — 백엔드가 코드를 고르고, `apps/frontend/src/api` 의 표가 그것을 **사람이 보는 유일한 문장**으로 바꾼다(백엔드의 영어 메시지는 화면에 절대 안 간다). 선언된 코드 중 아홉이 그 표에 없어서 전부 catch-all 로 떨어졌다.
  - 🔴 **여덟은 「덜 정확했다」이고, 아홉째는 반대를 말했다**: `INSTAGRAM_PUBLISH_IN_PROGRESS` 를 받는 사람은 **첫 게시가 실패했다고 이미 믿고 두 번째로 누른 사람** 하나뿐이다. 화면은 「요청을 처리하지 못했습니다」를 띄웠고, 그건 두 번째 실패로 읽히며, 다음 행동은 **세 번째 누름**이다 — 그 사이 첫 게시는 올라가고 있을 수 있다. 기록은 인스타그램이 수락한 **뒤에** 쓰이므로 그 몇 분간 앱은 정말로 모른다. 문구를 「기다리세요」가 아니라 **「다시 누르지 마세요」**로 쓰고, 짝이 **「실패」라는 낱말의 부재**까지 단언한다 — 다시 쓸 때 제일 쉽게 돌아올 것이 그 함의라서.
  - 🟢 **게시 경로에 이제 네 문장이 서로 다른 것을 말한다**: 진행 중(모른다·기다려라) · 결과 불명(모른다·가서 봐라) · 이미 게시됨(안다·올라갔다) · 실패(안다·아무것도 안 올라갔다). **`3f05957` 이 마지막 둘을 섞은 것**이었고, 이 표가 먼저 있었으면 그 결함은 안 났다.
  - 🟠 **둘은 접두사 어긋남**이었다: 화면이 `VIDEO_RESTORE_NOT_ALLOWED` 를 답하는데 백엔드는 `VIDEO_LIBRARY_*` 를 던진다. **코드가 아니라 키를 옮겼다** — 접두사를 떼면 `LONG_EPISODE_VIDEO_*` 와 부딪히고 **한 가지에 이름이 둘**이 된다. 옛 이름이 죽었다는 것도 짝으로 박았다(「예전 이름도 남겨 두자」가 이름 둘의 시작이라서).
  - 🔴 **Cowork 이 적어 준 진짜 교훈**: 옛 짝의 stub 이 `{ code: "VIDEO_RESTORE_NOT_ALLOWED" }` 였다 — 그 줄은 **「서버가 이 코드를 보낸다」는 단언**이고, 그 말이 틀렸다. 표와 짝이 **같은 틀린 이름을 공유해서 서로를 지켜 주고 있었다.** 안에서 서로 맞는 둘은 안에서는 영원히 안 잡히고, 이 sweep 이 잡은 이유는 짝을 표가 아니라 **백엔드와** 대조했기 때문이다. → **표의 키를 바꿀 때는 그 키를 stub 하는 짝도 함께 센다.**
  - 🟢 **`b0acf6d` 의 짝이 이것을 유한하게 만들었다**: 코드가 이름을 얻자마자 「예외가 이유보다 오래 살아남지 못한다」가 빨개져 아홉을 전부 뱉게 했다. 예외 목록 **열하나 → 셋**, 셋은 전부 결정(디스크 오류 둘 + 로컬 fake 전용 하나). **「어딘가 더 있을 것 같다」가 「셋이고 셋 다 이유가 있다」로 바뀐 것**이 이 항목의 산출물이다.
- [x] **🟠 문자열 두 개가 서로 다른 말을 하는 자리를 네 곳에서 찾고, 두 곳에 짝을 세웠다 (2026-09-08)**: 계약 필드(프론트의 기존 짝 둘) · **오류 코드**(`b0acf6d`) · **route 경로**(어긋남 0, 짝 안 만듦) · **vite 프록시 접두사**(`97b5cfe`). 남은 두 곳(`WorkflowState`, `data-testid`)은 **재 보고 접었다** — 앞의 것은 양쪽이 타입으로 쓰므로 컴파일러가 이미 보고, 뒤의 것은 차집합이 결함을 안 가리킨다.
  - 🔴 **route 쪽에서 제 자가 두 번 틀렸다**: 1차 124/148 「없음」(파라미터 이름을 한쪽만 정규화), 2차 2/148(호출된 route 헬퍼 `@Get(API_ROUTES.subtitleFont(":name"))` 를 파서가 못 봄). **앱이 돌아가는데 124개가 깨졌다는 결과가 나오면 결과가 아니라 자를 의심한다.** 두 번 틀린 파서를 짝으로 굳히면 그 짝은 늑대를 부르거나 잠들므로, **고칠 게 없는 자리에 못 믿을 감시를 놓지 않았다.**
  - 🔴 **`vite.config.ts` 는 exhaustive 하다고 산문으로 말하고 있었고, 그래서 한 번 거짓이 됐다** — `/audio`·`/videos` 가 빠져 두 화면이 브라우저에서 죽어 있었다(같은 주석에 그 사고가 적혀 있다). 프록시에 없는 경로를 **Vite 가 index.html 로 답하므로** fetch 는 200 으로 성공하고 JSON 파싱에서 죽는다 — 사람이 보는 것은 빈 화면이지 「백엔드에 못 닿았다」가 아니고, **패키징된 앱은 한 origin 이라 멀쩡해서 두 모드가 조용히 갈린다.**
  - 🟠 접두사를 소스 스캔이 아니라 **런타임 값**에서 뽑는다(헬퍼를 placeholder 로 호출). 인자를 받는 헬퍼가 바로 그날 손 스캔이 놓친 모양이라, 보간을 다시 구현하는 것은 같은 실수를 다시 만드는 것이다. 🟠 **한계도 재서 적었다**: 여기서 읽는 `API_ROUTES` 는 빌드된 패키지라, shared 를 다시 빌드하지 않으면 새 접두사를 못 본다(모든 검증 통과는 shared 를 먼저 빌드한다).
- [x] **🟢 그 깜빡임이 다시 나왔고, 적어 둔 대로 계측을 붙였다 (2026-09-09)**: `videos/project-lock.test.ts` 의 *"throws ProjectLockTimeoutError, never waiting forever"* 가 **2026-09-07 에 이어 두 번째로** 전체 실행에서 실패했다. 이번에도 **단독 7/7 · 전체 재실행 초록** — 같은 profile 이다.
  - 🔴 **문제는 깜빡임 자체가 아니라 실패가 아무 말도 안 한다는 것이었다.** `rejects.toThrow` 는 **서로 다른 결함 셋**을 한 문장으로 보고한다: ① 두 번째 도착이 잡힌 락을 **이겼다**(가드가 깨진 것), ② **다른 오류로** 졌다(파일시스템 오류가 acquire 루프를 빠져나온 것 — Windows 에서 `wx` 생성이 부하 중에 `EEXIST` 대신 `EPERM` 을 낼 수 있고, 이 코드는 EEXIST 가 아닌 것을 전부 rethrow 한다), ③ 그냥 **오래 걸렸다**(이벤트 루프가 굶은 것. 아무것도 안 틀렸다). **고치는 방법이 셋 다 다르다.**
  - 🟢 **결과를 이름 붙여 잡는다** — 어느 쪽이었는지, 몇 ms 기다렸는지, 포기하는 순간 **홀더의 락 파일이 아직 디스크에 있었는지**까지. 🔴 **재시도도 관용도 없다**: 단언 둘은 대체한 하나와 정확히 같은 만큼 엄격하다. 바뀐 것은 **다음 발생이 답을 달고 온다**는 것뿐이다 — 요청 시 재현되지 않는 깜빡임을 재현하러 다시 보내지 않는다.
  - 🟢 **확인했다**: 홀더를 일찍 놓아 「락을 이긴」 경우를 만들면 실패가 이렇게 읽힌다 — *"the second arrival WON a held lock after 1ms (holder's file still on disk: false)"*. 백엔드 1588. `c1ddb7e`.
  - 🟠 **2026-09-08: 같은 profile 의 실패를 한 번 더 봤는데, 어느 짝인지 못 남겼다.** 백엔드 전체 실행이 `1 failed | 1567 passed` 로 끝났고, 그 실행은 **같은 순간에 돌던 `git add`·`git commit` 와 디스크를 다투고 있었다.** 직후 전체 재실행 **네 번 연속 초록**(1568), `project-lock` 과 `assets` 를 **동시 부하를 걸어 세 번** 돌려도 초록이라 재현하지 못했다.
  - 🔴 **그래서 이 항목은 아직 못 닫는다 — 그리고 못 닫는 이유가 계측이 아니라 내 실행 방식이었다.** 실패한 짝의 **이름을 안 잡고** 다음 명령으로 넘어갔고, 이름이 없으면 「같은 것이 또 나왔다」는 말도 못 한다. 부하 의존 깜빡임에서 확인해야 할 단 하나가 **매번 같은 짝인가**인데, 그것을 확인할 수 없게 만들었다.
  - 🟢 **규칙**: 전체 실행은 **실패 이름을 남기는 형태로만** 돌린다(요약 줄만 grep 하지 않는다), 그리고 **다른 명령과 한 줄에 묶지 않는다** — 커밋과 경쟁시키면 관측하려던 조건을 관측 도중에 만들어 낸다.
  - 🟠 **2026-09-08 재현 시도, 실패**: 규칙대로(실패 이름이 남는 형태로, 다른 명령과 안 묶고) 백엔드 전체를 **다섯 번** 더 돌렸다 — 셋은 그냥, 둘은 **디스크 부하를 일부러 걸고**(별도 프로세스가 30KB 파일을 쉬지 않고 쓰고 읽고 지우는 동안). **다섯 다 1577 초록.** `project-lock` 과 `assets` 만 부하 아래 세 번 돌린 것도 초록이었다. 같은 세션의 다른 전체 실행까지 합치면 열 번이 넘는다.
  - 🔴 **그러니 이 항목은 「부하를 걸면 난다」가 아니다.** 그렇게 적고 싶었지만 재 보니 아니다 — 부하는 **한 번 관측된 순간에 함께 있었던 것**일 뿐, 재현 조건으로 확인되지 않았다. 다음 사람이 부하만 걸어 보고 "안 나네"로 닫지 않도록 적어 둔다. **아직 조건을 모른다.**
  - 🟠 **2026-09-08 저녁: 세 번째 관측 — 그리고 짝이 다르다.** 백엔드 전체 실행 한 번에서 *"serializes two concurrent holders for the same key"* 가 빨갔고(단독 7/7 초록), CLI Round 665 ⑦ 은 그것을 「660 그 짝」이라고 적었다. **그 문장은 틀렸다** — 이 항목이 이름을 박아 둔 짝은 *"throws ProjectLockTimeoutError, never waiting forever"* 로, `project-lock.test.ts` 안의 **다른 짝**이다(그 파일은 짝이 일곱이다).
  - 🔴 **이 정정이 항목의 질문을 바꾼다.** 부하 의존 깜빡임에서 확인해야 할 단 하나는 **매번 같은 짝인가**였고(이 항목이 위에서 🔴 로 적어 둔 그것), 이름을 둘 다 잡은 지금 대답은 **아니다**. 같은 파일의 서로 다른 짝 둘이 **각각 한 번씩** 빨개졌다. 그래서 의심할 것은 한 단언의 경계값이 아니라 **이 파일이 공유하는 것**(락 파일·실시간 대기·stale 만료)이다.
  - 🟠 **그래도 계측은 여전히 안 붙였다.** 세 번 다 단독으로는 재현되지 않고, 조건을 모르는 상태에서 붙이는 계측은 **무엇을 찍을지를 추측으로 고르는 일**이다. 다음 관측자가 해야 할 것은 하나다 — **짝 이름을 남기고, 이 줄에 세 번째인지 네 번째인지를 더한다.**
  - 🟢 **2026-09-08 밤 재확인(강제 종료 뒤 상태 점검, CLI Round 666)**: 전체 1회 · 백엔드 단독 1회 모두 초록(1584 pass | 1 skip). 두 짝 어느 쪽도 안 났다. **조건을 좁히지 못하는 관측이라 이것만으로는 닫지 않는다.**
...
- [x] **🟠 열려 있던 항목 둘을 닫았다 — 둘 다 이미 끝난 일인데 문서만 아니라고 말하고 있었다**
  - **Cowork 미커밋 1,368줄** → `31c5281` 로 들어갔고 작업 트리는 0이다. 그 항목을 연 것도 나이고, 닫지 않고 두면 **다음 사람이 없는 위험을 보고 대응한다.**
...
- [x] **🟢 "주석이 주장하는 보호막이 실제로 그 경우를 덮는가" 로 인스타그램 쪽을 훑었다 — 하나는 구멍이었고 나머지는 참이었다**: 어젯밤 것들과 달리 이 렌즈는 **문장을 먼저 읽고 코드를 나중에 본다.** 결과를 남겨 둔다(Cowork 585 처럼, 다음 사람이 다시 안 보게).
  ```
  🔴 "client_secret 은 절대 로그로 안 간다 — ProviderSettingsLogger 의 리댁션 참고"
     → 그 리댁터에 질의 문자열 패턴이 없었다. 5f35534 로 고침
  🟢 "앱 시크릿은 이 프로세스를 떠나지 않는다"
     → 참이다. 로그인 다이얼로그 URL 은 client_id · redirect_uri · state · response_type 만 싣고,
       상태 응답은 불리언과 만료 시각뿐이다. 데스크톱 셸은 URL 을 열고 돌아온 URL 만 돌려준다
  🟢 "Meta 자신의 문구는 사용자에게 절대 안 닿는다"
     → 참이다. InstagramAdapterError 의 message 는 범주별 고정 한국어가 기본값이고,
       Meta 문구는 detail 로만 간다. 생성 자리 16곳 전부 고정 문자열이거나 undefined 다
  🟢 "게시 대상 목록과 게시 직전 검사는 절대 어긋나면 안 된다"(D-006)
     → 짝이 있다("The account list and this check must never disagree…")
  ```
  - 🟠 **이 렌즈가 어제 것들보다 나은 점**: 중복 탐지는 *같은 것이 여러 번 있는가* 를 묻고, 이건 *쓰여 있는 것이 사실인가* 를 묻는다. 후자는 **한 번도 사실이 아니었던 것**도 잡는다 — 오늘 잡은 게 그거다.
- [x] **🔴 실패해도 흔적이 하나도 안 남는 유일한 통제에 자기 짝이 없었다**: `no-test-network.guard.ts` — 테스트 프로세스가 진짜 제공자 API 에 닿는 것을 거절하는 그 한 줄들. **`D-016` 이 그것 때문에 생긴 결정이다**: Runway 작업 넷, 약 $1.00, **어느 project.json 에도 어느 원장에도 없는** 요금.
  - 🔴 **그 통제가 조용히 무력화되는 길이 셋인데 전부 한 줄짜리다**: 환경 검사가 일찍 return 하도록 바뀌거나, mock 판별이 평범한 함수를 받아 주거나, 둘의 순서가 바뀌거나. **아무것도 그걸 보고 있지 않았다.**
  - 🟢 짝 넷: 평범한 `fetch` 면 던지고 **메시지에 제공자 이름이 들어간다**(남의 실행에서 이게 터지면 그 문장이 진단 전부다) / `vi.fn()` 은 통과 / **테스트 밖에서는 비켜선다**(이 통제가 진짜 실행을 막으면 앱을 지키는 게 아니라 앱을 세운다) / **`VITEST` 가 없어도 `NODE_ENV=test` 면 거절한다**(VITEST 를 안 세우는 러너가 바로 요금이 몰래 나가는 경우다).
  - 🟢 주입 둘: `NODE_ENV` 절반을 빼면 네 번째가, mock 판별을 `typeof function` 으로 낮추면 첫 번째와 네 번째가 빨개진다.
  - 🟠 그리고 **money-guard 목록에 이름으로 박았다** — 짝과 짝의 이름이 함께 사라지면 아무것도 안 남는데, 이건 사라졌을 때 남는 게 **제공자 쪽 청구서뿐**인 통제다. 주입: 짝 이름을 바꾸면 목록이 그 문장을 대며 빨개진다.
  - 백엔드 1498.
- [x] **🔴 이미 낸 돈의 결과를 지킬지 버릴지 정하는 통제 셋에 짝이 하나도 없었다**: `budget-ledger.ts` 의 `recordSpend`(세 줄) · `isBudgetLedgerUnreadable` · `spendUnrecordedWarning`.
  - 🟠 **세 줄짜리라서 안 붙어 있었던 것**이고, 그게 정확히 위험한 이유다 — 세 줄은 정리하고 싶어지는데, **정리하는 두 가지 방법이 둘 다 기록된 사고다**(`D-037`): 여기서 던지면 **이미 산 결과를 같이 잃고**, OpenAI 쪽은 이게 `finally` 안이라 **제공자의 진짜 오류까지 원장 오류로 바꿔치기한다.**
  - 🟢 짝 다섯: 던지지 않고 **실패를 보고**한다 / 성공했을 때 `false` 다(그게 *"경고해야 하나"* 에 대한 답이라, 여기서 true 면 성공한 모든 장면에 경고가 붙는다) / **두 제공자의 원장 오류를 다 알아본다**(열한 개 catch 자리가 이 술어 하나로 사람이 읽을 문장을 고른다) / 경고가 **파일 이름과 대상**을 담는다 / 두 원장 파일 이름이 다르다.
  - 🟠 **문장 전체는 일부러 안 박았다.** 이 저장소는 *"사실이 바뀌는 동안 문구만 붙들고 있던 테스트"* 에 한 번 당했다 — 다시 써도 살아남아야 하는 건 **원장 파일 이름과 대상**이지 표현이 아니다.
  - 🟢 주입 둘: `try/catch` 를 없애면 첫째가, 술어에서 제공자 하나를 빼면 셋째가 빨개진다. money-guard 목록에도 이름으로 박았다. 백엔드 1503.
- [x] **🟠 데이터 뿌리가 옮겨진 뒤 저장된 경로를 다시 세우는 코드에 짝이 없었다**: 이 앱의 저장 경로는 전부 절대 경로인데(에셋 `stored_path` · `generated_images` · 영상 기록 `output_path`), **learning-data 뿌리는 한 번 움직인다** — 패키징 첫 실행에서(`D-038`). 그 뒤에도 경로가 바이트를 가리키게 하는 게 이 파일이다.
  - 🔴 **중요한 건 "다시 세운다" 가 아니라 "어느 앵커가 이기는가" 다.** 주석이 말하는 대로 **끝에서부터** 훑어 **안쪽 앵커가 이겨야** 한다 — 뿌리 자체가 `projects` 라는 이름의 디렉터리 **안에** 있으면, 앞에서부터 훑을 경우 바깥 것에서 다시 세워 **다른 파일을 가리킨다.** 앞에서부터 훑는 게 더 자연스럽게 읽히고, 그래서 누군가 그렇게 고쳐 쓴다.
  - 🟠 그리고 **뿌리 밖 경로는 손대지 않는다** — 그게 *"이 파일은 없어졌다"* 와 *"이 파일은 뿌리와 함께 옮겨졌다"* 를 구별해 준다. 밖의 경로에 위치를 지어내면 **확신에 찬 모양의 틀린 경로**로 엉뚱한 질문에 답하게 된다.
  - 🟢 짝 다섯(앵커부터 재배치 / **안쪽 앵커가 이김** / 뿌리 밖은 그대로 / 상대 경로는 후보 없음 / **반대 플랫폼 구분자로 기록된 경로도 읽음** — 이 값들은 평문 JSON 에 산다).
  - 🟢 주입 둘: 앞에서부터 훑게 하면 안쪽 앵커 짝이, 구분자 분해를 posix 만으로 줄이면 셋이 빨개진다. 백엔드 1508.
- [x] **🟠 업로드 파일 이름을 고치고 검사하는 유일한 자리에 자기 짝이 없었다**: `safeUploadFilename` — 사용자가 올린 이름이 디스크에 닿기 전에 지나는 곳이다.
  - 🔴 **서로 반대로 당기는 두 일을 한다.** 한국어 이름을 **받아야** 하고(멀티파트가 Latin-1 로 넘겨서 UTF-8 이어 바이트가 **C1 제어문자와 같은 자리**에 앉는다 — 제어문자만 거절하는 검사기는 **모든 한국어 파일 이름을 거절하면서 이유도 말 안 한다**, `CLI Round 429` 의 실측), 동시에 **엉뚱한 데 쓰일 이름을 거절해야** 한다.
  - 🟢 짝 다섯: 한국어 이름 복원 / **진짜 Latin-1 이름은 안 건드림** / 경로 구분자·`..`·`.` 거절 / 제어문자·빈 이름·255자 초과 거절 **+ 가운데 공백은 정상**(의심스러운 걸 다 막을 때까지 조이면 **사람들이 실제로 쓰는 이름**을 거절하기 시작한다) / **끝의 점·공백 거절**(윈도가 조용히 떼어 내서 **디스크의 이름이 색인의 이름과 안 맞게** 된다).
  - 🔴 **짝 하나를 고쳐 썼다 — 주입이 안 빨개져서다.** 처음엔 `"café.png"` 로 "복원이 과하게 안 걸린다" 를 붙들려 했는데, 그 바이트는 애초에 유효한 UTF-8 이 아니라 **어느 쪽 코드에서도 복원이 안 걸린다** — 즉 아무것도 안 붙들고 있었다. `"Ã©.png"` 는 바이트가 **유효한 UTF-8 이고 "é" 로 디코딩**돼서, 조건을 느슨하게 하면 **아무도 부탁 안 한 이름을 고쳐 쓴다.** 그걸로 바꾸자 주입이 빨개졌다.
  - 🟠 오늘 두 번째로 같은 교훈이다(리댁션 짝도 그랬다): **짝이 무엇을 붙들고 있는지는 주입해 봐야 안다.** 그럴듯한 픽스처는 그럴듯할 뿐이다.
  - 🟢 주입 둘: 복원 조건 완화 → 둘째가, 끝의 점·공백 검사 제거 → 다섯째가 빨개진다. 백엔드 1513.
- [x] **🟠 다섯 서비스가 함께 쓰는 회차 매퍼의 "관대한 읽기" 넷에 짝이 하나도 없었다**: 넷 다 같은 모양의 질문에 답한다 — **이 사실이 여기 있는가 없는가** — 그리고 넷 다 틀렸을 때의 결과가 주석에 적혀 있다.
  - 🔴 **`toEpisodeUsedAudio`** — 크레딧 줄은 이것 하나로만 만들어진다. 반쯤 쓰인 기록을 "있음" 으로 읽으면 **출처 표시 없는 오디오를 있다고 보고**하는 것이고, 그게 오늘 게시 게이트가 거절하는 바로 그 사실이다.
  - 🔴 **`thumb_offset_ms` 의 `null` 과 부재는 다르다** — `null` 은 *"이번 게시는 표지를 안 골랐다"*, 부재는 *"이 기록은 그 필드보다 오래됐다"*. **같은 릴이 나오지만 같은 기록이면 안 된다.**
  - 🔴 **`errors: []` 는 "확인했고 괜찮다" 로 읽힌다** — 아무도 쓴 적 없는 필드보다 강한 주장이다. 그래서 비어 있으면 아예 안 싣는다.
  - 🟠 **이전 게시 목록은 안 맞는 항목만 버리고 전체를 버리지 않는다** — 그 목록은 *"라이브 기록을 지워도 계정에 아직 뭔가 올라가 있을 수 있다"* 를 남기려고 있는데, 파싱을 거부하면 **바로 그것을 지운다.**
  - 🟠 이 부류는 정리하다가 **"똑같은 뜻" 이라며 다시 쓰기 쉬운데, 그 동치가 전부 미묘하게 거짓이다.**
  - 🟢 짝 일곱 · 주입 둘(부재를 `null` 로 만들면 표지 짝이, 모드 검증을 없애면 오디오 짝이 빨개진다). 백엔드 1520.
- [x] **🔴 열세 분 정전 때 화면 두 개만 참말을 했고 스물두 개가 응답을 탓했다 (2026-09-06, 커밋 아래)**: 2026-09-05 백엔드가 13분 죽었을 때 `projectsApi` · `providerSettingsApi` 만 *"서버가 재시작 중이거나 꺼져 있을 수 있습니다"* 라고 말했고, 나머지는 전부 *"서버 응답을 확인할 수 없습니다"* 였다 — **안 떠 있던 서버를 두고 응답을 탓하는 문장**이라, 사람을 응답이 아니라 서버를 보러 가게 만들어야 할 자리에서 반대로 보낸다.
  - 🟠 **"열여섯" 이 아니라 스물둘이었다.** `httpError.ts` 주석의 열여섯은 `toApiErrorShape` 를 쓰는 모듈 수였고, **그 형태를 안 쓰는 여섯**(`archive` · `assets` · `generatedImages` · `instagramPublish` · `longStoryBible` · `photoCards`)이 같은 문장을 각자 갖고 있었다. `photoCards` 는 코드 없는 응답의 기본값이 `MALFORMED` 가 아니라 `UNKNOWN` 이라, 기존 `code` 줄을 안 건드리고 판정용 `carriedCode` 를 따로 뒀다.
  - 🔴 **`INTERNAL_ERROR` 문구는 스물둘 전부가 아니라 열넷만 받았다** — `httpError.ts` 가 스스로 적어 둔 경고대로다. *"다시 눌러도 같은 자리에서 나면"* 은 **프로젝트를 읽거나 설정을 저장할 때만 안전한 조언**이고, 유료 일곱(`imageGeneration` · `imageReview` · `longProjects` · `narration` · `storyPrompt` · `videoSubmission` · `videoWorkflow`)에서는 **재시도가 청구되는지를 `billedOnFailure` 가 아니라 이 문장이 답하게 된다**. 게시(`instagramPublish`)는 더 나쁘다 — 되돌릴 수 없는 쪽이라 그 조언이 **사람을 두 번 올리게** 만든다. 여덟은 `SERVER_UNAVAILABLE` 만 받고 `INTERNAL_ERROR` 는 지금처럼 UNKNOWN 으로 남는다.
  - 🟢 **짝은 바꾼 게 아니라 갈랐다.** 일곱 모듈이 `5xx + 코드 없는 본문`을 malformed 로 못 박고 있었는데, **지우지 않고 상태를 4xx 로 내려 원래 사실을 남긴 뒤 5xx 짝을 새로 붙였다**: `4xx + 못 읽는 본문` = 답을 하긴 했는데 못 읽는 답, `5xx + 오류 모양 없음` = 아무도 답하지 않은 것.
  - 🟠 **Cowork 이 갈라 놓은 다섯 말고 둘이 더 있었다**(`videoSubmissionApi` · `videoWorkflowApi`) — 그쪽은 vitest 를 못 돌려서 타입까지만 확인할 수 있었고, 이 둘은 **실행해야만 빨개지는 자리**였다. 그쪽이 같은 라운드에 정의한 패턴을 그대로 적용했다. 프론트 1344 → 1351.
- [x] **🟠 간격 두 변종을 통일하려다 전제가 틀린 걸 찾았다 — 취향이 아니라 세 층이었다 (2026-09-06)**: `space-y-3` / `space-y-4` 를 하나로 미는 일로 시작했는데, 세어 보니 **`space-y-4` 열세 군데가 같은 규칙**(카드·패널 `p-5`·`p-6` 안의 블록 사이)을 쓰고 있었고 **그 층만 디자인 시스템 문서에 없었다.**
...
- [x] **🔴 글꼴을 바꾸기 직전에 확인한 것: 폴백 짝은 살아 있었지만 **가족 목록을 손으로 들고 있어서** 바꾸는 순간 옛 가족을 지키게 돼 있었다 (2026-09-06)**: Cowork Round 590 이 `subtitle-file.ts:8` 주석이 가리키는 `subtitle-font.test.ts` 가 없다고 보고했다. **짝은 있다** — `subtitle-file.photo-card.test.ts` 안에 있고, `.ttf` 의 name 테이블에서 가족명을, OS/2 에서 굵기를 직접 읽는다. 주석의 파일명만 낡았다.
  ...
  - 🔴 **정정 (2026-09-06, 같은 날 뒤늦게)**: 이 항목을 처음 쓸 때 *"비율 짝(`subtitle-font-metrics.test.ts`)이 FFmpeg 이 없어 skip 되고, 백엔드 스위트의 그 「1 skipped」 가 이것"* 이라고 적었는데 **둘 다 틀렸다.** FFmpeg 은 이 기계에 설치돼 있고(9.0), 그 짝은 전체 스위트에서 **10.2초 동안 실제로 프레임을 렌더링하고 통과한다** — 즉 `PHOTO_CARD_SUBTITLE_CSS_RATIO` 는 지금도 지켜지고 있다. 「1 skipped」 는 `assets.cross-process.worker.test.ts` 이고, 그건 `describe.runIf(Boolean(root))` 로 **부모 테스트가 환경변수를 주고 진짜 별도 프로세스로 띄울 때만** 도는 헬퍼라 평소 실행에서 건너뛰는 게 설계다.
  - 🟠 **틀린 방식이 기록해 둘 만하다**: 스위트 요약의 `1 skipped` 를 보고 **어느 것인지 확인하지 않은 채** 마침 머릿속에 있던 짝(`skip()` 을 부르는 유일한 것으로 기억하던)에 갖다 붙였다. Cowork 이 같은 날 자기 것으로 박은 규칙 — *"`head` 로 자른 출력을 근거로 「전부」 라고 말하지 않는다"* — 의 같은 병이다: **개수만 보고 이름을 확인하지 않았다.** `--reporter=json` 이 30초 만에 답했다.
- [x] **🔴 명언 카드 다섯이 「단기 프로젝트」 제목 아래, 자기가 건너뛰는 단계를 세는 진행률 막대 위에 있었다 (2026-09-06, 캡틴D 지적)**: *"명언 카드는 단기 프로젝트와는 결이 다른데 왜 단기 프로젝트 칸에 있는 거야?"* — 카드는 **디스크에서는** 단기 프로젝트가 맞다(같은 저장소·병합·게시·음원 라이브러리, `D-048` 시절 세 번째 `ProjectType` 을 안 만든 이유). **화면에서는 아니다**: 사이드바 항목도 앞문도 따로 있고, 그 목록이 세는 단계 중 다섯을 건너뛴다.
  - 🔴 **거르기만 하면 치우는 게 아니라 완성된 작업을 못 찾게 만드는 것이라, 두 반쪽을 같은 변경으로 냈다**: `ProjectList` 가 `photoCard === true` 를 빼고(요약 줄도 같은 걸러진 집합을 센다), 「명언 카드」 화면에 **「만들어 둔 카드」 목록**이 생겼다 — 이제 카드로 가는 유일한 문이다. 이미 부르고 있던 `GET /projects` 를 그대로 쓰고 새 요청을 늘리지 않았다.
  - 🔴 **같은 변경이 두 문장을 거짓으로 만들었고 둘 다 고쳤다**: 중복 이름 안내의 *"프로젝트 목록에서 열면 됩니다"* 는 이제 갈 데가 없고, `listProjects()` 실패를 삼키던 `.catch(() => undefined)` 는 **그때는 중복 경고만 잃었지만 이제 "카드가 안 보인다" 와 구별이 안 된다** — 문구를 띄우되 새로 만들기는 그대로 둔다.
  - 🟠 **톤 표는 복사가 아니라 이사했다**: 카드 목록도 같은 상태 칩을 쓰는데 `statusTone` 이 `ProjectList` 안에 있었다. 복사했으면 **두 목록이 「실패」 를 다르게 그리게 되는 경로**가 생긴다. 라벨 표가 이미 사는 `utils/workflowStateLabels.ts` 로 옮겨 `workflowStateTone` 으로 내보냈다 — 같은 매핑의 나머지 절반이다.
  - 🟠 **`App.test.tsx` 의 짝 하나가 이 변경으로 빨개졌다** — 주제는 상세 화면의 두 단계인데 깨진 건 **들어가는 문**이었다(프로젝트 목록에서 카드를 클릭). 새 문으로 바꿨다: `#/photoCard` → 「만들어 둔 카드」의 그 카드 → 상세. 🟢 그래서 이 짝이 이제 **새 문이 실제로 카드에 닿는지까지** 붙든다 — 주입(`onOpenCard` 를 no-op 으로)에 빨개진다. 프론트 1351 → 1356.
- [x] **🔴 미리보기가 자막을 8% 좁게 그리고 있었다 — 비율을 뽑는 식이 한 항을 빠뜨렸고, 그걸 지키는 짝은 같은 식으로 검사하고 있었다 (2026-09-06, 캡틴D 지적)**: *"영상 병합하면 자막이 이렇게 안 나와"* — 미리보기는 한 줄인데 영상은 두 줄이었다. 실측(ffmpeg 9.0, 프레임 픽셀 직독)으로 재현했다: 본문 한 줄이 **957px** 인데 줄바꿈 폭이 **928px** 라 실제로 접힌다.
  - 🔴 **원인은 상수가 낡은 것도 폰트가 다른 것도 아니었다.** `PHOTO_CARD_SUBTITLE_CSS_RATIO` 를 뽑는 식이 `libass 글자당 advance ÷ ASS Fontsize` 였는데, 이 나눗셈은 **CSS 가 한글 한 글자를 1.0em 으로 그린다**고 가정한다. 두 face 다 아니다 — `hmtx`/`head` 가 직접 말한다: **Noto Sans KR Medium 0.920 em · Noto Serif KR Bold 0.966 em**(브라우저 실측과 소수 다섯 자리까지 일치).
  - 🔴 그래서 **모든 값이 정확히 그 인수만큼 작았다**: `0.6898 × 0.920 = 0.6346`(적혀 있던 값) · `0.6972 × 0.966 = 0.6735`. 0.635 → **0.690**, 0.671 → **0.697** 로 고쳤다.
  - 🔴 **짝은 이걸 구조적으로 못 잡았다** — 같은 식으로 나눠서 비교했으니 **상수를 그 상수가 나온 산수와 대조**하고 있었다. 언제나 맞는다. `hangulEmAdvance()` 로 파일에서 em 을 읽게 했고, 🟢 **폰트를 갈아 끼우면 저절로 따라간다**(나눔명조 건의 전제이기도 하다 — 안 고치면 같은 버그를 새 폰트로 다시 만든다).
  - 🔴 **허용치도 같이 조였다.** `toBeCloseTo(, 1)` 은 ±0.05, 0.635 에서 **±7.9%** 인데 통과시킨 오차가 **8.0%** 였다 — 0.1%p 차로 빠져나갔다. 이제 **상대 2%** 다: libass 판본 간 편차(6.1 대 9.0 이 같은 문자열을 2% 안팎 다르게 잰다)는 견디고, 이 짝이 존재하는 이유인 그 실수는 다시 못 들어온다.
  - 🟠 **폰트 파일 읽기를 `font-file-tables.ts` 한 집으로 모았다** — `familyNames` · `usWeightClass` 는 짝 안에 있었고 거기에 `hangulEmAdvance` 를 더했다. 두 짝이 같은 파일에 *"이 파일이 뭐라고 말하나"* 를 묻는데 사본이 둘이면 답이 둘이 된다.
  - 🟢 주입: `bodyEm` 을 `1.0`(옛 식)으로 되돌리면 **8.03% 로 빨개진다** — 고치기 전에는 같은 자리가 초록이었다. 백엔드 1520 · 프론트 1356.
  - 🟠 **남은 것은 Cowork 몫**: 섞인 문자열(괄호·한자)은 순한글로 뽑은 비율 하나로 못 맞춘다 — 제목이 여전히 2.4% 어긋난다. 경계 근처 줄에 *"영상에서 두 줄로 나뉠 수 있습니다"* 를 말하게 하는 것이 그 나머지다. **정확도를 높이는 것과 정확할 수 없다는 걸 말하는 것은 다른 일이다.**
- [x] **🔴 「보관하기」 확인란이 칠 수 없는 글자를 요구하고 있었다 — 그리고 그건 캡틴D 프로젝트 하나가 아니라 명언 카드 여섯 전부였다 (2026-09-06, 캡틴D가 20분 막히심)**: `꽃말_장미` 를 보관하려는데 버튼이 안 켜졌다. 화면은 *"계속하려면 정확한 주제를 입력하세요: 빨간 장미 열렬한 사랑"* 이라고 띄웠고, 그대로 쳐도 안 됐다.
  - 🔴 **저장된 값에 줄바꿈이 있었다**(`빨간 장미
열렬한 사랑`). HTML 은 공백·줄바꿈을 하나로 접어 그리므로 화면에는 한 줄로 보이고, **한 줄짜리 `<input>` 은 줄바꿈을 담을 수 없다.** 비교는 원본을 그대로 했다. **어떤 입력으로도 켜질 수 없는 버튼**이었고, 실패해도 아무 말이 없어서 **캡틴D는 본인이 오타 내는 줄 아셨다.** 화면이 사실이 아닌 것을 말하는 오늘의 그 줄기인데, 이건 한 발 더 나쁘다 — **사용자에게 자기 탓이라고 믿게 만든다.**
  - 🔴 **실제 저장소를 훑어 보니 `꽃말_장미` 만이 아니었다: 명언 카드 여섯이 전부 여러 줄 주제를 갖고 있고(둘은 줄바꿈 앞에 공백까지 둘), 여섯 다 같은 이유로 보관·삭제가 불가능했다.** 아무도 신고하지 않은 것은 아무도 카드를 지우려 한 적이 없었기 때문이다.
  - 🟢 **그래서 주제를 한 줄로 강제하지 않았다** — 그게 첫 반응이었고, 틀렸다. 명언 카드의 주제는 `quote` 그 자체이고(`photo-card.service.ts:67`), 그 줄바꿈은 `splitPhotoCardSubtitle` 이 **제목 줄과 본문**으로 가른다 — `subtitle-file.ts` 가 영상에 굽는 바로 그 구분이다. **정규화했으면 카드 여섯 편의 자막이 통째로 바뀐다.** 줄바꿈은 사고가 아니라 데이터다.
  - 🟢 **고친 자리는 비교하는 쪽이다**(`utils/confirmationMatch.ts`): 화면에 보인 형태(공백 연속을 접고 양끝을 다듬은 것)로 견주고, **서버에는 저장된 원본을 보낸다.** 뒤쪽이 핵심이다 — 화면만 느슨하게 하고 타이핑한 값을 보내면 서버가 거절해서 **실패 지점만 뒤로 옮긴다.** 화면은 「사람이 읽고 다시 쳤다」를 증명하는 자리이고, 서버로 가는 값은 저장된 그것이어야 한다. 서버의 정확 일치 검사(`projects.service.ts:93·131`)는 그대로 둔다.
  - 🟠 **곁다리로 하나 더 막았다**: 라벨이 빈 문자열이면 **빈 칸으로 완전 삭제가 통과**하고 있었다. 아무것도 안 친 것은 의사를 증명하지 못한다. 저장소 전수 확인 — 주제가 빈 프로젝트는 **0개**라, 이 가드가 가둬 놓는 프로젝트는 없다.
  - 🟢 주입: 비교를 옛 정확 일치로 되돌리면 **짝 넷이 빨개진다**(유틸 둘 · `ProjectDetail` · `ArchiveScreen`). 되돌리면 스물여섯 다 초록. 프론트 1363 → 1368.
  - 🟠 **줄바꿈을 처음 넣은 것은 같은 날 지운 `POST /flower-cards` 였다**(`efc27bb` → `8dc0729`). 그 길은 이미 없어졌지만 **버그는 그것과 무관하게 살아 있었다** — 명언 카드가 훨씬 오래 그러고 있었다. *증상을 만든 코드를 지웠다고 증상이 사라지는 것은 아니다.*
- [x] **🟢 꽃말 릴스 — 처음엔 사람이 대본을 쓰는 두 번째 길로 만들었고, 같은 날 그걸 걷어내고 기존 파이프라인의 **프리셋**으로 바꿨다 (2026-09-06, 캡틴D 주문 → 캡틴D 결정)**: *"꽃말의 유래와 의미를 릴스로."* 씨앗이 심기고 꽃이 피는 장면들, 장면마다 자막, 9:16·16:9, 배경음악.
  - 🔴 **처음 판(`efc27bb`)이 작동하지 않았고, 그걸 아무 짝도 잡지 않았다.** `POST /flower-cards` 가 사람이 쓴 `description`·`caption` 을 장면에 넣었는데, **장면에 키가 셋뿐**(`number`·`description`·`narration`)이었다. 이미지 프롬프트는 `visual_action`·`shot_size`·`camera_angle`·`composition`·`lens_feel`·`focus_subject` 를 읽는다 — **`imagePromptFor` 가 빈 문자열을 돌려준다.** 캡틴D가 치신 *"씨앗이 흙에 심긴다"* 는 그림 요청에 한 글자도 안 들어갔다.
  - 🟢 **돈은 안 나갔다.** `assertValidScenes`(`local-image-generation.service.ts:77`)가 `visual_action` 이 비면 `IMAGE_GENERATION_FAILED` 로 거절하고, `video-preview.service.ts` 의 `parseScenes` 는 17칸을 요구한다. **게이트 둘이 각자 막았다** — 잃은 것은 돈이 아니라 캡틴D의 시간이다.
  - 🔴 **왜 아무도 못 봤는지가 이 항목의 값이다.** 양쪽 짝 스물넷이 전부 **만드는 것까지만** 봤다: 프로젝트가 생기는가, 상태가 맞는가, 매핑 기준선이 서는가, 화면이 그리는가, 요청이 나가는가. **그 프로젝트로 그림을 살 수 있는가는 아무도 안 물었다.** 그 문장(*"파이프라인을 전부 그대로 쓴다"*)은 **저장·병합·게시에 대해서는 참이고 장면 필드에 대해서는 거짓**이었는데, 둘을 가르지 않았다. 실제 저장소에서 확인했다: `꽃말_장미`(캡틴D가 만드신 것) 장면 키 **3개** 대 프로젝트 1 장면 키 **18개**.
  - 🔴 **캡틴D 결정: 구멍을 메우지 말고 없앤다.** *"그냥 꽃말도 AI로 대본 생성 가능하게 하는데, 채워야 하는 부분만 다르게 채우면 되는 거 아니야?"* — 17칸을 정확히 채우는 것은 스토리 생성 하나뿐이다. 그래서 서버가 연출을 지어내지도(A), 프롬프트가 다른 필드로 떨어지지도(B), 사람이 더 쓰지도(C) 않는다. **세 안 다 안 하는 것이 답이었다.**
  - 🟠 **손으로 쓰기로 한 원래 이유(유래는 사실인데 모델은 사실처럼 생긴 것을 준다)는 취소된 게 아니라 자리를 옮겼다** — 대본 검토 화면이 무료이고, 이미지를 사기 전에 온다. 그걸 안 세고 판단했던 것이다. 프롬프트 층에도 방어를 하나 뒀다(*"확실하지 않은 유래는 「전해진다」 처럼 쓴다"*). 그리고 캡틴D가 유래를 적으시면 그 문장이 그대로 설정에 실려서, 모델은 **지어내는 게 아니라 나누는 일**만 한다.
  - 🔴 **없앤 것**: `POST /flower-cards` · `FlowerCardService`(+컨트롤러·모듈·오류) · `FlowerCardSceneInput`/`CreateFlowerCardRequest`/`Response` · `FLOWER_CARD_*` 넷 · `API_ROUTES.flowerCards` · `flowerCardsApi.*` · vite 프록시 한 줄. **죽은 라우트는 죽은 문서와 다르다** — 같은 날 `817c6d3` 에서 죽은 프롬프트 템플릿에는 "쓰이지 않음" 만 적고 남겼는데, 그건 문서다. 이건 **문**이고, 등록돼 있는 한 못 쓰는 프로젝트를 계속 만든다. 설계는 `efc27bb` 히스토리에 남는다.
  - 🟢 **새 모양에는 새 백엔드가 없다**: `POST /projects` → `PATCH …/settings`(프리셋) → 대본 생성($0.05) → 검토 → 매핑 → 이미지 → 영상 → 병합 → 게시. 전부 기존 경로다. 프리셋은 **실제로 읽히는 칸만** 채운다(608 조사 결과 그대로: 여덟 칸은 대본 프롬프트, 넷은 이미지 프롬프트).
  - 🔴 **돈 문구가 이 변경에서 제일 위험한 자리였다.** *"여기까지는 비용이 들지 않습니다"* 는 손으로 쓸 때는 참이었고 지금은 **돈을 쓰는 버튼 위의 거짓말**이다. 짝이 양쪽을 다 본다 — `$0.05` 를 포함하는가, 그리고 옛 문장을 포함하지 **않는가**. 주입: 옛 문장으로 되돌리면 그 짝만 빨개진다.
  - 🟢 **캡틴D가 이미 만드신 `꽃말_장미` 는 버리지 않아도 된다.** `WAITING_FOR_ASSET_MAPPING_REVIEW` · 장면 2 · 이미지 0 이라 **재생성 게이트 셋을 다 통과한다**(586). 「대본 재생성」 → 대본 생성이면 새 모양이 되고, `lore_context`(장면 2·10초·9:16·자막 켬)는 재생성이 안 건드린다. 🔴 **다만 순서가 조건이다** — 재생성은 캡틴D가 쓰신 유래를 지우므로, 그 문장을 유래 칸에 옮긴 뒤에 눌러야 한다.
  - 타입체크 0 · 백엔드 1520(-13) · 프론트 1363(-7) · 계약 57 · 데스크톱 37 · `nest build` 통과.
- [x] **🟢 폴더 이름 규칙을 세 화면이 같은 말로 하게 했다 — 남아 있던 하나가 옆 화면과 반대 말을 하고 있었다 (2026-09-06)**: 바로 위 항목이 「새 프로젝트」의 거짓말을 고치면서 `CreateLongProjectForm` 을 남겼고, 그러자 **두 폼이 서로 반대 말을** 하게 됐다. 장기도 같은 `isSafeProjectId` 를 쓴다(`long-project-paths.ts:36`) — 서버는 처음부터 한글을 받았다.
  - 🟢 라벨·안내·거절 문구 셋 다 같은 문장으로 맞췄다. 전 트리 재검색: 옛 라벨 `폴더 이름 (…)` **0건**, 옛 문장 `한글과 띄어쓰기는…` **0건**.
  - 🟠 **명언 카드 문장은 일부러 안 맞췄다** — 표현이 다르지만 뜻이 같고 이미 참이다. 오늘 고친 것들은 전부 *참이 아닌 것*이었고, **문장이 화면마다 조금 다른 건 문제가 아니라 서로 반대되는 게 문제**다. 넷을 한 상수로 묶는 것도 같은 이유로 접었다.
  - 🔴 **놓쳤던 방식이 기록할 값이 있다**: 앞 수정에서 `App.test.tsx` 의 같은 문자열 두 곳을 **개수를 세고** 바꿨는데, **그 파일이 두 컴포넌트를 테스트한다**는 것을 안 봤다 — 하나는 단기 폼, 하나는 장기 폼이었고 장기 쪽은 안 바뀌어서 그 짝이 빨개졌다. **개수가 아니라 소유자의 문제**다. 규칙을 한 줄 늘렸다: *같은 문자열을 여러 자리에서 바꿀 때, 개수만이 아니라 각각이 어느 것에서 오는지를 센다.*
  - 프론트 1370.
- [x] **🔴 프롬프트가 「없음」이라고 말한 다음 그것을 지키라고 시켰다 — 꽃 영상에 사람이 들어올 수 있던 문 (2026-09-06, 캡틴D가 승인 전에 직접 찾으심)**: `꽃말_빨간_장미` 의 대본 지시문을 읽으시다가 `[2]` 가 **「등록된 Character Asset 없음」** 이라고 말한 네 줄 뒤에 **「대표 캐릭터는 대본 전체의 중심으로 유지하십시오」** 라고 명령하는 것을 보셨다. 같은 프롬프트의 `[7] 피할 요소` 는 **사람**으로 시작한다. **한 요청 안에서 사람을 금지하고 주인공을 요구했고, 둘 다 지킬 방법이 없었다.**
  - 🔴 **범위는 넷이 아니라 하나였다.** Cowork 은 `[3][4][5][6]` 도 같은 모양이라고 보고했는데(값이 「없음」인데 설명이 남는다), **실제 템플릿을 렌더링해서 재 보니 성질이 다르다**: 그 넷의 문장은 *「…이 있으면 반영하십시오」* · *「이전 장면이 연결된 경우 …」* 로 **조건문**이라 조건이 거짓이면 아무것도 안 시킨다. `[2]` 만 **조건 없는 명령문**이다. 🟠 규칙으로 적는다: **값이 없는데 문장이 남는 것 자체는 버그가 아니다 — 조건문이면 소음이고 명령문이면 모순이다. 세는 대상은 섹션 개수가 아니라 문장의 서법이다.** 넷을 다 고쳤으면 장기 회차 열 편의 프롬프트까지 바뀌어, 6화 재생성이 이상할 때 원인을 못 갈랐다.
  - 🟢 **고친 방식은 「지시문을 값 안으로」다.** 템플릿에서 네 줄을 빼고 `characterCastMetadata()` 가 값과 함께 데리고 다니게 했다 — `story-prompt.service.ts` 의 기존 주석(*"The three metadata variables above already carry their own 「없음」 sentences"*)이 이미 가리키던 방향이다. **값이 없으면 문장째 사라진다.**
  - 🟠 **둘로 갈랐다.** 대표 캐릭터 두 줄은 **이름이 있으면** 나가고(Asset 없이 설정에 이름만 친 프로젝트도 대표 캐릭터가 있는 것이다 — 여기서 같이 지웠으면 **카스트 블록이 없어서 그 지시를 실어 나를 데가 없는 바로 그 프로젝트들**에서 지시가 사라졌다), Character Asset 두 줄은 **카스트가 있을 때만** 나간다(섞지 말라고 할 대상이 없으면 할 말이 아니다).
  - 🔴 **더 깊은 자리를 적어 둔다**: 진짜 원인은 `AUTONOMOUS_SETTING`(「자율」)이 **칸마다 뜻이 다르다**는 것이다. 시각 스타일·색감에서는 *「네가 정해라」* 로 무해한데 **대표 캐릭터에서만 *「인물을 하나 지어내라」*** 가 된다. 한 상수가 열 자리에 쓰이는데 한 자리에서만 명령이 된다.
  - 🔴 **짝이 하나도 없었다.** 고치기 전 백엔드 1520 이 전부 초록이었다 — 네 줄이 평문이라 아무도 안 보고 있었다. 실제 템플릿으로 짝 셋을 넣었다(결함이 반은 파일에, 반은 변수에 있어서 손으로 쓴 픽스처는 자기가 적은 절반만 증명한다).
  - 🟢 **주입 양방향:** 서비스에서 조건을 지우면(항상 내보내면) 짝 **둘**이 빨개지고, 템플릿에 그 문장을 도로 넣으면 짝 **하나**가 빨개진다. 되돌리면 서른 다 초록. 백엔드 1520 → **1523**.
  - 🟠 **이 변경은 단기 프로젝트에만 닿는다** — `describeCharacterCast` 와 `prompts/story/story_generation.txt` 는 둘 다 `story-prompt.service.ts` 하나만 읽는다(전 트리 확인). 장기 회차 프롬프트는 다른 경로다.
  - 🟢 캡틴D는 이번 편에서는 그 네 줄을 **화면에서 손으로 지우고** 승인하셨다. 이 수정은 **다음 프로젝트부터 지울 필요가 없게** 한다.
- [x] **🔴 이어붙임 참고 이미지의 기록된 이름이 「있다/없다」만 말하고 「달라졌다」는 절대 못 말했다 (2026-09-06)**: 장면 1이 이전 프로젝트의 마지막 그림을 참고로 받는 경로에서, 기록되는 이름이 **고정 문자열 `"continuity"`** 였다. 바로 위 주석은 *「이어붙임 이미지도 참고 하나이고 바뀔 수 있으니 이름을 붙인다」* 고 말하는데 **이름이 안 움직였다.**
  - 🔴 **연결을 다른 프로젝트로 갈아 끼워도, 연결된 프로젝트가 자기 마지막 장면을 다시 그려도 기록은 글자 하나 안 바뀐다.** 신선도 검사는 참고가 **생겼다·없어졌다**만 보고 **다른 그림이 됐다**는 못 본다 — 이미 없어진 참고로 그린 그림이 스스로를 최신이라고 보고한다.
  - 🟠 **경로로는 못 잡는다는 것을 확인하고 정했다**: `writeBinary` 가 임시 파일을 **같은 파일명 위로 rename** 하므로 재생성해도 경로가 그대로다. Asset 참고를 이름 짓는 **버전 번호도 여기엔 없다** — 생성된 장면 이미지는 Asset Library 항목이 아니다. 움직이는 것은 파일 자신뿐이고, `resolveReferences` 가 **이미 그 파일을 stat 하고 있어서** 추가 읽기 없이 쓸 수 있다.
  - 🟠 **바이트 해시가 아닌 이유를 적어 둔다**: 이 계산은 신선도를 다시 잴 때마다 장면당 한 번 도는데, 해시로 하면 **화면 한 번 여는 일이 참고 이미지 전부를 읽는 일**이 된다. 대신 치르는 값은 프로젝트 디렉터리를 **복사**했을 때(백업 복원, 다른 기계로 이동) 그림이 그대로인데 mtime 이 바뀌어 「참고가 달라졌다」로 뜨는 것 — **화면의 잘못된 배지이지 잘못된 그림도 잘못된 청구도 아니다.** 이미지를 쓸 때 해시를 같이 기록해 두면 둘 다 이기는데, 그건 이 변경보다 큰 변경이다.
  - 🔴 **여기도 짝이 없었다.** 고치기 전 1523 이 전부 초록이었고, 고친 뒤에도 전부 초록이었다 — **양쪽 다 아무도 안 보고 있었다는 뜻이다.** 짝 둘을 넣었다: 같은 경로에 다른 그림을 써 넣으면 이름이 달라지는가, 그리고 기록에 **절대 경로가 새지 않는가**(기계의 디렉터리 배치는 안정적이지도 의미 있지도 않고, 데이터 폴더를 옮기면 전부 「달라졌다」가 된다).
  - 🟢 주입: 이름을 옛 상수로 되돌리면 짝 **둘**이 빨개진다. 되돌리면 아홉 다 초록. 백엔드 1523 → **1525**.
  - 🟠 **이건 다음 항목의 전제다.** 장면 N 이 N-1 의 그림을 참고로 받게 넓히면 이 이름이 **기본 동작**이 된다 — 1번을 나중에 고쳤을 때 2번이 낡았다고 말할 수 있어야 재생성 순서를 강제하지 않아도 된다. **틀린 순서가 막히는 게 아니라 보이게 된다.**
- [x] **🔴 게시 경로에만 락 문장이 없었다 — 되돌릴 수 없는 그 하나에 (2026-09-06, 캡틴D 게시 중 `ECONNRESET` 신고에서 출발)**: Cowork 이 Vite 프록시 로그의 `read ECONNRESET` 을 넘겨 왔다(게시 자체는 올라갔고, 화면은 실패로 읽혔을 가능성이 크다는 보고). **먼저 실제 저장소를 읽어 사실부터 세웠다**: `명언_우공이산` 에 `media_id 18369788431213642`, `published_at 2026-09-06T13:35:20.385Z`, `previous_instagram_posts 0` — **기록은 남았고 중복 게시는 일어나지 않았다.**
  - 🔴 **그런데 시간이 문제를 드러냈다.** 프록시 오류는 13:30 대이고 기록은 **13:35:20** 이다. **클라이언트 연결이 끊긴 뒤로도 서버가 4분 넘게 일하고 성공했다.** 즉 느린 게시는 **성공하면서 화면에는 실패로 보인다** — 그리고 그 화면을 본 사람이 다시 누른다.
  - 🔴 **다시 눌렀을 때 무엇이 나오는지가 진짜 결함이었다**: 락 획득 시간은 `ACQUIRE_TIMEOUT_MS` **10초**인데 게시는 **몇 분**을 잡고 있다(업로드 + 최대 3분 처리 폴링). 그래서 두 번째 누름은 **반드시** 타임아웃에 닿는데, **이 앱에서 프로젝트 락을 쓰는 모든 서비스가 그 오류를 자기 문장으로 바꾸는데 게시만 안 했다** — 그대로 새어 나가 **이유 없는 500** 이 됐다. 되돌릴 수 없는 유일한 동작에서, 이미 실패했다고 믿는 사람에게.
  - 🔴 **찾다가 두 번째가 나왔다 — 기록 삭제가 게시와 다른 락을 잡고 있었다.** 삭제 경로는 `${id}:instagram-publish` 를, 게시는 `videos:final` 을 잡는다. **바로 그 자리 주석은 「게시가 잡는 것과 같은 락이라 게시가 기록을 쓰는 도중에는 지울 수 없다」고 말한다 — 거짓이었다.** 둘이 한 파일 위에서 경주할 수 있고, 지면 **존재하는 게시물의 기록이 지워진다.** 기록이 없는 프로젝트는 다시 게시할 수 있으므로, **D-005 가 절대 없어야 한다고 적은 그 중복 게시**가 「재게시를 신중하게 만들려고 둔 버튼」을 통해 나온다.
  - 🟢 **고침**: 네 자리(단기·장기의 게시와 기록 삭제) 전부 `videos:final` 하나를 잡게 하고, 타임아웃을 `INSTAGRAM_PUBLISH_IN_PROGRESS` 로 바꾼다. 🟠 **문구를 「기다리세요」가 아니라 「끝날 때까지 다시 누르지 말고, 인스타그램을 먼저 확인하라」로 썼다** — 기록은 Meta 가 수락한 **뒤에** 쓰이므로 그 몇 분 동안 앱은 성공했는지 **진짜로 모른다.** 「아무것도 게시되지 않았다」는 이 문장이 절대 암시하면 안 되는 것이다.
  - 🔴 **여기도 짝이 없었다** — 인스타그램 183개가 전부 초록인 채였다. 짝 둘을 넣고 서비스에 락 타임아웃 이음매를 하나 뒀다(`poll`·`sleep` 이 이미 그렇게 주입되고 있다). 🟢 주입: 매핑을 지우면 **둘 다** 빨개지고, 삭제 경로의 옛 키를 되돌리면 **삭제 쪽 하나만** 빨개진다 — 두 결함이 각자 잡힌다. 백엔드 1525 → **1527**.
  - 🟠 **남은 것은 더 큰 모양이라 여기 적어 둔다**: 요청이 클라이언트 연결보다 오래 산다는 것 자체는 그대로다. 영상 생성처럼 **작업을 띄우고 폴링**하는 모양이 답인데, 그건 계약·화면·백엔드가 같이 움직이는 변경이다. 지금 고친 것은 **그 사이에 사람이 하는 일이 중복 게시로 끝나지 않게** 하는 것이다.
- [x] **🟠 `[10]` 의 칸 설명이 전부 사람 기준이었다 — 사람을 금지한 프롬프트에서 (2026-09-06, 캡틴D 질문에서 나온 것)**: 캡틴D가 *"꽃말에 필요 없는 칸은 빼면 안 되나"* 라고 물으셨고, **답은 하나도 못 뺀다**였다(`openai-story-adapter.ts` 의 `required` 18 + `additionalProperties: false` — 설명만 지우면 모델은 그 칸을 **뜻을 모른 채 채우고 형식 검증을 통과한다**. 멈추는 실패보다 통과하는 실패가 나쁘다). 🟠 **그런데 재면서 다른 게 보였다**: `자세·시선` · `신체·소품` · `표정` · `인물` — **꽃에는 없는 것들**이고, 같은 프롬프트의 `[7] 피할 요소` 는 **사람**으로 시작한다.
  - 🟢 **칸을 빼는 게 아니라 주어를 바꾸는 문제였다**(Cowork Round 614 ③ 의 틀 그대로): `시선` → `향한 방향`, `신체·소품` → `피사체·소품`, `인물·배경` → `피사체·배경`. **사람이 나오는 프로젝트에 손해가 없다 — 사람도 피사체다.** 좁히는 게 아니라 넓히는 쪽이라 이게 재작성이 아니라 표현 교정이다.
  - 🟠 **`expression_change` 만 다르게 갔다.** 표정은 **있을 때는 진짜로 적을 것**이므로 지우지 않고, *"표정이 없는 피사체라면 그에 준하는 기색·분위기의 변화"* 를 덧붙였다 — 없을 때 무엇을 쓸지 말해 주는 조건절이다. 없는 얼굴을 지어내게 두는 것보다 낫다.
  - 🟢 **안 고친 것도 적어 둔다**: `description` 의 `필요한 대사` 는 **조건절**이라(필요하면) 값이 없을 때 아무것도 요구하지 않는다 — 같은 날 `[2]` 에서 세운 규칙(**조건문이면 소음, 명령문이면 모순**)을 여기에도 그대로 적용했다. 소음을 지우려고 멀쩡한 문장을 건드리지 않는다.
  - 🟢 **그리고 캡틴D 질문 자체를 막는 짝을 만들었다**: `required` 18개를 `STORY_SCENE_FIELDS` 로 이름 붙여 스키마가 쓰게 하고, **템플릿이 요구하는 칸 목록이 그것과 정확히 같은지**(순서까지) 보는 짝을 넣었다. 이제 `[10]` 에서 한 줄을 지우면 **커밋 전에 빨개진다** — 지금까지는 아무것도 두 목록을 잇고 있지 않아서, 프롬프트를 정리하는 것이 유료 이미지 호출 하나를 버릴 수 있었다.
  - 🟢 주입: 칸 한 줄을 지우면 짝 **둘**이 빨개지고, `시선` 을 되돌리면 **사람 가정 짝 하나만** 빨개진다. 백엔드 1527 → **1529**.
  - 🟠 **근거가 실사용에 있다**: 캡틴D가 이번 편에서 `[10]` 주어를 **손으로 고쳐** 승인하셨고, 그 회차의 영상 프롬프트 Performance 칸이 *"고요하고 닫힌 흙의 상태에서 생명이 깨어나는 상태로"* 로 나왔다(Cowork Round 617 ⑧). **손으로 한 것이 먹혔으니 템플릿에 넣는다.**
- [x] **🟢 계약: `sceneImageContinuityEnabled` — 장면끼리 그림을 잇는 스위치 (2026-09-06, Cowork Round 617 ① 의 계약 절반)**: 캡틴D가 첫 편을 끝까지 돌리시고 **1번 영상 마지막 프레임과 2번 첫 프레임이 전혀 안 이어진다**고 보고하셨다(검은 흙 · 무광 화분 → 테라코타 화분 · 다른 배경). 원인은 `image-reference-selection.ts` 가 앞 그림을 넘기는 길을 **「1번 장면에만, 이전 프로젝트에서만」** 열어 두고 있어서 **같은 프로젝트의 N-1 → N 이 아예 없다**는 것이다. 이 항목은 그 기능의 **계약과 설정 왕복**까지이고, 참고를 실제로 넘기는 것은 다음 항목이다.
  - 🟠 **전역 규칙이 아니라 스위치인 이유**: 사람이 나오는 이야기는 장면마다 장소가 바뀌는 게 정상이라, 앞 그림을 무조건 참고로 주면 **이야기가 장면을 바꿀 자유를 그림이 못 따라간다.** 꽃말 릴스는 정반대다 — 한 송이, 한 화분, 한 빛, 한 방향의 진행이라 **체인이 기능 하나가 아니라 전제**다.
  - 🟢 **기본값이 꺼짐인 것이 신선도 홍수도 같이 막는다**: 스위치가 꺼진 프로젝트는 `referenceSourcesForScene` 출력이 한 글자도 안 바뀌므로 **기존 프로젝트 전부 그대로**다. 09-05 에 `styleStale` 이 24장을 한꺼번에 넘긴 그 모양을 이번엔 **켠 사람만** 겪는다.
  - 🔴 **요청에서는 선택 필드로 했다 — 그리고 그게 오늘 배운 것을 적용한 자리다.** 형제인 `narrationEnabled`/`subtitlesEnabled` 는 필수인데, 그 둘은 **처음부터 있었으므로 안 보내는 클라이언트가 존재한 적이 없다.** 이건 **이미 브라우저에서 돌고 있는 빌드가 있는 상태**에서 추가되므로, 필수로 만들면 **그 페이지의 모든 설정 저장이 처음 듣는 이름 때문에 실패한다** — 오늘 `durationSeconds === 5` 한 줄이 캡틴D의 영상 단계를 통째로 막은 바로 그 모양이다. **없으면 꺼짐**이고, **있는데 boolean 이 아니면 여전히 거절**한다(「몰랐다」와 「다른 걸 보냈다」는 다른 일이다).
  - 🟠 `SETTINGS_KEYS` 가 **허용 목록과 필수 목록 두 몫**을 하고 있어서, 선택 필드를 두려면 그 둘을 갈라야 했다(`OPTIONAL_SETTINGS_KEYS`). 응답에서는 항상 값이 실린다 — **그림이 무엇을 보고 그려지는지를 정하는 설정에 「말 안 함」이 답이면 안 된다.**
  - 🟢 저장은 `lore_context.scene_image_continuity_enabled`. `subtitlesEnabled` 와 달리 **다른 필드로 폴백하지 않는다** — 같은 뜻이던 옛 필드가 없고, 이 필드 이전에 만들어진 프로젝트는 **체인 없이 그려졌다.** 켜진 것으로 읽으면 그 그림들이 본 적 없는 참고에서 나왔다고 말하는 셈이다.
  - 🟢 주입: 필드를 다시 필수로 만들면 짝 **다섯**이 빨개진다. 백엔드 1529 → **1531**, 계약 57.
  - 🟠 **프론트 한 줄은 내가 고쳤다** — `ShortProjectSettingsScreen.tsx` 의 `EMPTY_SETTINGS` 가 필수 필드 하나를 빼먹어 **내 계약 변경이 프론트 빌드를 깼다.** 내가 만든 깨짐이라 남겨 두지 않았고, 우편함에 명시했다. **화면의 스위치 자체는 Cowork 몫이고, 동작이 들어간 뒤에 붙인다** — 아직은 아무것도 안 하는 스위치라 켜면 화면이 거짓말을 한다.
- [x] **🔴 장면끼리 그림이 안 이어지던 것을 이었다 — 같은 프로젝트 안에서 N-1 → N 이 아예 없었다 (2026-09-06, Cowork Round 617 ① 의 동작 절반)**: 캡틴D의 첫 편이 **1번은 검은 흙·무광 화분, 2번은 테라코타 화분·다른 배경**으로 나왔다. 대본은 *"장면이 넘어가도 같은 화분"* 을 적었고 그대로 따랐다 — **그림만 그것을 못 받았다.** 앞 그림을 넘기는 길이 **「1번 장면에만, 이전 프로젝트에서만」** 열려 있었다.
  - 🟢 **규칙을 한 곳에 뒀다(`continuityForScene`)**: 묻는 쪽이 셋이다 — **그림값을 치르는 생성**, **재생성**, 그리고 **「당신 그림이 뒤처졌다」고 말하는 신선도 검사**. 이 규칙의 사본이 둘이 되는 순간 앞의 둘과 뒤의 하나가 어긋나고, 그건 이 파일이 D-031 로 이미 적어 둔 사고다. 에피소드는 `chainEnabled: false` 로 부르므로 **가지고 있던 「1번 장면만」 동작을 글자 그대로 유지**한다.
  - 🟠 **이름을 둘로 갈랐다**: 1번 장면이 받는 것은 **사람이 고른 링크**이고, N 이 받는 것은 **스위치가 한 번에 정한 것**이다. 기록에서 구분이 안 되면 무엇이 바뀌었는지 말할 수 없다 → `prev-scene:{N-1}@…` 와 기존 `continuity:…`. 🟢 **기존 접두사를 그대로 둔 이유**: 이미 그 참고를 가진 프로젝트들이 **이 커밋 하나로 전부 「달라졌다」로 읽히지 않게** 하려고.
  - 🟢 **Cowork 이 못 재겠다던 셋 중 2번(재생성 순서)은 앞 항목이 이미 답했다** — 이름이 파일에서 나오므로, 1번을 나중에 고치면 2번이 **저절로** 낡은 것으로 뜬다. **순서를 강제할 필요가 없고, 틀린 순서가 보이게 된다.**
  - 🟠 **3번(상한에서 누가 이기나)은 순서로 답했다**: 체인 참고를 **목록 맨 앞**에 둔다. 상한은 안 들어가는 것을 버리므로 **순서가 곧 「무엇을 버려도 되는가」** 인데, 스위치를 켠 사람은 *그림이 서로 이어져야 한다*고 말한 것이라 **제일 먼저 버릴 것이 아니라 마지막까지 지킬 것**이다. 🟢 1번 장면의 링크는 **원래 자리(맨 끝)를 유지** — 옮기면 그것을 가진 모든 프로젝트의 **기록된 참고 순서**가 바뀌고, 순서 변화 자체가 「뒤처졌다」로 읽힌다.
  - 🟢 주입: 체인을 절대 안 돌려주게 하면 짝 **셋**, 맨 앞 대신 맨 뒤에 붙이면 **순서 짝 하나만** 빨개진다. 백엔드 1531 → **1534**.
  - 🟠 **화면의 스위치는 아직 없다** — Cowork 몫이고, 동작이 들어간 지금 붙이면 된다. 계약만 먼저 나갔을 때 붙이지 말라고 한 이유는 **아무것도 안 하는 스위치가 곧 거짓말하는 화면**이기 때문이다.
- [x] **🟠 `characterCount` 에 진짜 이름을 줬다 — 다만 개명이 아니라 나란히 실었다 (2026-09-06)**: 그 숫자는 **등장인물 수**인데 이름이 `characterCount` 라, 화면이 프롬프트가 가득 찬 상자 위에서 **「글자 수: 0」** 이라고 말했다. 0 은 맞는 답이었고(꽃말 릴스에는 사람이 없다) 캡틴D가 멈춰서 *뭔가 잘못됐냐*고 물으셨다. 화면은 `3f869fd` 에서 먼저 고쳤고, 계약에 거짓 이름을 남겨 두면 **다음 사람이 그 단어를 곧이곧대로 믿는다.**
  - 🔴 **제자리 개명을 안 한 이유가 오늘의 그 교훈이다**: 응답은 **이미 브라우저에 떠 있는 빌드가 읽는다.** `storyPromptApi.ts:118` 의 가드가 이 필드를 필수로 보므로, 백엔드에서 이름만 바꾸면 그 페이지는 **멀쩡한 응답을 「서버 응답을 확인할 수 없습니다」로 거절**한다 — 오늘 아침 `durationSeconds === 5` 가 캡틴D의 영상 단계를 통째로 막은 바로 그 모양이다. **두 이름이 같이 나가고, 화면이 옮겨간 뒤에 옛 이름을 지운다.**
  - 🟠 **615 ⑤ 에서 내가 이걸 「타입체크가 전부 잡아 주는 안전한 개명」이라고 썼는데, 저장소 안에서만 참이었다.** 읽는 곳을 셀 때 **아직 우리 트리에 있는 것만 세면 안 된다 — 이미 사용자 화면에 떠 있는 판도 읽는 쪽이다.** Cowork 이 topic 정규화에서 소비처를 안 센 것을 정정한 바로 다음 라운드에 내가 같은 실수를 했다.
  - 🟢 **두 이름이 어긋날 수 있는 것이 하나 나쁜 이름보다 나쁘다**, 그래서 계약 짝과 백엔드 짝이 **둘이 항상 같은 수인지**를 본다. 주입: 한쪽만 0 으로 만들면 짝 **둘**이 빨개진다. 백엔드 1534 → **1536**, 계약 57.
  - 🟠 **남은 절반은 화면이다** — `StoryPromptScreen` 과 `storyPromptApi` 가 `castCount` 를 읽게 되면 그때 `characterCount` 를 계약에서 지운다. Cowork 몫으로 우편함에 넘겼다.
- [x] **🟢 장면 체인을 「돈을 쓰는 서비스」 층에서 한 번 더 물었다 — 단위 짝이 못 닿는 자리가 정확히 배선이다 (2026-09-06)**: 앞 항목이 해석기(`resolveReferences`)를 증명했는데, **새 프로젝트를 한 번에 돌릴 때 장면 2가 장면 1의 파일을 실제로 만나는가**는 해석의 문제가 아니라 **루프가 방금 그것을 디스크에 썼는가**의 문제다 — 단위 짝이 절대 못 닿는 자리다.
  - 🔴 **오늘 찾은 결함이 전부 그 모양이었다**: 초록인 단위 짝 밑의 배선 구멍(꽃말 릴스의 장면 키 3개, `photoCard` 분기 유실 두 번, 게시 락의 미매핑 타임아웃). **그래서 세 호출자가 만나는 층에서 물었다.**
  - 🟢 짝 둘: 스위치를 켠 프로젝트는 **장면 1만 이미지 1장, 2~6은 2장**을 보내고 기록된 `reference_sources` 의 **첫 자리가 `prev-scene:1@…`** 이다(맨 앞이라 상한이 먼저 못 버린다). 스위치를 안 켠 같은 프로젝트는 **예전에 사던 것을 글자 그대로** 산다 — 이 기능을 내보낼 수 있게 한 그 보증을 짝으로 고정한 것이다.
  - 🟢 주입: 호출자가 설정을 안 읽게 하면(`chainEnabled = false`) 체인 짝만 빨개진다. 백엔드 1536 → **1538**. 유료 호출 0(스텁 fetch).
  - 🟠 **한 가지 알게 된 것**: 참고 수집 전체가 `apiKey && budget` 분기 안에 있어서 **로컬 가짜 어댑터 경로에서는 참고도 기록도 아예 안 만들어진다.** 그래서 이 기능은 **유료 경로에서만 관측 가능**하고, 그 관측을 스텁으로 하는 이 짝이 유일한 상시 증거다.
- [x] **🔴 대본을 쓰는 모델이 「이 칸이 몇 초짜리 한 컷이 되는지」도, 「그 컷이 인물 유지를 요구받는지」도 몰랐다 (2026-09-06, 6화 6번 장면 실제 실패에서)**: 캡틴D의 6화 6번이 Runway 에서 `INTERNAL.BAD_OUTPUT.CODE01` 로 실패했다($0.25 청구됨, 1~5번은 성공). 그 프롬프트를 읽어 보니 **5초 한 컷에 카메라 이동 두 번 + 얼굴 변형 세 번 + `Pacing: 빠르고 불안정 / 최고조`** 를 요구하면서, 같은 요청의 **마지막 고정 줄이 `Maintain stable identity, anatomy, clothing …` 이었다.**
  - 🔴 **한쪽은 얼굴을 삼키라고 하고 다른 쪽은 해부학을 유지하라고 한다.** 그리고 **두 문장의 저자가 다르다** — 앞은 대본 모델이 쓴 `end_motion`, 뒤는 `promptFor()` 가 모든 장면에 조건 없이 붙이는 고정 접미사다. **대본 모델은 자기 문장 뒤에 그 줄이 붙는 걸 모르고, 코드는 대본이 뭘 썼는지 안 본다.**
  - 🔴 **클립 길이가 `narration` 칸 설명에만 있었다.** 움직임 칸들은 자기가 몇 초짜리 컷이 되는지 모른 채 쓰였다 — 그래서 5초에 안 들어갈 분량을 써도 아무도 안 막았다.
  - 🟢 **둘 다 칸을 요구하는 자리에서 말하게 했다**: 움직임 항목이 `$clip_duration_seconds`초 연속 한 컷이 된다는 것(주요 동작 하나 + 카메라 이동 하나 분량), 그리고 **영상 요청이 정체성·신체·의상 유지를 함께 요구하므로 얼굴·신체가 부서지거나 바뀌거나 사라지는 변화는 움직임 칸에 쓰지 말 것**(그 사건이 필요하면 `description` 에 서술하고 움직임 칸에는 직전까지만). **유료 영상 한 편이 실패하는 것보다 대본 호출 한 번이 싸다.**
  - 🟠 **접미사 쪽은 일부러 안 고쳤다 — 재 보니 비쌌다.** `describesSameScene` 은 `": "` 가 없는 줄을 값으로 통째로 비교하므로 **접미사도 비교 대상**이다. 한 단어만 바꿔 렌더링해 확인했다: **다른 장면으로 판정된다.** 즉 접미사를 손대면 디스크의 기록된 영상 프롬프트 **36개 전부**가 「장면 내용이 바뀐 뒤로 다시 만들지 않았습니다」로 뜨고, **전부 거짓이다.** 09-05 에 첫 줄(클립 길이·화면 방향)에서 고친 바로 그 결함이 **마지막 줄에 그대로 남아 있었다** — 그때 접두만 보고 접미는 안 봤다.
  - 🟠 그래서 접미사 수정은 `videoPromptDrift` 가 「템플릿이 바뀜」을 따로 말할 수 있게 된 뒤로 미룬다(계약 필드 + 배지 → Cowork 협업). 🟢 **다만 이 항목이 그 필요를 크게 줄인다** — 대본이 정체성을 부수지 않으면 접미사와 부딪힐 일이 없다.
  - 🟢 주입: 클립 길이를 5로 굳히면 10초 프로젝트 짝이, 정체성 문단을 지우면 다른 짝이 빨개진다. 백엔드 1538 → **1540**.
- [x] **🟠 영상 프롬프트의 마지막 고정 줄이 「장면 내용」으로 세어지고 있었다 — 09-05 에 첫 줄에서 고친 그 결함이 마지막 줄에 그대로 있었다 (2026-09-06)**: `describesSameScene` 은 `": "` 없는 줄을 값으로 통째로 비교한다. 모든 섹션은 `Label: value` 로 렌더링되므로 그 조건에 걸리는 줄은 **첫 줄과 마지막 줄뿐**이고, 09-05 에는 첫 줄만 다뤘다.
  - 🔴 **재서 확인했다**: 마지막 줄을 한 단어만 바꿔 렌더링하면 **다른 장면으로 판정된다.** 즉 그 문장을 고치는 순간 디스크의 기록된 영상 프롬프트 **36개 전부**가 「장면 내용이 바뀐 뒤로 다시 만들지 않았습니다」로 뜬다 — **전부 거짓**이고, 아무 대본도 안 움직였다.
  - 🟢 **이미지 쪽은 이 문제를 처음부터 안 만들었다**: `NO_LEGIBLE_TEXT_RULE` 은 **기록된 프롬프트에 아예 안 넣고** 요청에만 붙인다. 그 주석이 이유까지 적어 뒀다 — *「고정 줄을 기록에 더하면 지금까지 만든 모든 그림이 자기 대본보다 뒤처진 것으로 표시된다」*. **영상 쪽만 그 결론을 반대로 적용하고 있었다.**
  - 🟠 **다만 반대편 끝에서 같은 결론에 닿았다**: 그 줄은 **기록에도 요청에도 그대로 남긴다** — 그게 실제로 제공자에게 요구한 내용이기 때문이다. 대신 **비교만 그 줄을 뺀다.**
  - 🟢 **문장이 아니라 모양으로 뺀다**: 현재 문구와 정확히 일치하는 것만 지우면 **오늘 판을 가진 쪽에서만** 지워지고, 정작 중요한 비교(옛 기록 ↔ 바뀐 문장으로 재계산)는 여전히 어긋난다. 그래서 **구분자 없는 마지막 줄**을 뺀다. 첫 줄은 일부러 남긴다 — `videoPromptDrift` 가 클립 길이·화면 방향 변경을 장면 편집과 가르는 방법이 그것이다.
  - 🟢 주입: 마지막 줄을 다시 비교에 넣으면 짝 하나, 요청에서 그 줄을 빼면 짝 **둘**이 빨개진다(비교에서 빼는 것이 조용히 안 보내는 것으로 새지 않는다는 확인). 백엔드 1540 → **1542**.
  - 🟠 **이제 그 문장을 고칠 수 있다.** 6화 6번이 실패한 이유의 절반이 *「얼굴을 삼켜라」와 「해부학을 유지하라」가 한 요청에 있었다* 는 것인데, 바로 앞 항목이 대본 쪽 절반을 막았고 이 항목이 **코드 쪽 절반을 고칠 길을 열었다.** 실제 문구 수정은 별도 항목으로 둔다 — 지금 고칠 이유가 급하지 않고, 대본이 정체성을 안 부수면 부딪힐 일도 없다.
- [x] **🔴 정작 6화를 쓴 프롬프트는 장편 쪽 인라인이었다 — 같은 두 규칙을 거기에도 넣었다 (2026-09-06)**: 바로 앞 항목이 `prompts/story/story_generation.txt` 를 고쳤는데, **실제로 실패한 6화는 그 템플릿을 안 읽는다.** 장편 회차 대본은 `episode-scripts.service.ts` 의 `buildScriptPrompt()` 가 만든다. **고친 곳과 터진 곳이 달랐다.**
  - 🟠 **그 프롬프트에는 09-05 의 교훈이 이미 둘 들어 있었다** — 「한 장면에 속도 하나」와 「읽히는 글자를 사건으로 삼지 말 것」. 둘 다 유료 실패에서 나온 것이고, 그래서 이번 둘이 없다는 게 더 눈에 띈다: **페이스 규칙은 속도를 붙잡지만 그 속도로 몇 가지가 일어나는지는 말하지 않는다.**
  - 🟢 **단기 템플릿과 같은 문장으로 넣었다** — 「$clip_duration_seconds초 연속 한 컷 · 주요 동작 하나 · 카메라 이동 하나」와 「정체성·신체·의상 유지 요구가 함께 가므로 얼굴·신체가 부서지는 변화는 움직임 칸에 쓰지 말 것」. **두 경로가 다른 말을 하면 언젠가 한쪽만 고쳐진다.**
  - 🟠 **주입이 한 번 안 물어서 짝을 고쳤다**: 클립 길이를 리터럴 5로 굳혀도 픽스처가 5초라 통과했다. **5인 픽스처는 5와 하드코딩을 구별하지 못한다.** 그래서 같은 값이 이미 보간되던 `narration` 줄의 숫자와 **서로 같은지**를 보게 했다 — 둘 중 어느 쪽을 굳혀도 어긋난다. 🟢 다시 주입하니 빨개진다. 백엔드 1542 → **1543**.
- [x] **🟠 앞의 두 항목이 같은 문장을 두 곳에 만들었고, 같은지 보는 것이 없었다 — 내가 방금 만든 위험을 닫았다 (2026-09-06)**: 단기 대본은 `prompts/story/story_generation.txt`, 장편 회차 대본은 `episode-scripts.service.ts` 의 인라인 프롬프트가 만든다. **오늘 그 둘에 같은 두 규칙을 「같은 문장으로」 넣어 놓고, 커밋 본문에는 *두 경로가 다른 말을 하면 언젠가 한쪽만 고쳐진다* 고 써 놓고, 정작 같은지 확인하는 것은 안 만들었다.**
  - 🔴 **그 위험이 추상적이지 않다는 증거가 같은 날 있다**: 6화 실패를 고칠 때 **내가 먼저 고친 곳이 6화가 읽지 않는 파일이었다.** 두 곳을 손으로 맞춘 상태는 정확히 그 사고가 다시 나는 조건이다.
  - 🟢 **문장을 `story/motion-field-rules.ts` 한 곳으로 옮기고 서비스가 거기서 만들게 했다.** `.txt` 는 상수를 import 할 수 없지만 **상수에 비추어 검사할 수는 있다** — 템플릿이 그 문장을 그대로 담고 있는지 보는 짝을 넣었다.
  - 🟠 **길이 자리를 빼고 비교하지 않았다**: 템플릿은 `$clip_duration_seconds`, 서비스는 숫자를 보간한다. 초를 잘라내고 비교하면 **길이가 통째로 사라져도 안 잡힌다.** 그래서 한쪽에서 다른 쪽을 **렌더링해서** 비교한다.
  - 🟢 주입 양방향: 템플릿에서 「카메라 이동 하나」를 「두 번」으로 바꾸면 짝 하나, 상수에서 「얼굴이나 신체가」를 「얼굴이」로 줄이면 다른 짝이 빨개진다. 백엔드 1543 → **1546**.
- [x] **🔴 09-05 에 돈을 내고 배운 규칙 둘이 장편 프롬프트에만 있었다 — 단기 대본은 그걸 모른 채 같은 영상 모델에 나가고 있었다 (2026-09-06)**: 바로 앞 항목의 렌즈(**손으로 맞춘 것은 맞춰져 있다는 증거가 아니다**)를 두 대본 프롬프트에 그대로 대 봤더니, 오늘 넣은 둘 말고도 **더 오래된 비대칭이 있었다.**
  - 🔴 **장편에만 있던 규칙 둘**: 「`motion_speed`·`motion_intensity` 에는 장면 전체에 걸친 하나의 상태만」과 「읽히는 글자를 장면의 주된 사건으로 삼지 말 것」. **둘 다 09-05 의 유료 실패에서 나왔다** — 한 장면이 두 번 거절돼 **$0.50** 이 사라졌고(첫 프레임이 테이프 라벨 클로즈업), Episode 5 6번은 「느림에서 빠름으로 전환」으로 3.7초에 화면이 하얗게 날아갔다.
  - 🔴 **그런데 그 수정이 장편 프롬프트에만 들어갔다.** 단기 프로젝트는 **같은 영상 모델에 같은 돈으로** 그 두 가지를 그대로 쓸 수 있었고, 아무것도 그렇다고 말하지 않았다 — **두 프롬프트를 비교하는 것이 없었기 때문이다.**
  - 🟢 넷 다 `motion-field-rules.ts` 한 곳으로 모으고(`ONE_PACE_RULE` · `NO_TEXT_AS_EVENT_RULE` 추가), 장편은 거기서 줄을 만들고, **단기 템플릿에 그 두 문장을 넣었으며**, 짝이 **네 규칙 전부**를 양쪽에서 확인한다.
  - 🟢 주입: 단기 템플릿에서 글자 규칙을 다시 빼면 짝이 빨개진다. 백엔드 1546 → **1547**.
  - 🟠 **오늘 같은 모양이 네 번째다.** 앞의 셋과 다른 점은 **이번 것은 하루 더 오래됐고 아무도 안 밟았다**는 것이다 — 단기 프로젝트로 영상까지 간 게 오늘 꽃말 릴스가 처음이라, 그 두 실패가 단기 쪽에서 재현될 기회가 없었을 뿐이다.
- [x] **🔴🔴 장편 회차 프롬프트가 스키마가 요구하는 18칸 중 11개만 이름을 불렀다 — 안 불린 다섯이 전부 이미지 프롬프트로 가는 칸이다 (2026-09-06)**: 같은 렌즈를 반대 방향으로 대 보다가 나왔다. `buildScriptPrompt()` 는 열 칸을 나열하고 `narration` 을 따로 말하는데, `callOpenAiStoryApi` 가 보내는 스키마는 **`STORY_SCENE_FIELDS` 18개를 `required` 로, `additionalProperties: false` 로** 강제한다.
  - 🔴 **안 불린 다섯이 하필 `shot_size` · `camera_angle` · `composition` · `lens_feel` · `focus_subject` 다** — `image-prompt.ts:60-64` 가 읽는 정확히 그 다섯이다. **모든 회차 이미지가 부분적으로 「모델이 뜻을 모르고 채운 값」에서 그려지고 있었다.**
  - 🟠 **이게 왜 조용했는지가 이 항목의 값이다**: 필수 칸에 설명이 없으면 **빠지는 게 아니라 채워진다.** 형식 검증을 통과하고, 아무 오류도 안 나고, 그림만 엉뚱해진다 — Cowork 이 614 ② 에서 *「멈추는 실패가 통과하는 실패보다 낫다」* 로 정리한 그 모양이고, 그때는 **캡틴D가 칸을 빼려 하셨을 때** 그러면 안 되는 이유로 나왔던 논리다. **같은 논리가 이미 우리 프롬프트 안에서 실현돼 있는 줄은 아무도 몰랐다.**
  - 🟢 **단기 템플릿은 처음부터 18개를 다 설명하고 있었다.** 뒤처진 건 이 사본이고, **두 프롬프트를 비교하는 것이 없어서** 하루가 아니라 처음부터 그랬다.
  - 🟢 다섯 칸의 이름·뜻과 `number` 를 프롬프트에 넣고, 단기 템플릿이 이미 갖고 있던 안내(전체 카메라 느낌을 바탕으로 하되 현재 장면의 서사를 우선, `camera_motion` 은 정지 구도와 분리)도 같이 넣었다.
  - 🟢 **짝은 문장이 아니라 계약을 본다**: `STORY_SCENE_FIELDS` 전부가 실제로 보내진 프롬프트 안에 나오는지 확인한다 — 스키마에 칸이 늘면 **설명 없이 늘어나는 것이 막힌다.** 주입: 그 다섯을 다시 빼면 빨개진다. 백엔드 1547 → **1548**.
- [x] **🟠 장편 영상 검증이 정본 필드 목록을 손으로 다시 적고 있었다 — 값은 맞았고, 그게 요점이다 (2026-09-06)**: 같은 렌즈를 이미지·영상 경로에도 댔다. 🟢 **프롬프트 빌더는 양쪽이 공유한다**(`imagePromptFor`·`imagePromptForRequest`·`styleLineFrom`·`promptFor` 전부), 🟢 **스타일 줄도 `avoid` 까지 대칭이다** — 여기엔 고칠 게 없었다. 나온 것은 하나다.
  - 🟠 `episode-videos.service.ts` 의 `MOTION_SCENE_FIELDS` 가 *「video-preview.service.ts 의 SCENE_FIELDS 에서 number·narration 뺀 것」* 이라는 주석 아래 **열여섯 개를 손으로 적고 있었다.** 재 보니 **실제로 맞았다**(정본 17 − `number` = 16, 차이 0). **오늘 맞는 사본이라는 게 문제다.**
  - 🔴 **뒤처지면 어떻게 조용한지**: 정본에 칸이 하나 늘면 이 검증만 하나 모자라고, **그 칸이 빠진 장면이 `scenes()` 를 통과해 `promptFor` 에 도달한다.** 거기서 그 섹션은 빈 값으로 렌더링되고, **빈 섹션 필터가 조용히 지운다.** 결과는 불완전한 프롬프트로 산 영상이고, 아무도 거절하지 않는다.
  - 🟢 `SCENE_FIELDS` 를 export 해서 **파생**으로 바꿨다(`filter((field) => field !== "number")`). `number` 만 빼는 이유도 적었다 — 그건 바로 아래에서 **값까지 아는 더 강한 검사**(`item.number !== index + 1`)를 따로 받는다.
  - 🟢 그리고 **세 목록의 관계 자체**를 짝으로 고정했다: `STORY_SCENE_FIELDS`(모델이 반환해야 하는 것)와 `SCENE_FIELDS`(영상 프롬프트가 읽는 것)는 **`narration` 하나만 다르다** — 그건 드리프트가 아니라 결정이다. 한쪽에만 칸을 더하면 이제 빨개진다. 주입으로 확인. 백엔드 1548 → **1549**.
- [x] **🟠 내레이션만 모델 이름을 손으로 적고 있었다 — 이미지·스토리는 상수를 쓴다 (2026-09-06)**: 아직 안 본 유료 경로가 TTS 하나 남아 있어서 같은 렌즈를 댔다. 세 제공자 계열 중 **둘은 자기 상수에서 기록하고 하나는 아니었다.**
  - 🟠 `OPENAI_TTS_MODEL = "gpt-4o-mini-tts"` 가 **실제로 요청을 보내는 어댑터** 안에 있는데, 기록을 쓰는 **네 자리가 같은 문자열을 손으로** 적고 있었다(`local-narration-generation` · `narration-review` · `episode-narration` ×2).
  - 🔴 **기록은 「무엇이 이 오디오를 만들었나」를 말하는 자리다.** 모델을 바꾸면 어댑터는 새 모델을 부르고 **기록 넷은 전부 옛 이름을 계속 말한다** — 만들지도 않은 모델을 지목하는 기록이다. 대조군이 명확했다: `OPENAI_IMAGE_MODEL` 과 `OPENAI_STORY_MODEL` 은 기록 지점에서 상수를 쓴다.
  - 🟢 네 자리 전부 상수를 쓰게 했다. 리터럴은 이제 **어댑터 한 줄에만** 남는다(전 트리 확인).
  - 🟢 짝은 **보낸 요청의 `model` 과 기록된 `adapter` 가 같은 값인지**를 본다 — 문자열이 아니라 상수와 비교하므로 **리터럴을 다시 타이핑하는 것이 빨개지는 조건**이다. 주입: 기록만 다른 이름으로 바꾸면 빨개진다. 백엔드 1549 → **1550**.
- [x] **🟢 「장면 이어 그리기」 스위치 — 고쳐 놓고 켤 수 없던 기능에 손잡이를 달았다 (2026-09-06)**: 계약·백엔드·짝이 다 들어간 `sceneImageContinuityEnabled` 가 **화면에 없어서 아무도 못 켜는 상태**로 있었다. 요청에서 선택 필드이고 기본값이 꺼짐이라, 스위치가 없으면 도달할 방법이 없다 — **캡틴D가 「1번과 2번 영상이 전혀 안 이어진다」고 신고하신 그 문제가 고쳐진 채로 닿지 않았다.**
  - 🟠 **구역 판단을 적어 둔다.** 이 화면은 Cowork 구역이고, 그쪽이 625 ⑤ 에서 *「스위치 UI 시작합니다」* 라고 착수를 알렸다. 그래서 처음 두 틱은 손대지 않았다 — 「다 쓰인 채 멈춘 것」과 「쓰고 있는 것」은 다르기 때문이다. 🟢 **그런데 3시간 반 뒤 트리를 다시 보니 그쪽이 쓴 파일이 0개였다**(`git status --untracked-files=all apps/frontend` 가 비어 있다). **착수 선언만 있고 편집은 없었다** — 구역 규칙이 막으려는 동시 편집 사고의 조건 자체가 없었다.
  - 🟢 **문구는 그쪽 것을 그대로 썼다**(625 ⑤): *「앞 장면 그림을 참고해서 다음 장면을 그립니다. 같은 사물이 계속 나오는 영상에 켜세요. 장면마다 장소가 바뀌는 이야기라면 꺼 두세요.」* — 🟠 **끄는 경우를 같이 적는 이유도 그쪽 것이다**: 이건 좋은 기능이 아니라 **두 종류의 영상 중 하나를 고르는 것**이고, 켜는 쪽만 설명하면 전부 켜게 된다.
  - 🟠 내레이션 묶음에 한 줄로 넣지 않고 **자기 상자**로 뒀다 — 그 묶음은 소리에 관한 것이고 이것은 **그림이 무엇을 보고 그려지는지**를 정한다.
  - 🟢 짝 둘: 켜면 `sceneImageContinuityEnabled: true` 가 PATCH 에 실리고 **그전까지는 꺼져 있다**(기존 프로젝트가 계속 그래야 하는 답이다), 그리고 **끄는 경우 문장이 화면에 있다**. 주입 양방향으로 각각 빨개진다. 프론트 1380 → **1382**.
- [x] **🟢 꽃말 프리셋이 체인을 켜고, 장면을 넷으로 늘렸다 — 순서 조건이 풀린 뒤에 (2026-09-06)**: Cowork 이 617 ④ 에서 *「장면 2개는 이 기능에 너무 적다 — 새싹에서 활짝 핀 장미로 한 컷에 건너뛴다」* 고 했고, **① 이 들어가기 전에는 안 바꾸겠다**고 순서를 걸었다. 이유가 정확했다: **체인이 없으면 장면이 많을수록 화분이 더 여러 번 바뀐다.** 이제 둘 다 들어갔다.
  - 🟢 **프리셋이 `sceneImageContinuityEnabled: true` 로 만든다** — 이 설정이 존재하는 이유가 이 구분이다. 장소가 바뀌는 이야기는 앞 그림에 발목이 잡히고, 꽃말 릴스는 정반대다(한 송이·한 화분·한 빛·한 방향). 🟠 **프리셋의 `fullStory` 는 이미 「장면이 넘어가도 같은 화분」을 요구하고 있었다** — 이제야 그림이 그걸 지킬 수단을 갖는다.
  - 🟢 **2장면 × 10초 → 4장면 × 5초**: 씨앗 → 싹 → 봉오리 → 개화. **영상비는 같고**(총 길이·단가 동일) **이미지비만 $0.20 늘어난다** — 그게 빠져 있던 두 박자 값이다.
  - 🟠 **낡은 주석 하나를 같이 고쳤다**: `avoid` 위에 *「각 장면의 영상은 각자 따로 그린 그림에서 나오고, 앞 클립의 마지막 프레임을 아무것도 이어주지 않는다」* 고 적혀 있었다. **오늘까지 참이었고 이제 거짓이다.** 그 문장이 캡틴D가 신고하신 증상의 설명이었는데, 원인이 사라졌으므로 주석도 바뀌어야 한다 — 그대로 뒀으면 다음 사람이 **없어진 제약을 근거로 판단**한다.
  - 🟢 주입: 프리셋에서 체인 켜기를 빼면 짝이 빨개진다. 프론트 1382(짝 내용 교체, 개수 동일).
- [x] **🟢 화면이 `castCount` 를 읽는다 — 그리고 옛 이름을 지우려다 오늘 하루 종일 경고한 실패를 내가 만들 뻔했다 (2026-09-06)**: `763b201` 이 두 이름을 나란히 낸 목적이 이것이었다. 화면(`StoryPromptScreen`)과 응답 가드(`storyPromptApi`)를 `castCount` 로 옮겼다.
  - 🔴 **그 다음에 계약에서 `characterCount` 를 지웠고, 되돌렸다.** 타입체크·짝이 다 초록이 될 때까지 가서야 무엇을 하고 있는지 보였다: **캡틴D 브라우저에 떠 있는 빌드는 아직 옛 이름을 읽는 가드를 갖고 있다.** 별칭을 지우는 순간 그 페이지는 **멀쩡한 응답을 「서버 응답을 확인할 수 없습니다」로 거절**한다 — 오늘 아침 `durationSeconds === 5` 가 캡틴D의 영상 단계를 통째로 막은 그 모양이고, 내가 615 ⑤ 에서 한 번 틀렸다가 625 ② 에서 정정한 바로 그 논리다. **세 번째로 같은 자리에 왔고, 이번엔 코드를 다 쓴 뒤에 알아챘다.**
  - 🟢 **그래서 계약은 그대로 두고 프론트 절반만 낸다.** 별칭은 **이 빌드가 사람들이 실제로 돌리는 빌드가 된 뒤** 제거한다. 저장소 안의 상태(초록)와 사용자 화면의 상태(옛 빌드)는 다른 것이고, **전자만 보고 판단하면 안 된다** — 이게 오늘 세 번째로 확인된 사실이다.
  - 🟢 짝 셋: 화면이 **두 이름이 다른 값일 때 새 이름을 읽는지**(둘이 같으면 어느 쪽을 읽는지 알 수 없다), 가드가 **별칭이 없는 응답도 받는지**(별칭이 다음에 사라질 쪽이므로), 그리고 「글자 수」가 화면에 없는지. 주입: 가드를 옛 이름으로 되돌리면 빨개진다. 프론트 1382 → **1384**.
- [x] **🟠 오늘 아침 그 결함의 이웃을 고쳤다 — 값은 맞았고, 쌍둥이는 맞지 않았다 (2026-09-06, Cowork Round 617 ② 가 넘긴 것)**: `longProjectsApi.ts` 의 회차 영상 미리보기 가드가 `durationSecondsPerScene === 5 || === 10` 을 손으로 적고 있었다. **오늘 아침 단기 프로젝트의 쌍둥이 줄은 `=== 5` 였고, 캡틴D의 영상 단계를 통째로 막았다**(`9fff608`).
  - 🟠 **이건 값이 맞았다.** 그래서 Cowork 이 *「지금은 안 터지지만 상수가 아니라 손으로 쓴 목록」* 이라고 넘겼고, 그 판단이 옳았다 — `RUNWAY_CLIP_DURATIONS` 에 셋째 길이가 생기면 **멀쩡한 서버 응답을 「서버 응답을 확인할 수 없습니다」로 거절**한다.
  - 🟠 **그리고 그 줄 바로 옆의 `model` 검사는 이미 같은 이유로 넓혀져 있었다.** 오늘 아침 단기 쪽에서 본 것과 같은 모양이다 — **한 함수 안에서 한 줄은 배웠고 다음 줄은 안 배웠다.**
  - 🟢 `RUNWAY_CLIP_DURATIONS` 에서 파생하게 바꿨다. 짝은 **계약이 허용하는 길이 전부**를 돌고(둘째만 시험하면 「10만 받는」 가드도 통과한다) **허용하지 않는 7초는 여전히 거절**하는지 본다. 주입: 다시 `=== 5` 로 굳히면 빨개진다. 프론트 1384 → **1385**.
- [x] **🟠 꽃말 폼과 그 다음 화면이 같은 세 값을 연달아 묻고 있었다 — 폼 쪽을 뺐다 (2026-09-08, Cowork Round 661 ① 이 넘긴 것)**: 캡틴D가 *「장면이랑 길이 설정하는 부분이 겹친다」* 고 하셨고, **겹친다.** `CreateFlowerReelForm` 의 장면 수 · 장면당 길이 · 화면 비율 select 셋은 `handleCreated` 가 곧바로 여는 `ShortProjectSettingsScreen` 이 다시 묻는 바로 그 셋이다.
  - 🔴 **「나중에 바꾸기」가 아니라 「다음 화면」이었다.** 그리고 거기서 잠겨 있지도 않다 — 서버가 `sceneCountChangeable` 를 `stored.scenes.length === 0` 에서, `aspectRatioChangeable` 를 `stored.generated_images.length === 0` 에서 뽑는데(`projects.service.ts:153-154`), **방금 만든 프로젝트는 둘 다 0** 이다. 연속된 두 화면이 같은 걸 **똑같이 편집 가능한 채로** 묻고 있었다.
  - 🟠 **한쪽만 뺄 수 있고, 뺄 수 있는 쪽은 폼이다.** 설정 화면 것은 꽃말이 아닌 **모든** 단기 프로젝트의 유일한 자리다.
  - 🟢 **프리셋 값은 안 바뀌었다** — 4장면 × 5초 × 9:16. 4×5 를 고른 이유(씨앗·싹·봉오리·개화, 617 ④ · 618 ④)는 `useState` 위 주석에서 상수 위 주석으로 옮겼다. 바뀐 것은 **묻느냐 정해 두느냐** 하나다.
  - 🟠 **폼 절반을 뺄 수 있었던 조건 하나를 확인하고 적어 둔다**: `presetSettings` 의 `fullStory` 는 장면 수를 문장에 넣지 않는다. 그래서 다음 화면에서 장면 수를 바꿔도 **브리핑이 자기모순이 되지 않는다.** 이게 아니었으면 뺄 수 없었다.
  - 🟢 총액 문단이 **반응형에서 고정으로** 바뀌었으므로 *「설정 화면에서 장면 수나 길이를 바꾸시면 이 금액도 그만큼 달라집니다」* 를 같이 넣었다 — **총액이 조용히 총액이 아니게 되는 것**이 그 문단이 막으려고 생긴 실패다.
  - 🟢 짝: 옛 테스트는 select 를 움직여 금액이 따라오는지 봤고, select 가 없으니 **$1.45 고정 · `flower-shape-note` 문구 · 세 testid 가 전부 null** 로 바뀌었다. 프리셋의 모양을 지키는 건 이제 첫 테스트의 `settings.styleNotes.aspect === "9:16"` 한 줄이다. `56f7d01`.
- [x] **🟠 이미지 검토 화면이 「이동했습니다」라고만 하고 갈 방법을 안 줬다 (2026-09-08, Cowork Round 661 ② 가 넘긴 것)**: 마지막 장면을 승인하면 *「장면 이미지 N장이 모두 승인되어 영상 생성 확인 단계로 이동했습니다」* 가 뜨는데, **그 아래에 아무것도 없었다.** 화면에 남은 조작은 맨 위 「프로젝트로 돌아가기」뿐이었다.
  - 🔴 **화면이 거짓말을 한 게 아니다.** 이동은 진짜로 일어났다(`WAITING_FOR_VIDEO_CONFIRMATION`). **참인 것을 아무도 누를 수 없는 자리에 둔 것**이고, 이건 틀린 문장을 띄우는 것과 같은 등급이다.
  - 🔴 **`ContinueToNextStep` 을 일부러 안 썼다.** 그 컴포넌트는 스스로 `getProject` 를 부르고 **실패하면 조용히 null 을 반환**한다 — 이 화면에 붙이면 fetch 가 실패할 때마다 **캡틴D가 신고하신 그 화면(문장은 있고 버튼은 없음)이 정확히 재현된다.** 이 화면은 이미 `currentProject` 를 들고 있으므로 그걸 `resumeTarget()` 에 직접 넣었다. **다음 화면이 어디인지는 여전히 `resumeTarget` 한 곳만 안다** — 화면 이름을 두 번 타이핑하지 않았다.
  - 🟢 `onResume` 은 선택 prop 이고, 없으면 `nextStep` 이 null 이라 **버튼 자체가 안 생긴다** — 아무 일도 안 하는 버튼은 없는 것보다 나쁘다. `IMAGES_REVIEW` 에서도 안 뜬다(그 상태의 `resumeTarget` 은 **지금 보고 있는 이 화면**이다).
  - 🟢 짝 둘: 6번 승인 → 버튼이 뜨고 누르면 `onResume({ screen: "videoPreview", … })` 가 호출된다(라벨 문구는 `expect.any(String)` — 문구를 복사하면 `resumeTarget` 이 아니라 문자열을 검증하게 된다), 그리고 핸들러가 없으면 버튼이 없다. 프론트 1401 초록. `e146d73`.
- [x] **🔴 장면 자막이 명언 카드가 이미 고친 그 자리에 그대로 있었다 — 양쪽 절반 다 들어갔다 (2026-09-08, Cowork Round 664 가 넘긴 것)**: 캡틴D 신고 「자막이 잘 안 보이고 너무 내려가 있다」. `PHOTO_CARD_SUBTITLE_CENTER` 주석이 **왜 아래가 안 되는지를 이미 적어 두고 있었다** — Reels 가 캡션·계정명·버튼을 그 자리에 그린다. 카드는 그래서 0.40 으로 올라갔고, **장면 자막은 그 수정을 못 받았다.** `MarginV = 0.042×height` 로 아래 정렬이라 덩어리 전체가 화면 아래 13% 안에 있었고, 외곽선도 카드가 3→4 로 올린 뒤에도 **3 인 채**였다(꽃 릴스 장면은 전부 사진이다).
  - 🟠 **카드의 답을 그대로 쓰지는 못한다.** `subtitle-file.ts` 의 다른 이유가 지금도 참이다 — 화면 한가운데는 그 장면이 보여주려는 것이다. 그래서 **고정값이 아니라 손잡이**이고, 기본값은 캡틴D가 렌더 네 장을 보고 고른 **0.78 · 외곽선 4** 다. 🟠 **크기(0.033)는 일부러 안 움직였다** — 위치·외곽선·크기를 같이 바꾸면 다음 릴이 왜 달라졌는지 못 가린다.
  - 🔴 **두 레이아웃은 같은 `{ scale, center }` 라 타입으로는 안 갈린다.** 카드의 0.40 이 장면으로 새면 자막이 화면 한가운데 뜨고, 컴파일러는 조용하다. **갈라 놓는 건 이름 하나뿐이다**: 요청·저장 키·API 필드·렌더러 입력 전부 `sceneSubtitleLayout`, 그리고 `sceneSubtitleAss` 는 두 레이아웃을 **이름 붙은 키**로 받는다(뒤에 붙는 위치 인자 둘이면 서로 바꿔 넣어도 안 빨개진다 — 실제로 이 변경에서 옛 위치 호출 하나가 빨개졌고, 그게 이 모양이 작동한다는 증거다). 각 필드는 반대쪽 프로젝트에서 **무시가 아니라 거절**된다. 브랜드 타입은 안 썼다: 이 값들은 파싱된 JSON 으로 도착해서 생성 지점마다 캐스트가 필요하고, **캐스트는 검사를 끄는 자리**다.
  - 🟠 **좌우 여백을 높이에서 너비로 옮겼다.** 가로 여백은 가로 질문이다. `0.042×height` 는 세로 프레임에서 우연히 맞는 값 근처(81 vs 76)이고, 1920×1080 가로에서는 **45px** 을 준다 — 1920 폭 줄이 프레임 가장자리 2% 까지 간다. 이제 양쪽 레이아웃이 `SUBTITLE_SIDE_MARGIN_RATIO` 하나를 읽는다.
  - 🔴 **실제 FFmpeg + 배포 폰트로 재서, 이 라운드를 제안한 계측 하나를 정정한다.** `\an5\pos` 가 감긴 덩어리 전체를 중앙 정렬한다는 것은 맞다(1498 요청 → 잉크 중앙 1500.5). **그런데 0.85 는 「마지막으로 안전한 값」이 아니다** — 최대 크기에서 4줄 덩어리는 아래로 109px 이 남고, **6줄 덩어리는 그림의 마지막 줄에 잉크가 있다**(잘린다). 같은 6줄이 기본 0.78 에서는 99px 남는다. 즉 **범위는 손잡이의 한계이지 글자가 프레임 안에 남는다는 약속이 아니다.** 덩어리 높이가 문장에 따라 변하는 한 어떤 범위도 그 약속을 못 한다 — 그래서 **화면의 프레임 이탈 경고는 있으면 좋은 것이 아니라 이 설계를 성립시키는 부분**이다. `SCENE_SUBTITLE_CSS_RATIO` 를 카드의 측정 상수에서 **파생**시킨 것도 그래서다(폰트 파일의 성질이지 레이아웃의 성질이 아니다).
  - 🟢 주입: 아래 정렬을 되돌리면 **세 파일에서 짝 7개가 빨개진다.** 백엔드 1577 → **1584**. 계약·백엔드 절반만 낸 것이고, `VideoMergeScreen` 의 슬라이더·미리보기·경고는 Cowork 구역이다. 🟠 **`apps/frontend` 한 줄이 이 커밋에 있다** — `mergeVideos` 의 인자. 계약의 요청 커버리지 가드가 **이 앱이 이름조차 안 부르는 필드를 거절**하고, 그 필드를 쓸 화면은 이름이 정해지기 전에는 못 쓰기 때문이다. `80c5a0f`.
  - 🟢 **화면 절반 (2026-09-08, Cowork Round 668, `11396c2`)**: `SceneSubtitleFieldset` — 슬라이더 둘(범위는 공유 상수에서 오므로 병합이 거절할 값에 손잡이가 닿지 못한다), 프레임을 **실제 크기로 그리고 축소한** 미리보기, 넘치는 장면 **번호를 말하는** 경고. 작은 상자에 작게 그리면 1080폭이 감는 자리와 다른 자리에서 감기고, 줄 수가 곧 이 항목의 남은 질문이다. 🟢 살아 있는 앱과 실제 ffmpeg 대조: 줄 셋 다 같은 자리에서 끊기고 블록 중앙 1498 대 1500, 폭은 일정하게 ~2% 큰데 이는 **advance width 대 잉크 바운딩 박스**라는 계통 오차다.
  - 🔴 **경고의 판단을 DOM 에서 떼어 낸 이유 — jsdom 에는 레이아웃 엔진이 없다.** 거기서는 모든 `offsetHeight` 가 0 이라, 컴포넌트를 몰아 보는 짝은 **높이 0 인 블록을 넘겨받아 「무엇을 줘도 프레임 안」이라는 답을 얻고 초록이 된다** — 브라우저가 화면 밖에 글자를 그리는 동안. **0 만 보는 가드는 무엇에나 동의한다.** 그래서 결정은 `utils/subtitleOverflow.ts` 의 순수 함수가 하고 DOM 은 숫자 넷만 읽어 넘긴다. 🟠 **그 대신 vitest 로 못 덮는 것이 남는다: 「DOM 이 옳은 숫자를 주는가」** — 그건 위의 살아 있는 화면 대조로만 확인했다.
  - 🔴 **경고는 보고 있는 장면이 아니라 모든 장면을 잰다.** 카드는 사람이 친 한 줄이지만 릴은 **하나의 레이아웃이 N개의 문장에 걸리고, 넘치는 것은 그중 가장 긴 문장**이다. 화면에 띄운 장면만 재는 경고는 **정작 넘치는 그 장면에서 조용해진다** — 이 변경이 없애려던 결함(화면이 사실 아닌 말을 하는 것)과 같은 모양이다. 숨긴 층은 `visibility: hidden` 이다(`display: none` 은 높이가 안 잡혀 잴 것이 없어진다).
  - 🟠 **자막을 안 굽는 병합에는 값을 안 보낸다** — 손잡이를 보여줄 조건과 값을 보낼 조건이 같은 하나(`!photoCard && 자막 있는 장면 > 0 && subtitlesEnabled`)다. 자막이 꺼진 프로젝트도 내레이션 문장은 들고 있으므로 「내레이션이 있는가」는 틀린 질문이고, **안 그린 것에 대해 고른 위치를 저장하면 어떤 영상도 그렇게 만들어진 적 없는 값이 프로젝트에 남는다** — 저장된 레이아웃이 하는 유일한 약속이 그것이다.
  - 🔴 **통합에서 짝 하나를 고쳤다: 위쪽 넘침 짝이 넘치지 않는 숫자를 단언하고 있었다.** `centerY 576 · height 1000` 은 76~1076 으로 **프레임 안**이라, 그 짝은 자기가 이름 붙인 상황을 만들지 못한 채 빨갰다. center 하한이 0.30(576)이므로 **위쪽은 1152px 보다 큰 블록에서만 깨진다.** 경계 양쪽(1100 안 / 1300 밖)으로 고치고, 주석에 **이 끝은 잰 것이 아니라 만든 것**이라고 적었다 — 최대 크기에서도 여기서 렌더된 가장 긴 덩어리의 두 배가 필요하다. 아래쪽 끝이 이 손잡이를 필요하게 만든 진짜 경우다.
  - 🟢 통합 검증(main): typecheck 0, 백엔드 1584 pass | 1 skip, 프론트 **1401 → 1409**, shared 57, desktop 38. 유료 제공자 호출 없음.
- [x] **🔴 설정 화면이 「설정 완료 · 계속 진행하기」라고 적어 놓고 프로젝트 화면으로 갔다 (2026-09-08, Cowork Round 670 이 넘긴 것)**: 캡틴D — *「여기서 프로젝트로 돌아가기는 이상하지 않아? 다음 단계로 이어져야 하지 않나?」*. 신고하신 화면(나중에 다시 연 설정)은 라벨이 맞았고, **결함은 갓 만든 프로젝트 가지에 있었다** — 그 버튼은 계속 진행한다고 적고 `onBack` 을 불렀다. 668 의 이미지 검토 화면과 같은 결함군인데 **문구는 이미 옳았고 목적지만 없었다.**
  - 🟢 나가는 버튼 둘이 같은 말을 하고(같은 곳으로 가므로), 그 옆에 `ContinueToNextStep` 이 붙는다. 목적지는 `resumeTarget` 이 정하고 이 화면은 아무것도 안 정한다.
  - 🔴 **맨 위 폼이 저장 안 된 동안은 거절한다.** 세 섹션은 누를 때마다 저장되는데 폼은 「설정 저장」을 눌러야 하고, 그 규칙은 화면이 스스로 적어 두고 있다. 앞으로 가는 버튼을 그 위에 놓으면 **적어 둔 규칙이 함정**이 되며, 버려지는 값이 하필 **유료 대본이 무엇으로부터 생성되는지를 정하는 값**이다. 숨기지 않고 비활성으로 뒀다 — 숨기면 **다음 단계가 원래 없는 화면**처럼 보이고 그게 이 변경이 없애려는 모양이다. 스냅샷은 **저장 응답의 `settings`** 로 잡는다(보낸 것으로 잡으면 서버가 trim 한 순간 「저장하자마자 안 저장됨」이 된다).
  - 🟠 **668 ② 에서 이 컴포넌트를 피한 판단은 그대로다.** 그때 이유(`getProject` 실패 시 조용히 사라짐)가 여기서는 반대 결론을 낸다 — 이미지 검토 화면에서는 그게 **앞으로 가는 유일한 길**이라 사라지면 신고된 화면이 재현되고, 설정 화면은 나갈 길이 둘 더 있어 **지름길 하나를 잃을 뿐**이다. 그 문장을 코드 주석에 넣었다.
  - 🟢 통합에서 짝 둘을 고쳤다: `App.test.tsx` 가 **옛 라벨을 클릭하고 있었고**(이제 testid), 그리고 Cowork 이 지운 `settings-prefilled-note` 단언을 **되살렸다** — 그 문구는 화면에 그대로 있고, 지운 이유가 어디에도 없었다. `NarrationReviewScreen` 쪽 짝(`disabled` 를 안 넘기는 호출)도 확인, 초록. 프론트 1409 → **1410**. `826e095`.
- [x] **🔴 캡틴D: 「식물이 가만히 있고 성장을 안 한다」 — 서식의 브리핑 절반 (2026-09-08, Cowork Round 671 이 넘긴 것)**: 원인 넷이 전부 「움직이지 마라」 쪽을 가리켰고, 그중 서식이 쓰는 둘을 고쳤다. 브리핑은 **릴 전체의 호(弧)** 만 적고 **한 클립 5초 안에서 무엇이 일어나야 하는지**는 안 적었으며, 카메라 줄은 「거의 고정, 아주 느린 접근」이었다 — **느림 하나를 카메라와 피사체에 함께 적용**하게 만든 문장이다. 개나리 네 장면이 전부 `motion_speed: 느림` · `motion_intensity: 약함` · 느린 푸시인으로 돌아왔고, 그 두 칸은 Runway 프롬프트로 그대로 나간다.
  - 🟢 브리핑이 **타임랩스**를 이름으로 부르고, 형용사가 아니라 **첫 프레임과 마지막 프레임의 차이**로 적는다. 한 장면 한 단계. 카메라는 완전히 잠갔다 — **움직일 수 있는 게 식물밖에 안 남는다.**
  - 🟠 **나머지 셋은 안 건드렸다**: `story_generation.txt` 가 성장 장면에 어떤 `motion_speed` 가 맞는지 안내하지 않는 것, `Pacing` 을 한국어 그대로 보내는 것, 그리고 `STABILITY_RULE` 이 모든 프롬프트에 붙는 것. 마지막 것은 **명언 카드와 장기 에피소드도 받고 있고 그쪽은 지금 잘 된다** — 성장 샷 하나 때문에 전부에 걸린 줄을 건드리면 잘 되던 것을 깨는 쪽이 크다. 어느 원인이 결정적인지는 **안 쟀다**(유료 호출이라 못 돌린다). 다음 릴 하나면 이 절반만 바뀐 결과가 나온다.
  - 🟠 이미 만들어진 프로젝트에는 소급되지 않는다 — 서식은 생성 시점에만 설정을 쓴다. 프론트 1410(짝 내용 추가, 개수 동일). `999d0b1`.
- [x] **🟢 장면 자막을 레퍼런스 값으로 옮기고, 굵게 갈 길을 텄다 (2026-09-08, Cowork Round 672 가 넘긴 결정 셋)**: 캡틴D가 자막 스타일 레퍼런스를 주셨고 Cowork 이 재서 수치로 냈다. 크기 기본값 0.033 → **0.050**, 세로 위치 0.78 → **0.66**.
  - 🔴 **`scale.max` 는 올렸다(0.050 → 0.060) — Cowork 의 권고와 반대다.** 이유는 그쪽이 적어 둔 정밀도다: 환산이 **±0.003** 을 갖는데 천장이 기본값과 같으면 **그 오차가 한 방향으로만 열린다** — 작게 나온 판독은 사람이 고칠 수 있고 크게 나온 판독은 못 고친다. **여유는 쟀다**: 저장소가 만든 **서로 다른 내레이션 27개 전부**를 실제 FFmpeg 1080×1920 · 배포 폰트로 렌더해서, 0.050 에서 최악이 5줄·아래 여유 426px, 0.060 에서 5줄·381px. 디스크의 어떤 문장보다 1.8배 긴 합성 문장도 0.060 에서 94px 남는다. 🟠 **안 말하는 것**: 0.060 이 `center` 최대값(0.85)에서도 안전하다는 뜻은 아니다(가장 긴 문장이 16px 남는다). 그 구멍은 이미 0.050 에서도 6줄이 잘리던 그 구멍이고, 닫는 것은 미리보기의 이탈 경고다. `8d8a6ed`.
  - 🔴 **Bold 는 새 family 이름을 안 준다 — 한 이름 아래 파일 둘, 고르는 것은 스타일의 `Bold` 필드다** (근거 전문: `docs/06_DECISIONS.md` D-049). 이름으로 피할 길이 없다: 배포된 Medium 이 nameID 1 과 16 을 **둘 다 "Noto Sans KR"** 로 선언한다(파일에서 읽었다). 그래서 *「fontconfig 이 고르는 건 통제도 검증도 못 한다」* 던 절반을 **실제 FFmpeg 로 쟀다** — 한 family 아래 500·700 을 두면 `Bold: 0` 은 **500 만 있는 디렉터리와 바이트까지 같은 프레임**, `Bold: -1` 은 **700 만 있는 것과 바이트까지 같은 프레임**(합성 없음)이다. **명언 카드 본문은 Bold 파일이 들어와도 그대로다** — 이게 이 결정의 조건이었다.
  - 🟢 `fontFileForFamily(dir, family, weight)` 로 바꿨고, 가드는 **렌더러가 쓰는 ASS 스타일 줄에서 요청(family + Bold)을 읽는다.** bold 요청의 하한은 700 이라, **장면 스타일이 bold 를 요구하는 날 가드가 진짜 Bold 파일을 요구하며 빨개진다.** 백엔드 1584 → **1587**. `3c08a14`.
  - 🟠 **남은 것 — 파일이 들어와야 한다.** `Bold: -1` 로 뒤집는 것과 `fonts/NotoSansKR-Bold.ttf` 투입은 **같은 변경**이어야 한다: 파일 없이 플래그만 켜면 렌더가 조용히 **합성 굵게**로 바뀌고(위 측정의 셋째 줄), 그건 아무도 고른 적 없는 세 번째 얼굴이다. 그 다음이 Cowork 의 `SCENE_SUBTITLE_CSS_RATIO` 재계측 — 지금 0.690 은 Medium 에서 잰 값이고, **틀리는 방향이 나쁘다**(미리보기가 늦게 감겨 경고를 덜 띄운다).
- [x] **🟢 굵기 플래그를 뒤집었다 — 그리고 그 뒤에 온다던 비율 재계측은 필요 없었다 (2026-09-09, Cowork Round 674 가 파일을 넣고 675 로 답)**: `fonts/NotoSansKR-Bold.ttf` 가 들어왔다(Cowork 이 컨테이너에 있던 가변폰트를 `varLib.instancer` 로 wght=700 고정한 인스턴스다 — 구글 배포 static 그 자체는 아니라고 밝혀 왔다. usWeightClass 700 · fsSelection BOLD · fvar 없음 · 한글 11,172자 전부). 위 항목이 「같은 변경이어야 한다」고 적은 대로 **파일 투입과 `Bold: -1` 을 한 커밋에** 냈다.
  - 🔴 **뒤집은 스타일은 둘이다 — 장면의 `Default` 와 명언 카드의 `Body`. 바로 위 항목이 「명언 카드 본문은 그대로다」라고 적은 조건을 되돌린 것이고, 취향이 아니라 강제였다**: `SCENE_SUBTITLE_CSS_RATIO` 는 `PHOTO_CARD_SUBTITLE_CSS_RATIO.body` 에서 **파생**된다(비율은 레이아웃이 아니라 얼굴의 성질이라는 근거로). 한쪽만 굵히면 **두 레이아웃이 서로 다른 파일로 그리는데 미리보기가 읽는 숫자는 하나**가 되고, 그 하나는 둘 중 하나에 대해 반드시 틀린다. 카드의 `Quote`(Serif Bold)는 원래 `-1` 이라 그대로다.
  - 🟢 **플래그가 파일을 골랐다는 것을 프레임으로 확인했다**(대체 파일이 아니라 실물로, 1080×1920 · Fontsize 96): `Bold: -1` + `fonts/` 전체는 **Bold 파일만 있는 디렉터리와 sha256 까지 같은 프레임**, `Bold: 0` 은 **Medium 만 있는 디렉터리와 sha256 까지 같은 프레임**, `Bold: -1` + Medium 만은 **둘 중 어느 것도 아닌 세 번째 프레임**(libass 합성). 잉크 픽셀 36,352 → 39,506(+8.7%). 눈으로도 확연히 두껍다.
  - 🔴 **그리고 「그 다음」이라던 비율 재계측은 안 해도 됐다 — 재 보니 Bold 가 Medium 보다 넓지 않다.** 두 파일 다 `가` 를 **0.9200 em** 으로 보낸다(`hmtx`/`head` 에서 직접 읽었다). 실제 FFmpeg 재계측도 `body 0.6898 / 게시값 0.690`, `heading 0.6949 / 0.697` — **안 움직였다.** 위 항목의 🟠 가 예고한 방향(미리보기가 늦게 감긴다)은 **일어나지 않는다**: 굵기는 **획을 바꾸고 advance 를 안 바꾼다**. 잉크 bbox 의 가로 폭이 두 경우 동일하다(x 104–977).
  - 🟠 **다만 참인 조건이 있다 — 브라우저가 진짜 700 face 를 로드할 때만이다.** `styles.css` 에는 500 짜리 `@font-face` 뿐이고 `font-synthesis: none` 이 걸려 있어, 미리보기가 `font-weight: 700` 만 올리면 **아무 일도 안 일어난다**(500 face 를 그대로 그린다). 그 줄이 나중에 지워지면 이번엔 **브라우저 합성 볼드**가 들어오고 **그건 실제로 더 넓다** — 이 예외를 `domain.ts` 의 상수 주석에 적어 뒀다. 백엔드는 손댈 것이 없다: `FontsController` 가 `fonts/` 의 `.ttf` 를 이름으로 내주므로 Bold 는 이미 서빙된다.
  - 🟢 **짝도 낡지 않게 고쳤다**: `subtitle-font-metrics.test.ts` 가 나눗셈에 쓰는 em 을 **렌더러가 쓰는 스타일 줄에서 읽는다**(리터럴로 들고 있으면, 다음에 어떤 `Bold` 플래그가 움직여도 가벼운 얼굴의 em 으로 계속 나누면서 초록으로 남는다 — em 항 자체가 1년간 빠져 있던 것과 같은 모양의 침묵이다). 백엔드 1587(개수 동일, 단언 추가). `3a56577`.
- [x] **🟢 전 화면을 목업 느낌으로 다시 칠했다 — 그리고 그 과정에서 짝이 회귀 둘을 잡았다 (2026-09-09, Cowork Round 675·676·678 · 검증과 커밋은 내 몫)**: 사장님 지시 — *「화면 띄우는 곳 전부 다 바꾸고 하나도 빠짐없이」*. `apps/frontend` 만, 59개 파일(수정 52 · 신규 7). **기능 추가 없음, 새 색·새 토큰 없음** — `docs/05_DESIGN_SYSTEM.md` §3.2 팔레트와 §3.6 글로우 셋 안에서만 움직였다.
  - 🟢 **부품 넷**: `ScreenHeader`(16개 화면이 붙여넣기로 수렴해 있던 다섯 줄. 그 중 다섯은 이미 `text-slate-200` 으로 어긋나 있었다 — 디자인 시스템에 없는 색이다), `surfaces.ts`(같은 문자열이 21·15·19개 파일에 있던 버튼·카드 표면), `StepRibbon`(사이드바 열 → 본문 상단. *「지금 6단계 중 4단계」* 를 사이드바 열도 퍼센트 바도 이어하기 버튼도 각자 절반씩만 답하고 있었다), `Panel`(아직 아무 화면도 안 쓴다).
  - 🔴 **짝이 잡은 회귀 1 — 뒤로가기 버튼이 접근성 이름을 잃었다. 테스트가 낡은 게 아니었다.** 옛 마크업은 화살표를 `<span aria-hidden>` 에 담아 이름 **밖**에 뒀는데, `backLabel="← 프로젝트 목록으로"` 로 옮기면서 이름 **안**으로 들어왔다(스크린 리더가 "왼쪽 화살표 프로젝트 목록으로"로 읽는다). 🟢 **단언을 고치면 이 회귀가 초록으로 덮인다**고 적어 돌려보냈고, 화살표를 부품이 그리는 쪽으로 고쳐졌다. 🟢 **그러자 아무도 신고하지 않은 둘째가 같이 닫혔다**: 호출부 21곳 중 화살표를 쓴 곳 11 · 잊은 곳 10 이라 **화면 10개가 이미 화살표를 잃은 상태**였다. 부품이 그리면 잊을 수가 없다.
  - 🟠 **짝이 잡은 회귀 2 — 이건 내가 만든 낡음이다.** `PhotoCardSubtitleFieldset` 짝이 미리보기 본문 굵기를 `500` 으로 요구했는데, 그 근거였던 *「family 당 정적 파일 하나 — Serif Bold, Sans Medium」* 을 하루 전 내가 깼다(`3a56577`). **단언만 바꾸면 다음에 또 낡으므로** 규칙(*미리보기는 렌더가 실제로 쓰는 파일의 굵기를 부른다* — 안 변함)과 답(*Sans 는 둘이고 카드 본문은 700 쪽* — 변함)을 갈라 적어 달라고 했고, 그렇게 됐다.
  - 🟢 **`StepRibbon` 짝은 반대 판정이었다**: 번호 배지가 `aria-hidden` 이라 접근성 이름은 멀쩡했고, `textContent` 를 읽던 단언이 낡은 쪽이었다. `data-testid="step-ribbon-label"` 로 갔다. **두 판정을 가른 것은 「접근성 이름이 망가졌는가」 하나다** — 망가졌으면 코드를 고치고, 멀쩡하면 짝을 고친다.
  - 🟠 **Cowork 은 `vitest` 를 못 돌린다**(컨테이너 npm 403). 세 라운드에 걸쳐 빨간불 6 → 6 → 0 으로 좁혔고, 판정은 전부 이쪽에서 냈다. 프론트 1410(개수 동일). tsc·빌드 전 워크스페이스 초록. 유료 호출 0. `544d14f`.
  - 🟠 **남은 것**: 카드 안쪽을 `Panel` 로 바꾸는 작업, 전 화면 버튼 클릭 검증(작업 #76), 그리고 사장님이 릴 하나 만들어 확인하실 4가지.
- [x] **🟢 미리보기와 렌더가 폰트를 놓고 어긋날 수 있는데 아무도 안 보고 있었다 — 짝으로 만들었다 (2026-09-09)**: 어제 D-049 와 `domain.ts` 주석에 **문장으로만** 적어 둔 불변식이 있었다 — *「미리보기가 진짜 700 face 를 로드할 때만 비율이 안 움직인다는 결론이 참이다」*. 문장은 지켜지는지 아무도 안 본다. `subtitle-file.photo-card.test.ts` 에 짝을 붙였다: **ASS 스타일이 libass 에게 요청하는 모든 weight 가, `styles.css` 에 `@font-face` 로 선언돼 있고, 그 `src` 가 `fonts/` 가 배포하는 같은 파일을 가리켜야 한다.**
  - 🔴 **이 불변식은 두 워크스페이스 사이에 있어서 어느 쪽 짝도 혼자서는 못 본다.** 그래서 이 짝은 일부러 `apps/frontend/src/styles.css` 를 읽는다(수정은 안 한다). 그리고 이건 가정이 아니라 **이미 두 번 틀렸던 것**이다: 처음은 미리보기가 브라우저가 모르는 family 이름을 불러 시스템 폰트로 떨어진 것(캡틴D — *「글씨가 너무 얇아」*), 두 번째가 이번 `Bold: -1` 전환이다.
  - 🔴 **두 번째가 조용한 방향이 문제다.** `font-synthesis: none` 은 없는 weight 를 **안 만들어 준다** — 즉 미리보기는 조용히 500 을 계속 그린다. 화면은 멀쩡해 보이고, 영상보다 얇은 줄을 보여 주고, **이탈 경고를 늦게 띄운다.** 아무 데서도 예외가 안 난다. 이번엔 Cowork 이 `@font-face` 를 같이 넣어서 안 났지만, **안 넣었어도 초록이었다.**
  - 🟢 **비교하는 것은 숫자가 아니라 파일이다.** 선언이 어떤 weight 를 적든 **그 파일이 실제로 그 weight 이면** 된다 — 배포된 face 에 없는 weight 를 부르면 브라우저가 근접도로 고르게 되고, 그건 D-049 가 렌더 쪽에서 일부러 그만둔 그 추측이다. `font-synthesis: none` 자체도 단언한다(그게 「없으면 조용하다」를 만드는 절반이라서).
  - 🟢 **빨개지는지 확인했다** — 스타일시트를 세 가지로 망가뜨려 봤다: 선언 삭제 / 선언의 숫자가 파일과 다름 / `fonts/` 에 없는 파일을 가리킴. **셋 다 빨개진다.** 옛 가드가 인라인으로 하던 요청 파싱은 이제 둘이 공유한다 — 둘 다 **렌더러가 쓰는 스타일 줄**을 읽지, 여기 적힌 목록을 읽지 않는다. 백엔드 1587 → **1588**. `b3605f0`.
- [x] **🟢 장편 파이프라인 아홉 화면에 제목이 아예 없었다 — 단기만 고치고 장편을 안 본 결과다 (2026-09-09, Cowork Round 684·686·688·690)**: #76(전 화면 버튼 클릭 검증)을 시작하자마자 나왔다. 장편 프로젝트 상세·설정·회차 나누기·에피소드 여섯 화면이 **버튼 줄로 시작하고 작품 제목은 항목 목록 안의 「제목」 값**이었다. `<h1>` 0개, `<h2>` 0개.
  - 🔴 **모양이 중요하다: 단기 쪽은 674 에 고쳤고 장편은 한 번도 안 봤다.** 한쪽을 고치면 **끝난 것처럼 읽혀서** 나머지 절반이 조용히 벌어진다. 지금까지 이 모양이 셋이다(상세 제목 · 파이프라인 제목 · 그리고 카드 제목 규격은 일괄이라 우연히 같이 됐다). #76 순서에 **「단기에서 고친 것을 장편에서 확인한다」**를 넣었다.
  - 🔴 **`LongProjectDetail` 은 처음에 `ScreenHeader` 의 사본으로 갔다** — 표면·윗변 하이라이트·모서리 글로우·h1 발광 점·뒤로가기까지 **클래스 문자열이 전부 같은** 25줄이었다. 대조해서 돌려보냈고, 물은 것은 「왜 부품을 안 썼나」가 아니라 **「부품이 못 담는 모양인가」**였다. 답이 「예」였다(제목 아래 사실 두 칸을 놓을 자리가 없었다). 🟢 **그래서 사본이 아니라 부품이 넓어졌다** — `meta` 슬롯, `actions` 와 분리(하나는 누르는 것, 하나는 읽는 것). **이 변경의 값은 정돈이 아니라, 화살표의 `aria-hidden` 규칙이 부품 안으로 돌아온 것**이다: 사본이 없으면 그 규칙을 화면이 기억할 필요가 없다.
  - 🟢 **§2.5 의 「발광하는 점은 화면당 하나」가 이제 구조적으로 참이다** — `shadow-[0_0_6px_rgba(216,180,254,0.7)]` 가 저장소에 **정확히 한 번**, `ScreenHeader` 안에만 있다. 남아 있던 점들은 h3 옆과 접기 라벨 앞이었고 둘 다 화면 제목이 아니다.
  - 🟠 **중간에 초록이던 짝 하나가 깨졌다가 돌아왔다.** Cowork 이 문자열 치환을 「첫 일치」로 돌려 **839(장편) 대신 126(단기)** 을 바꿨고, 「고쳤다」고 보고한 라운드에서 실패가 1 → 3 으로 늘었다. 🔴 **`vitest` 를 못 돌리는 쪽이 「짝을 고쳤다」고 단정한 것**이 원인이고, 다음부터는 **바꾼 줄의 내용을 붙여서** 보고하기로 했다 — 그러면 이쪽이 트리와 대조할 수 있다.
  - 🟢 **주석만 있던 결정 둘이 짝을 얻었다**: 「전체 에피소드는 헤더가 아니라 단계 요약이 갖는다」(Cowork 이 스스로 되돌릴 뻔한 것을 브라우저에서 눈으로 잡았다 — 두 번 기댈 것이 못 된다), 그리고 「갈 곳 없는 단계는 버튼이 아니라 표식」. 프론트 1410. `9f1c07d`.
- [x] **🟠 장편 회차에는 자막 손잡이가 없다 — 결정이 필요하다 (2026-09-09 발견, 안 고쳤다 → 2026-09-11 닫음)**: Cowork 이 화면 쪽에서 찾아낸 「단기만 고치고 장편을 안 봤다」가 **렌더 쪽에도** 있었다. `episode-video-merge.service.ts` 의 `mergeScenes()` 가 `MergeSceneInput` 을 만들 때 `sceneSubtitleLayout` 을 안 넘긴다 — 단기 쪽(`video-merge.service.ts`)은 넘긴다.
  - 🔴 **그 자리 주석은 *「exactly the same gating as video-merge.service.ts's identical mergeScenes()」* 라고 적혀 있었다.** 손잡이가 생긴 날 그 문장이 거짓이 됐는데 아무도 안 봤다 — **「확인된 것처럼 들리는 문장」이 차이를 안 보이게 만든 것**이다. 문장을 사실로 고치고 차이를 이름으로 적었다(`2fcab62`).
  - 🟢 **잃는 것은 「손잡이」이지 「모양」이 아니다.** `sceneSubtitleAss` 가 `DEFAULT_SCENE_SUBTITLE_LAYOUT` 으로 떨어지고, 그 기본값이 캡틴D가 레퍼런스에서 고른 **0.050 / 0.66** 그대로다. 그래서 **오늘 회차와 릴은 같아 보인다.** 회차가 못 하는 것은 **조절**이다.
  - 🔴 **안 고쳤다. 이건 정리가 아니라 결정이라서다** — 회차에 손잡이를 주려면 화면도 있어야 하고(`apps/frontend`), 그 화면이 어디에 붙는지(회차 설정? 병합 화면?)가 정해져야 한다. 🟠 **그리고 아무도 「회차는 조절 못 한다」고 정한 적이 없다** — 그냥 단기 쪽만 만들어졌다. 사장님 판단이 필요한 자리다.
  - 🟠 **`docs/01_CURRENT_PRODUCT_SPEC.md` 의 화면 대응표가 이걸 못 잡는다** — 표는 화면 짝을 보고, 이건 **서비스 짝**이다. 표의 범위를 렌더까지 넓힐지도 같이 정할 일이다.
  - 🟢 **2026-09-11 — 결정은 이미 났고 양쪽 다 들어가 있었다. 이 칸만 안 닫혀 있었다.** 사장님이 손잡이를 승인하셨고 구현이 디스크에 있다: 화면은 `LongEpisodeVideoMergeScreen.tsx:255` 가 `SceneSubtitleFieldset` 을 `previewImageUrl` 과 함께 그리고, 렌더는 `episode-video-merge.service.ts` 의 `mergeScenes()` 가 `sceneSubtitleLayout` 을 `MergeSceneInput` 에 넘긴다(`:226`) — 요청 검증(`:299`)과 락 안의 render(`:383`)까지 이어져 있다.
  - 🔴 **닫는 데 이틀이 걸린 이유가 이 항목의 값이다.** 「결정이 필요하다」고 적힌 칸은 **아무도 다시 재지 않는다** — 결정이 난 뒤에도 목록에서는 계속 미결로 읽히고, 이 세션은 실제로 그것을 대기 항목으로 들고 있었다. 결정을 기다리는 항목은 **결정이 난 날 닫는 것이 그 결정의 일부**다.
- [x] **🔴 되돌리기가 자기만의 락 키를 쓰고 있었다 — 막으라고 만든 경합을 하나도 안 막고 있었다 (2026-09-09)**: 어제 만든 서비스 짝 표에서 「대조 안 함」 줄들을 실제로 대조하러 들어갔다가 나왔다. **표의 줄이 아니라 가는 길에 나온 것이고, 더 무겁다.**
  - 🔴 `project-lock.ts` **자신의 주석**이 같은 파일을 만지는 셋을 이름으로 적고 있다 — *「the merge, the version restore and the Instagram publish」*. 그중 **둘만** `FINAL_VIDEO_LOCK_KEY` 를 쓰고, **되돌리기는 `"videos:restore"`** 를 썼다. 락은 **키별**이라 자기만의 키는 **아무도 안 막는다.**
  - 🔴 **그래서 실제로 가능했던 일**: 병합이 `instagram_reel.mp4` 를 쓰는 중에 되돌리기가 그 파일을 갈아치우는 것, 그리고 발행이 보낼 바이트를 이미 읽은 뒤에 갈아치우는 것 — **인스타에는 한 컷, 디스크에는 다른 컷, 기록은 나중에 쓴 쪽**. D-005 가 적어 둔 바로 그 피해이고, **「절대 파괴적이지 않다」고 문서화된 그 버튼**을 통해 도달한다.
  - 🟠 **같은 결함이 같은 모양으로 이미 한 번 잡혔다** — 발행의 기록 지우기 경로가 그랬고, **그것도 「둘은 같은 락」이라고 적힌 주석 바로 아래**였다. **주석은 락이 아니다.**
  - 🟢 **키를 공유하면 기다림이 진짜가 되고, 그러면 나머지 절반이 드러난다: 타임아웃이 안 매핑돼 있었다.** 옛 키에서는 되돌리기가 다른 되돌리기 말고는 기다릴 일이 없어 사실상 타임아웃이 안 났다. 새 코드를 줬다(`VIDEO_LIBRARY_RESTORE_IN_PROGRESS`) — `RESTORE_NOT_ALLOWED` 를 안 쓴 이유는 그건 **「상태가 금지한다」**는 뜻이고 그 사실은 기다려도 안 바뀌는데, 이건 바뀌기 때문이다.
  - 🟢 **짝은 되돌리기 자신의 키가 아니라 `FINAL_VIDEO_LOCK_KEY` 를 잡고 단언한다** — 그래야 누가 사설 키를 되돌려 놓으면 빨개진다. **실제로 되돌려서 확인했다**: 그 상태에서는 되돌리기가 잡힌 락을 뚫고 파일을 썼다. 백엔드 1588 → **1589**. `6af20e1`.
  - 🟠 **화면 문장은 아직 없다** — `apps/frontend` 몫이라 Cowork 에 넘겼고, `error-code-reach.test.ts` 의 허용 목록에 **「결정이 아니라 공백」**으로 적어 뒀다. 화면이 그 코드를 부르는 순간 그 항목이 빨개지므로 지울 때를 가드가 알려 준다.
- [x] **🔴 장편 회차 되돌리기도 같은 사설 락 키를 쓰고 있었다 — 한 시간 전에 적은 규칙이 그대로 찾아냈다 (2026-09-09)**: `docs/03_TEAM_WORKFLOW.md` 에 *「한 곳을 고쳤으면, 그 근거 문장이 부르는 나머지 이름도 그 자리에서 센다」*를 적었다. 단기 되돌리기를 찾아낸 그 문장(*「the merge, the version restore and the Instagram publish」*)에 그 규칙을 적용했더니 장편 쪽이 나왔다.
  - 🔴 `episode-videos.service.ts` 의 `restoreVersion` 이 `videos_restore_${number}` 를 썼다. **디렉터리는 맞고 키가 틀렸다** — 회차 병합과 회차 발행은 **같은 디렉터리에** `FINAL_VIDEO_LOCK_KEY` 를 건다. 그리고 병합 쪽 주석이 깨지고 있던 규칙을 그대로 적어 두고 있다: *「the post must never be built from a cut this render is replacing」*. **되돌리기가 갈아치우는 게 정확히 그 컷이다.**
