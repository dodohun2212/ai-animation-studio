...
- [x] **🟢 꽃말 릴스 — 대본을 사람이 쓰는 단기 프로젝트 (2026-09-06, 캡틴D 주문)**: *"꽃말의 유래와 의미를 릴스로."* 씨앗이 심기고 꽃이 피는 장면들, 장면마다 자막, 9:16·16:9, 배경음악.
  - 🟠 **새 파이프라인을 만들지 않았다.** 이미지 → 영상 → 병합 → 게시가 전부 그대로 쓰인다. 다른 것은 하나뿐이다: **대본이 유료 호출이 아니라 사람 손에서 온다.** 캡틴D가 직접 쓰기로 하신 이유는 비용이 아니라 정확성이다 — 꽃말의 유래는 **사실**인데, 사실을 요구받은 이야기 모델은 **사실처럼 생긴 것**을 돌려준다.
  - 🔴 **그래서 명언 카드와 반대로, 표식을 붙이지 않았다.** 명언 카드는 파이프라인 다섯 단계를 건너뛰어 목록에서 갈라야 했고(같은 날 `e291de6`), 꽃말 릴스는 **모든 단계를 실제로 거치므로** 진행률 막대가 참말을 한다. **같은 기준의 반대 답**이라, 두 항목을 같이 읽으면 규칙이 보인다: *가르는 것은 종류가 아니라 「막대가 세는 단계를 실제로 밟는가」다.*
  - 🔴 **화면에서는 안 보이는 절반이 이 작업의 핵심이었다.** `WaitingForAssetMappingReview` 로 앉히는 것만으로는 **못 쓰는 프로젝트**가 나온다 — 스토리 생성은 상태를 바꾸면서 `mappings.beginReview()` 로 **매핑 검토 기준선**도 세우는데, 그게 없으면 `script_fingerprint` 가 `""` 로 읽히고 승인이 `no_baseline` 로 거절한다(캡틴D가 부딪혀 멈추셨던 자리, Cowork Round 533). 응답에 열린 `review` 를 실어 보낸다. 주입: `beginReview` 를 빼면 짝 둘이 빨개진다.
  - 🟠 **`narration` 플래그를 일부러 안 넣었다** — `caption` 을 장면의 나레이션 텍스트로 저장해 두면 음성 생성 여부는 **자기 확인 절차가 있는 나레이션 화면**이 정한다. 여기 플래그를 두면 만들기 버튼 뒤에 유료 단계가 숨는다.
  - 🔴 **길이 한도는 계약에 산다**(`FLOWER_CARD_*_MAX_LENGTH` 넷). 처음엔 서비스 안 지역 상수로 뒀는데, 사본이면 **화면이 「괜찮다」 하고 서버가 거절**하고 — 꽃말 릴스는 여러 장면을 한 자리에서 쓰는 폼이라 그 대가가 문장 하나가 아니라 **폼 전체**다. 짝은 경계 양쪽을 다 본다: **정확히 초과는 거절, 정확히 도달은 통과**(카운터의 마지막 글자가 닿을 수는 있는데 쓸 수는 없으면 그것도 거짓말이다).
  - 🟠 **경로도 계약에서 온다**(`API_ROUTES.flowerCards`) — 컨트롤러가 답하는 경로와 같은지 짝으로 묶었다. 그리고 접두어가 늘자 `devProxy.test.ts` 가 잡았다: 프록시 항목이 없으면 개발 브라우저에서 만들기 버튼이 **`index.html` 을 받아 JSON 파싱하다 죽고, 아무 데도 「백엔드에 못 닿았다」고 안 적힌다.** 접두어를 추가한 쪽이 프록시도 추가한다.
  - 🟢 **곁다리로 화면이 하던 거짓말 하나가 고쳐졌다**: 「새 프로젝트」가 *"한글은 폴더 이름으로 쓸 수 없습니다"* 라고 했는데, **두 검증기 다 처음부터 유니코드 문자를 허용한다**(`/^[\p{L}\p{N}_-]+$/u`, 파이썬 `isalnum()` 이식). 🔴 **거짓말이 거절로 나타나면 누군가 신고하지만, 기능 축소로 나타나면 아무도 신고하지 않는다** — 캡틴D는 명언 카드에서 한글 이름을 쓰고 계셨으니 두 화면이 반대 말을 하는 걸 겪으면서 **덜 되는 쪽을 그냥 피해 가고 계셨다.**
  - 🔴 **남은 것 하나**: `CreateLongProjectForm.tsx` 는 아직 그 문장을 그대로 갖고 있다(라벨 `폴더 이름 (영문·숫자)`, 오류 문구도). 장기 프로젝트는 **같은 `isSafeProjectId` 를 쓰므로 한글을 받는다** — 즉 이제 두 폼이 서로 반대 말을 한다. 짝은 사실에 맞춰 두었고(그 라벨을 그대로 찾는다), 문구는 화면 담당 판단이다.
  - 백엔드 1533(+13) · 프론트 1370(+14) · 계약 57 · 데스크톱 37.
