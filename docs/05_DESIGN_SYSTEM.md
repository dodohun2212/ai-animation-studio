...
1. **작업 도구다.** 이 앱은 감상용 랜딩페이지가 아니라 6장면 파이프라인을
...
- **유료 요청을 전송하는 버튼은 반드시 주 CTA 스타일**이고, 라벨에 행동과
  대상을 명시한다 ("영상 6개 생성 전송" — "확인" 같은 모호한 라벨 금지).
...
장면 목록은 항상 `grid gap-3 sm:grid-cols-2 xl:grid-cols-3` — 6개가
2×3 또는 3×2로 정확히 떨어진다. 장면 카드 구성(위→아래):
...
- 전체 확정 현황을 상단에 요약 (`6장면 중 4장면 확정` —
  `text-sm text-slate-300 tabular-nums`).
- 병합 CTA는 6/6 확정 전까지 disabled + 이유를 보조 텍스트로 표시.
